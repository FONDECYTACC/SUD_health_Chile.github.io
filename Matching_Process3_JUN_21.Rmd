...
flexsurv_cumhaz_a <- flexsurv::msfit.flexsurvreg(sel_param_fits_a,#sel_param_fits, 
                                             newdata = newdat_a,
                                             t = seq(0, max(ms_d_match_surv$time,na.rm=T)),
                                             #B = n_iter/50,
...
cumhaz_data_a_1096 <- rbind(data.frame(msf1$Haz,
                                model = "NP Cox"),
...
cumhaz_data_a_1096$trans <- factor(cumhaz_data_a_1096$trans,
                            levels = seq(1, 4),
                            labels = trans_lab_four_st$paste)
cumhaz_data_b_1096 <- rbind(data.frame(msf0$Haz,
                                model = "NP Cox"),
                      data.frame(cox_cumhaz_b$Haz,
                                model = "Cox"),
                      data.frame(flexsurv_cumhaz_b$Haz,
...
The cumulative hazards obtained are used to simulate the times at which patients transition between health states, that is,  state occupancy probabilities with a “clock-reset” model at the times specified by `r length(seq(0, max(ms_d_match_surv$time,na.rm=T))) %>% format(big.mark=",")` points.

We used a number of `r format(n_iter/20, big.mark=",")` resamples of trajectories.
...
We estimated the median of stay, percentiles 25, 75 and confidence intervals based on `r (n_iter/20) %>%  format(big.mark=",")` resamples, by simulating a large sample of individuals from the model depending on the type of program they were admitted at baseline.
