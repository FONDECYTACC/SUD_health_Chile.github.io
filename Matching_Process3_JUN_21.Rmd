...
newdat_a_1096 <- data.table::data.table(tipo_de_plan_res_1= factor(c(rep(1,1*n_trans2))),
  #comp_status= factor(rep(c("Therapeutic discharge","Discharge without clinical advice"),2)),
    TD_1=rep(0,), 
  TD_2=rep(0,), 
  TD_3=rep(0,),
  TD_4=rep(0,),
  arrival=rep(round((365.25*3),0),),
  strata= rep(1:n_trans2,1)
                       )
newdat_b_1096 <- data.table::data.table(tipo_de_plan_res_1= factor(c(rep(0,1*n_trans2))),
  #comp_status= factor(rep(c("Therapeutic discharge","Discharge without clinical advice"),2)),
    TD_1=rep(0,), 
  TD_2=rep(0,), 
  TD_3=rep(0,),
...
# fits_c_ggam2 fits_c_rp032/fits_c_logn2 fits_c_rp022/fits_c_logn2 fits_c_genf2/fits_c_rp022
#dists_w_covs_4s_w_arrival

invisible(c("dists_w_covs_4s_w_arrival ver para ver la distribución"))
...
sel_param_fits_a <- vector(length = n_trans, mode = "list") 
    sel_param_fits_a[[1]]<- fits_c_genf2[[1]] #fits_c_genf2[[1]] 
    sel_param_fits_a[[2]]<- fits_c_genf2[[2]]
    sel_param_fits_a[[3]]<- fits_c_logn2[[3]] #Porque gen gamma y gen f no convergen
...
sel_param_fits_b <- vector(length = n_trans, mode = "list") 
    ...
    sel_param_fits_b[[3]]<- fits_c_rp032[[3]]
...
cox_cumhaz_b <- mstate::msfit(cox_fits, 
                            newdata = newdat_b,
                            trans = trans_matrix, variance = T)
cox_cumhaz_a_1096 <- mstate::msfit(cox_fits, 
                            newdata = newdat_a_1096,
                            trans = trans_matrix, variance = T)
cox_cumhaz_b_1096 <- mstate::msfit(cox_fits, 
                            newdata = newdat_b_1096,
                            trans = trans_matrix, variance = T)
```
...
flexsurv_cumhaz_a_1096 <- flexsurv::msfit.flexsurvreg(sel_param_fits_a,#sel_param_fits, 
                                             newdata = newdat_a_1096,
                                             t = seq(90, 1826, length.out= 180),
                                             #B = n_iter/20,
                                             trans = trans_matrix, variance = F)
...
flexsurv_cumhaz_b_1096 <- flexsurv::msfit.flexsurvreg(sel_param_fits_a,#sel_param_fits, 
                                             newdata = newdat_b_1096,
                                             t = seq(90, 1826, length.out= 180),
                                             #B = n_iter/20,
                                             trans = trans_matrix, variance = F)
...
flexsurv_cumhaz_rp_a_1096 <- flexsurv::msfit.flexsurvreg(sel_param_fits_b,#sel_param_fits, 
                                             newdata = newdat_a_1096,
                                             t = seq(90, 1826, length.out= 180),
                                             #B = n_iter/20,
                                             trans = trans_matrix, variance = F)

flexsurv_cumhaz_rp_b_1096 <- flexsurv::msfit.flexsurvreg(sel_param_fits_b,#sel_param_fits, 
                                             newdata = newdat_b_1096,
                                             t = seq(90, 1826, length.out= 180),
                                             #B = n_iter/20,
                                             trans = trans_matrix, variance = F)
...
#seq(90, 1826, length.out= 180)

#msf1 tipo de plan 1 ==1 
#msf0 tipo de plan 1 ==0
cumhaz_data_a <- rbind(data.frame(msf1$Haz[msf1$Haz$time<=1826,],
                                model = "NP Cox"),
...
cumhaz_data_b <- rbind(data.frame(msf0$Haz[msf0$Haz$time<=1826,],
                                model = "NP Cox"),
...
cumhaz_data_a_1096 <- rbind(data.frame(msf1$Haz[msf1$Haz$time<=1826,],
                                model = "NP Cox"),
                      data.frame(cox_cumhaz_a_1096$Haz,
                                model = "Cox"),
                      data.frame(flexsurv_cumhaz_a_1096$Haz,
                                model = "Parametric"),
                      data.frame(flexsurv_cumhaz_rp_a_1096$Haz,
                                model = "Parametric (RP)"))
cumhaz_data_a_1096$trans <- factor(cumhaz_data_a_1096$trans,
                            levels = seq(1, 4),
                            labels = trans_lab_four_st$paste)
cumhaz_data_b_1096 <- rbind(data.frame(msf0$Haz[msf0$Haz$time<=1826,],
                                model = "NP Cox"),
                      data.frame(cox_cumhaz_b$Haz,
                                model = "Cox"),
                      data.frame(flexsurv_cumhaz_b$Haz,
                                model = "Parametric"),
                      data.frame(flexsurv_cumhaz_rp_b_1096$Haz,
                                model = "Parametric (RP)"))
cumhaz_data_b_1096$trans <- factor(cumhaz_data_b_1096$trans,
                            levels = seq(1, 4),
                            labels = trans_lab_four_st$paste)
```
...
ggplot()+
  ...
  #"Cox",
  scale_linetype_manual(name= "Modality",values=c("dashed","solid"), labels=c("Residential","Outpatient"))+ #dashed, a; solid, b
...
fig_diff_mod_ab_1096<-
ggplot()+
  geom_step(data=reshape2::melt(cumhaz_data_a_1096 %>% dplyr::filter(model!="Cox"),id.vars=c("time","trans","model")), aes(time, value, color= model,linetype="dashed"),size=1, alpha=.65) + 
  geom_step(data=reshape2::melt(cumhaz_data_b_1096%>% dplyr::filter(model!="Cox"),id.vars=c("time","trans","model")), aes(time, value, color= model,linetype="solid"),size=1, alpha=.65) + 
  facet_wrap(trans~., ncol=1, scales="free_y") + 
  scale_color_manual(name ="Model",values=c("#e66101","#fdb863","#b2abd2", "#5e3c99"),labels= c("NP Cox","Par","Par(RP)"))+
  #"Cox"
...
```{r cum_haz22, eval=T, echo=T, paged.print=TRUE, fig.height=30, fig.width=10, fig.cap="Figure 1b. Estimate of Cumulative Hazards, Five States Model, 2nd Patient (3 years arrival)", fig.align="center", error=T}
fig_diff_mod_ab_1096+ theme(legend.position=c(.9,.585))
```

```{r cum_haz23, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 1c. Estimate of Cumulative Hazards, Five States Model, 1st & 2nd Patient", fig.align="center", error=T}
ggarrange(fig_diff_mod_ab+ theme(legend.position="none")+ggtitle("A) Arrival at 0 days"),
          fig_diff_mod_ab_1096+ theme(legend.position=c(.87,.585))+ ylab("")+ggtitle("B) Arrival at 3 years"))
```
...
for(i in c(90,183,365,1096,1826)){
  ...
  flexsurv::pmatrix.simfs(x = sel_param_fits_a,
                        ...
                        trans = trans_matrix) %>% 
  assign(paste0("pmatrix1_t_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  flexsurv::pmatrix.simfs(x = sel_param_fits_b,
                        ...
                        trans = trans_matrix) %>% 
  assign(paste0("pmatrix1_t_rp_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  flexsurv::pmatrix.simfs(x = sel_param_fits_a,
                        ...
                        M = 1e5,#Number of individual trajectories to simulate.
                        trans = trans_matrix) %>% 
  assign(paste0("pmatrix1_t_a_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  flexsurv::pmatrix.simfs(x = sel_param_fits_b,
                        ...
                        cores= 8,#N of processor cores
                        trans = trans_matrix) %>% 
  assign(paste0("pmatrix1_t_b_1096_",i),.,envir=.GlobalEnv)
}
...
We estimated the median of stay, percentiles 25, 75 and confidence intervals based on `r (n_iter/10) %>%  format(big.mark=",")` resamples, by simulating a large sample of individuals from the model depending on the type of program they were admitted at baseline.
...
#n_iter=12
fmsm_sel_param_fits_a<-
    fmsm(sel_param_fits_a[[1]],sel_param_fits_a[[2]],sel_param_fits_a[[3]],sel_param_fits_a[[4]], trans=trans_matrix)

fmsm_sel_param_fits_b<-
    fmsm(sel_param_fits_b[[1]],sel_param_fits_b[[2]],sel_param_fits_b[[3]],sel_param_fits_b[[4]], trans=trans_matrix)

#(90,183,365,1096,1826
...
simfinal_fmsm(fmsm_sel_param_fits_a, #Object returned by fmsm, representing a multi-state model formed from transition-specific time-to-event models fitted by flexsurvreg.
              newdata = rbind.data.frame(newdat_a[1,],newdat_b[1,])[,-"strata"], #Data frame of covariate values, with one column per covariate, and one row per alternative value.
              t= 90, #itempo máximo al que simular, de manera que el resumen se toman de la muestra de individuos en los datos simulados quienes están en el estado absorvente a ese tiempo.
              probs = c(0.025,0.25, 0.5, 0.75,0.975), 
              #M=100000, #necesita grandes números
              B=n_iter/10, #Pondría 1,000
              cores=8
              )
### Error in do.call("rbind", quants): second argument must be a list
invisible(c("Error"))
set.seed(1234)
simfinal_fmsm_183d<-
simfinal_fmsm(fmsm_sel_param_fits_a, #Object returned by fmsm, representing a multi-state model formed from transition-specific time-to-event models fitted by flexsurvreg.
              newdata = rbind.data.frame(newdat_a[1,],newdat_b[1,])[,-"strata"], #Data frame of covariate values, with one column per covariate, and one row per alternative value.
              t= 183, #itempo máximo al que simular, de manera que el resumen se toman de la muestra de individuos en los datos simulados quienes están en el estado absorvente a ese tiempo.
              probs = c(0.025,0.25, 0.5, 0.75,0.975), 
              #M=100000, #necesita grandes números
              B=n_iter/10, #Pondría 1,000
              cores=8
...
simfinal_fmsm(fmsm_sel_param_fits_a, #Object returned by fmsm, representing a multi-state model formed from transition-specific time-to-event models fitted by flexsurvreg.
              newdata = rbind.data.frame(newdat_a[1,],newdat_b[1,])[,-"strata"], #Data frame of covariate values, with one column per covariate, and one row per alternative value.
              t= 365, #itempo máximo al que simular, de manera que el resumen se toman de la muestra de individuos en los datos simulados quienes están en el estado absorvente a ese tiempo.
              probs = c(0.025,0.25, 0.5, 0.75,0.975), 
              #M=100000, #necesita grandes números
              B=n_iter/10, #Pondría 1,000
              cores=8
...
set.seed(1234)
simfinal_fmsm_90d_1096<-
simfinal_fmsm(fmsm_sel_param_fits_a, #Object returned by fmsm, representing a multi-state model formed from transition-specific time-to-event models fitted by flexsurvreg.
              newdata = rbind.data.frame(newdat_a_1096[1,],newdat_b_1096[1,])[,-"strata"], #Data frame of covariate values, with one column per covariate, and one row per alternative value.
              t= 90, #itempo máximo al que simular, de manera que el resumen se toman de la muestra de individuos en los datos simulados quienes están en el estado absorvente a ese tiempo.
              probs = c(0.025,0.25, 0.5, 0.75,0.975), 
              #M=100000, #necesita grandes números
              B=n_iter/10, #Pondría 1,000
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_rp_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_rp_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_str2_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_str2_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_rp_str2_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_rp_str2_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_str3_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_str3_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_rp_str3_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_rp_str3_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_str4_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_str4_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_a_rp_str4_1096_",i),.,envir=.GlobalEnv)
}
...
for(i in c(90,183,365,1096,1826)){
  ...
  assign(paste0("tolos_t_b_rp_str4_1096_",i),.,envir=.GlobalEnv)
}
...
if (grepl("CISS Fondecyt",rstudioapi::getSourceEditorContext()$path)==T){
    save.image("C:/Users/CISS Fondecyt/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_3_jun.RData")
  } else if (grepl("andre",rstudioapi::getSourceEditorContext()$path)==T){
    save.image("C:/Users/andre/Desktop/SUD_CL/mult_state_3_jun.RData")
  } else if (grepl("E:",rstudioapi::getSourceEditorContext()$path)==T){
    save.image("E:/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_3_jun.RData")
  } else {
    save.image("G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_3_jun.RData")
  }
