...
We estimated the probability to end in the readmission state, median of stay, percentiles 25, 75 and confidence intervals based on `r n_iter/40` resamples, by simulating a large sample of individuals from the model depending on the type of program they were admitted at baseline.
...
```{r sim_final_plot1, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.cap="Figure 6b. Time to Readmission for Users who Experience it (Mean and 95%CIs), Four-states model", fig.align="center", error=T}
simfinal_fmsm_4_st_df %>%
  dplyr::mutate(year=factor(year,levels=c(.3, 1, 3, 10))) %>% 
  dplyr::group_by(year, tipo_de_programa_2) %>% 
  summarise(mean=sum(val[quantity=="mean"],na.rm=T),
