...
trans_1_3s<-
data.table::data.table(round(exp(fits_c_gomp[[1]]$res),2)[,1:3],keep.rownames = T) %>% 
...
trans_2_3s<-
data.table::data.table(round(exp(fits_c_genf[[2]]$res),2)[,1:3],keep.rownames = T) %>% 
    dplyr::left_join(dt_coefs_simple_surv, by=c("rn"="real_vars")) %>% 
    #dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",as.character(sprintf("%1.3f",p.value)))) %>% 
    dplyr::mutate(formal_vars=ifelse(is.na(formal_vars),rn, formal_vars)) %>% 
    dplyr::mutate(conf.low=sprintf("%1.2f",`L95%`),
...
trans_3_3s<-
data.table::data.table(round(exp(fits_c_ggam[[3]]$res),2)[,1:3],keep.rownames = T) %>% 
    dplyr::left_join(dt_coefs_simple_surv, by=c("rn"="real_vars")) %>% 
    #dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",as.character(sprintf("%1.3f",p.value)))) %>% 
    dplyr::mutate(formal_vars=ifelse(is.na(formal_vars),rn, formal_vars)) %>% 
    dplyr::mutate(conf.low=sprintf("%1.2f",`L95%`),
...
trans_1_4s<-
data.table::data.table(round(exp(fits_c_gomp2[[1]]$res),2)[,1:3],keep.rownames = T) %>% 
    dplyr::left_join(dt_coefs_simple_surv, by=c("rn"="real_vars")) %>% 
    #dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",as.character(sprintf("%1.3f",p.value)))) %>% 
    dplyr::mutate(formal_vars=ifelse(is.na(formal_vars),rn, formal_vars)) %>% 
    dplyr::mutate(conf.low=sprintf("%1.2f",`L95%`),
    ...
    dplyr::select(formal_vars, statistic, `CI 95%`) 
trans_2_4s<-
data.table::data.table(round(exp(fits_c_gomp2[[2]]$res),2)[,1:3],keep.rownames = T) %>% 
    dplyr::left_join(dt_coefs_simple_surv, by=c("rn"="real_vars")) %>% 
    #dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",as.character(sprintf("%1.3f",p.value)))) %>% 
    dplyr::mutate(formal_vars=ifelse(is.na(formal_vars),rn, formal_vars)) %>% 
    dplyr::mutate(conf.low=sprintf("%1.2f",`L95%`),
    ...
    dplyr::select(formal_vars, statistic, `CI 95%`) 
trans_3_4s<-
data.table::data.table(round(exp(fits_c_logn2[[3]]$res),2)[,1:3],keep.rownames = T) %>% 
    dplyr::left_join(dt_coefs_simple_surv, by=c("rn"="real_vars")) %>% 
    #dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",as.character(sprintf("%1.3f",p.value)))) %>% 
    dplyr::mutate(formal_vars=ifelse(is.na(formal_vars),rn, formal_vars)) %>% 
    dplyr::mutate(conf.low=sprintf("%1.2f",`L95%`),
    ...
    dplyr::select(formal_vars, statistic, `CI 95%`) 
trans_4_4s<-
data.table::data.table(round(exp(fits_c_ggam2[[4]]$res),2)[,1:3],keep.rownames = T) %>% 
    dplyr::left_join(dt_coefs_simple_surv, by=c("rn"="real_vars")) %>% 
    #dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",as.character(sprintf("%1.3f",p.value)))) %>% 
    dplyr::mutate(formal_vars=ifelse(is.na(formal_vars),rn, formal_vars)) %>% 
    dplyr::mutate(conf.low=sprintf("%1.2f",`L95%`),
    ...
    dplyr::select(formal_vars, statistic, `CI 95%`) 
trans_5_4s<-
data.table::data.table(round(exp(fits_c_logn2[[5]]$res),2)[,1:3],keep.rownames = T) %>% 
    dplyr::left_join(dt_coefs_simple_surv, by=c("rn"="real_vars")) %>% 
    #dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",as.character(sprintf("%1.3f",p.value)))) %>% 
    dplyr::mutate(formal_vars=ifelse(is.na(formal_vars),rn, formal_vars)) %>% 
    dplyr::mutate(conf.low=sprintf("%1.2f",`L95%`),
...
fig_cox_ab2<-
ggplot()+
  geom_step(data=reshape2::melt(cumhaz_data_a,id.vars=c("time","trans","model")), aes(time, value, color= model,linetype="solid"),size=1, alpha=.65) + 
  geom_step(data=reshape2::melt(cumhaz_data_b,id.vars=c("time","trans","model")), aes(time, value, color= model,linetype="dashed"),size=1, alpha=.65) + 
  facet_wrap(trans~., ncol=1) + 
  scale_color_manual(name ="Model",values=c("#0E53A7","#200772","#BF8830"),labels= c("Cox","NP Cox","Par"))+
  scale_linetype_manual(name= "Type of Plan",values=c("solid","dashed"), labels=c("Women-specific","General Population"))+
...
fig_cox_cd2<-
ggplot()+
  geom_step(data=reshape2::melt(cumhaz_data_c,id.vars=c("time","trans","model")), aes(time, value, color= model,linetype="solid"),size=1, alpha=.65) + 
  geom_step(data=reshape2::melt(cumhaz_data_d,id.vars=c("time","trans","model")), aes(time, value, color= model,linetype="dashed"),size=1, alpha=.65) + 
  facet_wrap(trans~., ncol=1) + 
  scale_color_manual(name ="Model",values=c("#0E53A7","#200772","#BF8830"),labels= c("Cox","NP Cox","Par"))+
  scale_linetype_manual(name= "Type of Plan",values=c("solid","dashed"), labels=c("Women-specific","General Population"))+
  scale_x_continuous(breaks=seq(0, 11, by = 1/2))+
...
cumhaz_plot1<-
ggplot()+
  geom_step(data=data.frame(flexsurv_cumhaz_a$Haz) %>% dplyr::mutate(trans=factor(trans)), aes(time, Haz, color= trans,linetype="solid"),size=1, alpha=.65) + 
  geom_step(data=data.frame(flexsurv_cumhaz_b$Haz)%>% dplyr::mutate(trans=factor(trans)), aes(time, Haz, color= trans,linetype="dashed"),size=1, alpha=.65) +
...
cumhaz_plot2<-
ggplot()+
  geom_step(data=data.frame(flexsurv_cumhaz_c$Haz) %>% dplyr::mutate(trans=factor(trans)), aes(time, Haz, color= trans,linetype="solid"),size=1, alpha=.65) + 
  geom_step(data=data.frame(flexsurv_cumhaz_d$Haz)%>% dplyr::mutate(trans=factor(trans)), aes(time, Haz, color= trans,linetype="dashed"),size=1, alpha=.65) +
  scale_color_manual(name ="Transitions",values=c("gray40","#1B0773","#6E0069","#025167","#A68600"),labels= c("Admission-> Therapeutic Discharge","Admission-> Discharge Without Clinical Advice","Admission->Readmission","Therapeutic Discharge->Readmission","Discharge Without Clinical Advice"))+
  scale_linetype_manual(name= "Type of Plan",values=c("solid","dashed"), labels=c("Women-specific","General Population"))+
...
pmatrix_t_a_df_final %>% 
  ...
  dplyr::select(-t) %>% 
  dplyr::select(`trans`,`Trans_1_cis.y`,`Trans_2_cis.y`,`Trans_3_cis.y`,`Trans_1_cis.x`,`Trans_2_cis.x`,`Trans_3_cis.x`) %>% 
...
pmatrix_4s_t_a_df_final %>% 
  ...
  dplyr::select(-t) %>% 
  dplyr::select(trans,Trans_1_cis.y,Trans_2_cis.y,Trans_3_cis.y,Trans_4_cis.y,Trans_1_cis.x,Trans_2_cis.x,Trans_3_cis.x,Trans_4_cis.x) %>% 
      #2021-04-27: drop from/actual state, readmission
  dplyr::filter(!grepl("Readmission",trans)) %>% 
          knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
...
pmatrix2_t_a<-
do.call('rbind', lapply(c(.25,1,3,5), function(t) {
...
cbind.data.frame(t=rep(c(.25,1,3,5),each=dim(mat_3_states)[2]),
  trans=rep(1:dim(mat_3_states)[2]),
...
cbind.data.frame(t=rep(c(.25,1,3,5),each=dim(mat_4_states)[2]),
  trans=rep(1:dim(mat_4_states)[2]),
...
pmatrix2_t_b_df_final %>% 
    dplyr::mutate(t=round(t,3)) %>% 
    dplyr::select(t, trans, ends_with("cis")) %>% 
  dplyr::filter(t%in% c(.25, 1, 3, 5)) %>%
      dplyr::mutate(t=factor(t, levels=c(.25, 1, 3, 5),
                                labels=c("~91 days/3 months", "~1 year", "~3 years", "~5 years")))
pmatrix2_t_a_df_final %>% 
     dplyr::mutate(t=round(t,3)) %>% 
  dplyr::select(t, trans, ends_with("cis")) %>% 
  dplyr::filter(t%in% c(.25, 1, 3, 5)) %>%
  ...
  kableExtra::add_footnote("Note. We removed Readmission because it was an absorbing state", notation="none") %>% 
  kableExtra::add_header_above(c(" ", "Women-specific: Transition to" = 3,"General Population: Transition to" = 3)) %>% 
  kableExtra::pack_rows("At ~30 days/1 month", 1, 2) %>% 
  kableExtra::pack_rows("At ~1 year", 3, 4) %>%
  kableExtra::pack_rows("At ~3 years", 5, 6) %>%
  kableExtra::pack_rows("At ~5 years", 7, 8) %>%
  kableExtra::scroll_box(width = "100%", height = "375px")
```
...
pmatrix2_4s_t_a_df_final %>% 
  ...
  kableExtra::pack_rows("At ~5 years", 10, 12) %>%
  kableExtra::scroll_box(width = "100%", height = "375px") 
```


```{r transprob_pat2_a, eval=T, echo=T, paged.print=TRUE, fig.height=10,  fig.width = 12, fig.cap="Figure 7a. Transition Probabilities (and 95% confidence intervals), 2nd patient, Three-states model", fig.align="center", error=T}
...
if(no_mostrar==1){
ggsave("eso13b.jpg", transprob_pat2_a,width = 12, height = 10, dpi = 300, units= "in")
dev.off()
}

```

```{r transprob_pat2_b, eval=T, echo=T, paged.print=TRUE,  fig.height=10,  fig.width = 12, fig.cap="Figure 7b. Transition Probabilities (and 95% confidence intervals), 2nd patient, Three-states model", fig.align="center", error=T}
...
```{r transprob_pat_a, eval=T, echo=T, paged.print=TRUE, fig.height=10,  fig.width = 12, fig.cap="Figure 8a. Transition Probabilities (and 95% confidence intervals), 1st patient, Three-states model", fig.align="center", error=T}
...
```{r transprob_pat_b, eval=T, echo=T, paged.print=TRUE, fig.height=10,  fig.width = 12, fig.cap="Figure 8b. Transition Probabilities (and 95% confidence intervals), 1st patient, Three-states model", fig.align="center", error=T}
...
cbind.data.frame(tolos2_t_b_str2_df,program="General Population", start=2))%>% 
  ...
  dplyr::mutate(comb=paste0(start, "_", state)) %>%  #janitor::tabyl(start, state)
  dplyr::mutate(t=sprintf("%1.4f",t)) %>%
  dplyr::filter(t %in% c("0.2500", "1.0000", "3.0000", "5.0000")) %>% 
  dplyr::mutate(t=factor(t,
                levels=c("0.2500", "1.0000", "3.0000", "5.0000"), 
                labels=c("~91 days/3 months", "~1 year", "~3 years", "~5 years"))) %>% 
  dplyr::arrange(start, state, t) %>% 
  dplyr::select(-start,-comb) %>%
  dplyr::select(state, t, `Women-specific`, `General Population`) %>% 
    knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
       caption = paste0("Table 6a. Length of Stay, Three-states model; patient 2"),
       col.names = c("State","Time","Women-specific","General population"),
               align =rep('c', 101)) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
  kableExtra::pack_rows("Starting From Admission", 1, 8) %>% 
  kableExtra::pack_rows("Starting From Therapeutic Discharge", 9, 12) %>% 
  kableExtra::add_footnote("Note. Readmission state will not be considered because is an absorbing state", notation="none") %>% 
  kableExtra::scroll_box(width = "100%", height = "375px")
```
...
cbind.data.frame(tolos2_4s_t_b_str3_df,program="General Population", start=3))%>% 
  ...
  dplyr::select(state, t, `Women-specific`, `General Population`) %>% 
    knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
       caption = paste0("Table 6b. Length of Stay, Four-states model; patient 2"),
       col.names = c("State","Time","Women-specific","General population"),
               align =rep('c', 101)) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
  #kableExtra::add_header_above(c(rep("",1),"Women-specific"=1, "General population"=1)) %>% 
  kableExtra::pack_rows("Starting From Admission", 1, 12) %>%
  kableExtra::pack_rows("Starting From Therapeutic Discharge", 13, 16) %>%
  kableExtra::pack_rows("Starting From Discharge Without Clinical Advice", 17, 20) %>%
  kableExtra::add_footnote("Note. Readmission state will not be considered because is an absorbing state", notation="none") %>% 
  kableExtra::scroll_box(width = "100%", height = "375px")
```


```{r flexsurv_t_arm_3s_plot3_pat2, eval=T, echo=T, paged.print=TRUE, fig.height=10,  fig.width = 12, fig.cap="Figure 9a. Estimated Length of Stay (w/ confidence intervals), 2nd patient. Three-states model", fig.align="center", error=T}
#tolos_t_a_df2  tolos_t_b_df2 tolos_4s_t_a_df2 %>% dplyr::filter(t==10) tolos_4s_t_b_df2 tolos_4s_t_a_10
...
```{r flexsurv_t_arm_4s_plot3_pat2, eval=T, echo=T, paged.print=TRUE, fig.height=10,  fig.width = 12, fig.cap="Figure 9b. Estimated Length of Stay (w/ confidence intervals), 2nd patient. Four-states model", fig.align="center", error=T}
...
```{r flexsurv_t_arm_3s_plot3_pat1, eval=T, echo=T, paged.print=TRUE, fig.height=10,  fig.width = 12, fig.cap="Figure 10a. Estimated Length of Stay (w/ confidence intervals), 1st patient. Three-states model", fig.align="center", error=T}
#tolos_t_a_df2  tolos_t_b_df2 tolos_4s_t_a_df2 %>% dplyr::filter(t==10) tolos_4s_t_b_df2 tolos_4s_t_a_10
...
```{r flexsurv_t_arm_4s_plot3_pat1, eval=T, echo=T, paged.print=TRUE, fig.height=10,  fig.width = 12, fig.cap="Figure 9b. Estimated Length of Stay (w/ confidence intervals), 1st patient. Four-states model", fig.align="center", error=T}
