...
```{r flexsurv_t_arm_3s_plot1, eval=T, echo=T, paged.print=TRUE, fig.height=15, fig.width=10, fig.cap="Figure 3a. Estimate of State Occupancies (w/ confidence intervals), Three-states model", fig.align="center", error=T}
...
cbind.data.frame(pmatrix_t_b_df_final_mod,program="General population")) %>% 
  ...
  xlab("Years")+
  scale_fill_manual(name ="Transition to", values=c("#374140","#D9CB9E","#751c12"))+
  scale_color_manual(name ="Transition to", values=c("#374140","#D9CB9E","#751c12"))+
  scale_y_continuous(labels = scales::percent_format(accuracy = 2), limits=c(0,1))+
...
```{r flexsurv_t_arm_4s_plot2, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.cap="Figure 3d. Estimate of State Occupancies (Area Plot), Four-states model", fig.align="center", error=T}
rbind.data.frame(
cbind.data.frame(pmatrix_4s_t_a_df_final_mod,program="Women-specific"),
cbind.data.frame(pmatrix_4s_t_b_df_final_mod,program="General population")) %>% 
  dplyr::mutate(combi=paste0(trans_from,"-",trans_to)) %>% 
  dplyr::filter(!combi %in% c("2-1","3-1","2-3","3-2","3-1","3-2","4-1","4-2","4-3","4-4")) %>% 
    dplyr::mutate(trans_from=dplyr::case_when(trans_from==1~"1)From Admission",
                                            trans_from==2~"2)From Therapeutic\nDischarge",
                                            trans_from==3~"3)From Discharge Without\nMedical Advice",
                                            trans_from==4~"4)From Readmission")) %>% 
  dplyr::mutate(trans_to=dplyr::case_when(trans_to==1~"1)Admission",
                                            trans_to==2~"2)Therapeutic\nDischarge",
                                            trans_to==3~"3)Discharge Without\nMedical Advice",
  ...
  theme(legend.position="bottom")+
  xlab("Years")+
  scale_fill_manual(name ="Transition to", values=c("#374140","#D9CB9E","#751c12","#939da3"))+
  scale_color_manual(name ="Transition to", values=c("#374140","#D9CB9E","#751c12","#939da3"))+
  scale_y_continuous(labels = scales::percent_format(accuracy = 2), limits=c(0,1))+
  scale_x_continuous(breaks=seq(0, 11,2))
```
...
pmatrix_t_b_df_final %>% 
  ...
  dplyr::filter(t%in% c(0.000, 0.083, 0.333, 0.50, .75, 1, 3, 5)) %>%
      dplyr::mutate(t=factor(t, levels=c(0.000, 0.083, 0.333, 0.50, .75, 1, 3, 5),
                                labels=c("0 days", "~30days/1 month", "~91 days/3 months", "~183 days/6 months", "~274days/9 months", "~1 year", "~3 years", "~5 years")))
...
pmatrix_t_a_df_final %>% 
 ...
 # kableExtra::add_footnote("Note. NA= Null values", notation="none") %>% 
  kableExtra::add_header_above(c(" ", "General Population: Transition to" = 3, "Women-specific: Transition to" = 3)) %>% 
  ...
  kableExtra::pack_rows("At ~183 days/6 months", 10, 12) %>%
  kableExtra::pack_rows("At ~274days/9 months", 13, 15) %>%
  kableExtra::pack_rows("At ~1 year", 16, 18) %>%
  kableExtra::pack_rows("At ~3 years", 19, 21) %>%
  kableExtra::pack_rows("At ~5 years", 22, 24) %>%
...
pmatrix_4s_t_a_df_final %>% 
  ...
  dplyr::select(-t) %>% 
          knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               ...
               col.names = c("Actual state", rep(c("Admission","Therapeutic Discharge","Discharge Without\nMedical Advice","Readmission"),2)),
               align =rep('c', 101)) %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
 # kableExtra::add_footnote("Note. NA= Null values", notation="none") %>% 
  kableExtra::add_header_above(c(" ", "General Population: Transition to" = 4, "Women-specific: Transition to" = 4)) %>% 
  kableExtra::pack_rows("At 0 days", 1, 4) %>% 
  kableExtra::pack_rows("At ~30 days/1 month", 5, 8) %>% 
  kableExtra::pack_rows("At ~91 days/3 months", 9, 12) %>% 
  kableExtra::pack_rows("At ~183 days/6 months", 13, 16) %>%
  kableExtra::pack_rows("At ~274days/9 months", 17, 20) %>%
  kableExtra::pack_rows("At ~1 year", 21, 24) %>%
  kableExtra::pack_rows("At ~3 years", 25, 28) %>%
  kableExtra::pack_rows("At ~5 years", 29, 32) %>%
  kableExtra::scroll_box(width = "100%", height = "375px") 
#
```
...
```{r flexsurv_t_arm_3s_plot3, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.cap="Figure 4a. Estimate of Length of Stay (w/ confidence intervals), Three-states model", fig.align="center", error=T}
rbind.data.frame(
...
```{r flexsurv_t_arm_3s_plot4, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.cap="Figure 4b. Estimate of Length of Stay (w/ confidence intervals), Three-states model (start from Therapeutic\nDischarge)", fig.align="center", error=T}
rbind.data.frame(
...
```{r flexsurv_t_arm_4s_plot3, eval=T, echo=T, paged.print=TRUE, fig.height=15, fig.width=10, fig.cap="Figure 5a. Estimate of Length of Stay (w/ confidence intervals), Four-states model", fig.align="center", error=T}
rbind.data.frame(
...
```{r flexsurv_t_arm_4s_plot4, eval=T, echo=T, paged.print=TRUE, fig.height=15, fig.width=10, fig.cap="Figure 5b. Estimate of Length of Stay (w/ confidence intervals), Four-states model", fig.align="center", error=T}
rbind.data.frame(
...
simfinal_fmsm_3_st_df %>%
  dplyr::mutate(year=factor(year,levels=c(.08,.25, .333, .5, 1, 3, 10), labels=c("30 days", "90 days", "4 months", "6 months", "1 year", "3 years", "10 years"))) %>% 
...
```{r LMAJ, eval=F, echo=T, paged.print=TRUE, fig.height=15, fig.width=10, fig.cap="Figure 1b. Estimate of Cumulative Hazards, Four States Model", fig.align="center", error=T}
