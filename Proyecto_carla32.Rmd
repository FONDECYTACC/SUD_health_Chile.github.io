---
title: "Readmission risk among women in women-only versus mixed gender treatment programs for substance use disorder in Chile (2nd step)"
...
msf0 <- mstate::msfit(object = c0, trans = mat_3_states)
msf1 <- mstate::msfit(object = c1,  trans = mat_3_states)

msf0_4s <- mstate::msfit(object = c0_4s, trans = mat_4_states)
msf1_4s <- mstate::msfit(object = c1_4s,  trans = mat_4_states)
...
invisible(c("Dont know where to put in predt"))

pt0 <- mstate::probtrans(msf0, predt=0)[[2]]
pt1 <- mstate::probtrans(msf1, predt=0)[[2]]

pt0_ci95 <- mstate::probtrans(msf0, predt=0)
pt1_ci95 <- mstate::probtrans(msf1, predt=0)

pt0_4s_ci95 <- mstate::probtrans(msf0_4s, predt=0)
pt1_4s_ci95 <- mstate::probtrans(msf1_4s, predt=0)


invisible(c("probtrans is for markov models, not semi"))
...
#cumulative hazards. Their increments will be used in the Aalen-Johansen estimator
par(mfrow=c(1,2))
...
sub_ms_0<-subset(ms_CONS_C1_SEP_2020_women_imputed, tipo_de_programa_2==0)
sub_ms_1<-subset(ms_CONS_C1_SEP_2020_women_imputed, tipo_de_programa_2==1)

attr(sub_ms_0, "trans") <- mat_3_states
attr(sub_ms_1, "trans") <- mat_3_states
...
#Error in order(msd$id, msd$trans, msd$Tstart) : 
#  argumento 1 no es un vector
```
...
```{r probtrans_semimark_ci951,eval=T, echo=T, paged.print=TRUE}
...
mstate::mssample(mrcox_a$Haz, trans = mat_3_states, clock = "reset", M = n_iter, tvec = seq(0, 12, by = 0.1))%>% 
  assign(paste0("pmatrix_cox_a"),.,envir=.GlobalEnv)
...
mstate::mssample(mrcox_b$Haz, trans = mat_3_states, clock = "reset", M = n_iter, tvec = seq(0, 12, by = 0.1))%>% 
  assign(paste0("pmatrix_cox_b"),.,envir=.GlobalEnv)
#}
...
pmatrix_t_a<-
do.call('rbind', lapply(seq(0, 12, by = 0.1), function(t) {
...
```{r probtrans_semimark_ci95_2, eval=F, echo=T, paged.print=TRUE, error=T}
library(multistateutils)
time_before_probtrans_ci952<-Sys.time()

predict_transitions(sel_param_fits_b, newdat3b, mat_3_states, times=12, tcovs='arrival')
#Error in `contrasts<-`(`*tmp*`, value = contr.funs[1 + isOF[nn]]) : 
  #contrasts can be applied only to factors with 2 or more levels
predict_transitions(sel_param_fits_b , newdat5b, mat_3_states, times=2, ci=TRUE, M=10)
predict_transitions(sel_param_fits_b , newdat3b, mat_3_states, times=2, ci=TRUE, M=10, tcovs='arrival')
#Error in `contrasts<-`(`*tmp*`, value = contr.funs[1 + isOF[nn]]) : 
#contrasts can be applied only to factors with 2 or more levels
...
#Log-logistic
#Gompertz
#Aún asi no fnunciona
plot_predicted_pathway(sel_param_fits_b , mat_3_states, newdat3b, seq(0, 10, by=2))

time_after_probtrans_ci952<-Sys.time()

paste0("Time in process: ");time_after_probtrans_ci952-time_before_probtrans_ci952

#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_#_
#https://www.uniklinik-freiburg.de/imbi/stud-le/competing-risks-and-multistate-models-with-r/r-code-for-chapter-10.html
#Computation of the confidence intervals of the predicted transition probabilities
# Después de probtrans
...
flexsurv::totlos.simfs(sel_param_fits_b, 
            ...
            # start = 1, #Starting state.
             ...
             M = 1e+05, #Number of individuals to simulate in order to approximate the transition probabilities. Users should adjust this to obtain the required precision.
             B = 1000, #Number of simulations from the normal asymptotic distribution used to calculate variances. Decrease for greater speed at the expense of accuracy.
  cl = 0.95)
}
...
flexsurv::sim.fmsm(
  x,
  trans = NULL, #Matrix indicating allowed transitions. See msfit.flexsurvreg.
  t, #Time, or vector of times for each of the M individuals, to simulate trajectories until.
  newdata = NULL, #A data frame specifying the values of covariates in the fitted model, other than the transition number. See msfit.flexsurvreg.
  start = 1, #Starting state, or vector of starting states for each of the M individuals
  M = 10, #Number of individual trajectories to simulate.
  tvar = "trans", #Number of individual trajectories to simulate.
  tcovs = NULL, #Names of "predictable" time-dependent covariates in newdata, i.e. those whose values change at the same rate as time. Age is a typical example. During simulation, their values will be updated after each transition time, by adding the current time to the value supplied in newdata. This assumes the covariate is measured in the same unit as time. tcovs is supplied as a character vector.
  tidy = T, #If TRUE then the simulated data are returned as a tidy data frame with one row per simulated transition. See simfs_bytrans for a function to rearrange the data into this format if it was simulated in non-tidy format.
  debug = FALSE
)
