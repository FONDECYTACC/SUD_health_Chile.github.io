...
CONS_C1_df_dup_SEP_2020 %>% dplyr::filter(as.character(motivodeegreso_mod_imp)!="Ongoing treatment"|is.na(motivodeegreso_mod_imp)) %>%
    dplyr::filter(as.character(motivodeegreso_mod_imp)=="Early Drop-out"|
                  as.character(motivodeegreso_mod_imp)=="Late Drop-out") %>% 
  tabyl(dup) %>% 
  summarise(more_one_treat=sum(n[dup>1]))
#al excluir me quedo desde 22,383 a 11834
...
We selected the cases that had completed treatments, leaving `r as.numeric(sum(sel_treat_tabyl$n))%>%  format(big.mark=",")` observations. Then, we selected those cases that had either early drop-outs and late drop-outs (n=`r as.numeric(sum(sel_treat2_mot_egreso_tabyl)) %>% format(big.mark=",")`). Must take note that we did not include those that did not finished their treatment (n= `r tot_sin_motivo_egreso %>% nrow() %>% format(big.mark=",")`), but we included those that had more than one finished treatment previous to the selected (n= `r sel_treat_more_one_treat %>%  as.numeric() %>% format(big.mark=",",format="g")`).
...
match.on_tot <- c("row", "hash_key","sus_ini_mod_mvv","estado_conyugal_2","escolaridad_rec","condicion_ocupacional_corr","edad_ini_cons","freq_cons_sus_prin","origen_ingreso_mod","nombre_region","tipo_centro_pub","abandono_temprano_rec","tipo_de_plan_res","evaluacindelprocesoteraputico_min","prev_treat","sexo_2","edad_al_ing","fech_ing_num")
...
```{r miss_plot_perc_var, warning=F,message=F,fig.align='center', fig.cap="Figure 3. Bar plot of Porcentaje of Missing Values per Variables", error=T}
...
```{r mice2_age_onset_drug_use, warning=F,message=F,fig.align='center', fig.cap="Figure 5. Bar plot of Percentage of Incorrect Imputed Values per Imputation Sample", error=F}
...
Considering that some missing values were not possible to impute, we started having `r nrow(CONS_C1_df_dup_SEP_2020_match_not_miss) %>%  format(big.mark=",")` cases (users=`r length(unique(CONS_C1_df_dup_SEP_2020_match_not_miss$hash_key))%>%  format(big.mark=",")`), and ended having `r nrow(CONS_C1_df_dup_SEP_2020_match_not_miss2) %>%  format(big.mark=",")` complete cases (users=`r length(unique(CONS_C1_df_dup_SEP_2020_match_not_miss2$hash_key))%>%  format(big.mark=",")`).  
...
```{r  Fig 6_dist_no_treat_treat, warning=FALSE, fig.align = "center", message=F, fig.height=10, cache=T, eval=T, fig.cap="Figure 6. Covariates Balance on Different Values"}
...
## Explore Results of the Matching(1){.tabset .tabset-fade}

<br>

```{r  Fig 7_dist_no_treat_treat, warning=FALSE, fig.align = "center", message=F, fig.height=7, cache=T, eval=T, fig.cap="Figure 7. Empirical Cumulative Distribution Functions on the Matched Sample", results='asis'}
...
```{r  Fig 8_dist_no_treat_treat, warning=FALSE, fig.align = "center", message=F, fig.height=7, cache=T, eval=T, fig.cap="Figure 8. Love plot of the Matched Sample in Covariates v/s Unmatched Sample", results='asis'}
...
## Explore Results of the Matching(2){.tabset .tabset-fade}
...
```{r  tab3_match2, warning=FALSE, fig.align = "center", message=F, fig.height=10, cache=T, eval=T}
options(knitr.kable.NA = '')
...
mom_covs3 = cbind(CONS_SEP_match2$edad_ini_cons, 
                   ...
                   CONS_SEP_match2$`nombre_region_Araucanía (09)`,
                   CONS_SEP_match2$`nombre_region_Arica (15)`,
                   CONS_SEP_match2$`nombre_region_Atacama (03)`,
                   CONS_SEP_match2$`nombre_region_Aysén (11)`,
                   CONS_SEP_match2$`nombre_region_Biobío (08)`,
                   ...
                   CONS_SEP_match2$`nombre_region_Ñuble (16)`,
                   CONS_SEP_match2$`nombre_region_O'Higgins (06)`,
                   CONS_SEP_match2$`nombre_region_Tarapacá (01)`,
                   CONS_SEP_match2$`nombre_region_Valparaíso (05)`,
...
out3= cardmatch(t_ind, #ES NECESARIO QUE LOS TRATAMIENTOS ESTEN ORDENADOS Y LOS OTROS VECTORES TAMBIËN 
                #mom= mom3,
                fine = fine3,# ya los definí list(covs = mom_covs, tols = mom_tols, targets = mom_targets), 
          solver = solver3)
end.time3 <- Sys.time()
...
## Explore Results of the Matching (3){.tabset .tabset-fade}

<br>

```{r  Fig 11_plot, warning=FALSE, fig.align = "center", message=F, fig.height=10, cache=T, eval=T, fig.cap="Figure 11. Empirical Cumulativ e Distribution Functions on the Matched Sample", results='asis'}
...
```{r  Fig 12_plot, warning=FALSE, fig.align = "center", message=F, fig.height=10, cache=T, fig.cap="Figure 12. Love plot of the Matched Sample in Covariates v/s Unmatched Sample", results='asis'}
...
```{r  tab3, warning=FALSE, fig.align = "center", message=F, fig.height=10, cache=T, eval=T}
options(knitr.kable.NA = '')
...
```
```{r  Fig 13_plot, warning=FALSE, fig.align = "center", message=F, fig.height=13, cache=T, fig.cap="Figure 13. Love plot of the Matched Sample in Covariates v/s Unmatched Sample"}
...
```{r inf_2by2, warning=FALSE, message=F, cache=T, eval=T, paged.print=TRUE}
...
```{r ors, warning=FALSE, message=F, cache=T, eval=T, paged.print=TRUE}
...
```{r  Fig 14_plot_sens, warning=FALSE, fig.align = "center", message=F, fig.height=7, cache=T, fig.cap="Figure 14. Sensibility Parameters for the Models Specified"}
...
Sensitivity analysis revealed that these effects were robust, as an unobserved variable would need to increase the odds of treatment by at least `r scales::percent(as.numeric(max_gamma)-1)` ($\Gamma$ = `r as.numeric(max_gamma)`) in order to confound the estimated treatment effects.

<br>


```{r sens2, warning=FALSE, message=F, cache=T, eval=F, paged.print=TRUE, error=F}
