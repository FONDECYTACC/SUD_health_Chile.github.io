...
#_time tipo_de_plan_res_1 TD_1 TD_2 TD_3 _start#

formula3_mstate<-
as.formula(paste0("Surv(time, status) ~", paste0(fitform2," + factor(TD_1)+ factor(TD_2)+ factor(TD_3)+ arrival + strata(trans)")[[3]]))


formula3_mstate_stratified<-
as.formula(paste0("Surv(time, status) ~", paste0(fitform2," + factor(TD_1)+ factor(TD_2)+ factor(TD_3)+ arrival")[[3]]))
...
form<-
  function(i=1){
  f<-dplyr::case_when(i==4~paste0("Surv(time, status) ~ ",fitform2," + factor(TD_1)+ factor(TD_2)+ factor(TD_3)+ arrival")[[3]],i==3~paste0("Surv(time, status) ~ ",fitform2," + factor(TD_1)+ factor(TD_2)+ arrival")[[3]],i==2~paste0("Surv(time, status) ~ ",fitform2," + factor(TD_1)+ arrival")[[3]],T~paste0("Surv(time, status) ~ ",fitform2,"")[[3]])
  return(f)
}
...
# Probability Density Functions
f0_tr3 <- function(x,
               mu = coef(fits_c_ggam2[[3]])["mu"],
               sigma =exp(coef(fits_c_ggam2[[3]])["sigma"]),
               Q = coef(fits_c_ggam2[[3]])["Q"]){
  dgengamma(x=x, mu=mu, sigma=sigma, Q=Q)
}
f1_tr3 <- function(x,
               mu = coef(fits_c_ggam2[[3]])["mu"]+coef(fits_c_ggam2[[3]])["factor(tipo_de_plan_res_1)1"],
               sigma =exp(coef(fits_c_ggam2[[3]])["sigma"]),
               Q = coef(fits_c_ggam2[[3]])["Q"]){
  dgengamma(x=x, mu=mu, sigma=sigma, Q=Q)
}
ft_tr3 <- function(x){ (f0(x) + f1(x))}
# Cumulative Density Functions
S0_tr3 <- function(x,
               mu = coef(fits_c_ggam2[[3]])["mu"],
               sigma =exp(coef(fits_c_ggam2[[3]])["sigma"]),
...
St_tr3 <- function(x){ ( S0(x) * f0(x) + S1(x) * f1(x) ) / ( f0(x) + f1(x) ) }
## St <- function(x){ (S0(x) + S1(x))/2} ## assuming same number of events
## Hazard functions
h0_tr3 <- function(x,
               mu = coef(fits_c_ggam2[[3]])["mu"],
...
sahr.maxt25p_trans3 <- calcAHRsmp(x=max(subset(ms_d_match_surv_exp, trans==3)$time)/4, h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)
print(sahr.maxt25p)
## sAHR for max/2 FU period (50%)
sahr.maxt50p_trans3 <- calcAHRsmp(x=max(subset(ms_d_match_surv_exp, trans==3)$time)/2, h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)
print(sahr.maxt50p)
## [1] 0.1131397
## gAHR for max FU period (100%)
sahr.maxt100p_trans3 <- calcAHRsmp(x=max(subset(ms_d_match_surv_exp, trans==3)$time), h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)
print(sahr.maxt100p)
## [1] 0.2209214

ff2 <- function(x){calcAHRgeo(x=x, h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)}
tt <- seq(0.2*365.25,10*365.25,0.1*365.25)
yy2 <- sapply(tt, FUN=ff2)
plot(tt, yy2, ylim=c(0,2), xlab="TIME", ylab="P(Survival)")


ocahr.maxt3m_trans3 <- calcAHRoc(x=90, f0=f0_tr3, f1=f1_tr3, S0=S0_tr3, S1=S1_tr3)

ocahr.maxt1yr_trans3 <- calcAHRoc(x=365.25, f0=f0_tr3, f1=f1_tr3, S0=S0_tr3, S1=S1_tr3)

ocahr.maxt3yr_trans3 <- calcAHRoc(x=365.25*3, f0=f0_tr3, f1=f1_tr3, S0=S0_tr3, S1=S1_tr3)

ocahr.maxt5yr_trans3 <- calcAHRoc(x=365.25*5, f0=f0_tr3, f1=f1_tr3, S0=S0_tr3, S1=S1_tr3)

ahr.maxt3m_trans3 <- calcAHRgeo(x=90, h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)

ahr.maxt1yr_trans3 <- calcAHRgeo(x=365.25, h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)

ahr.maxt3yr_trans3 <- calcAHRgeo(x=365.25*3, h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)

ahr.maxt5yr_trans3 <- calcAHRgeo(x=365.25*5, h0=h0_tr3, h1=h1_tr3, ft=ft_tr3, wt=St_tr3)
...
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
    "3 months",
    "\nthe average hazard ratio is ",
    round(ahr.maxt3m_trans3, 2), ".")
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
     "one year",
    "\nthe average hazard ratio is ",
    round(ahr.maxt1yr_trans3, 2), ".")
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
    "three years",
    "\nthe average hazard ratio is ",
    round(ahr.maxt3yr_trans3, 2), ".")
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
    "five years",
    "\nthe average hazard ratio is ",
    round(ahr.maxt5yr_trans3, 2), ".")
...
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
    "3 months",
    "\nthe average hazard ratio (odds of concordance) is ",
    round(ocahr.maxt3m_trans3, 4), ".")
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
     "one year",
    "\nthe average hazard ratio (odds of concordance) is ",
    round(ocahr.maxt1yr_trans3, 4), ".")
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
    "three years",
    "\nthe average hazard ratio (odds of concordance) is ",
    round(ocahr.maxt3yr_trans3, 4), ".")
cat("The average HR (residential:outpatient) for patients with a follow-up period of",
    "five years",
    "\nthe average hazard ratio (odds of concordance) is ",
    round(ocahr.maxt5yr_trans3, 4), ".")
```
