...
CONS_C1_df %>%
    ...
    knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                 ...
                 align =rep('c', 5))  %>%
    kable_styling(bootstrap_options = c("striped", "hover"),font_size = 10) %>%
    add_footnote( c("Si= Pregnant", "No= Not Pregnant"), notation = "symbol")
...
```{r echo=F, paged.print=TRUE}
CONS_C1_df %>%
    count(identidad.de.genero, Sexo) %>% 
    group_by(Sexo) %>% 
    mutate(prop = paste0(round(100 * prop.table(n)),"%")) %>%
    data.table::data.table() %>% 
    reshape::melt(id.vars = c(1:2)) %>% 
    ...
    knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                 ...
                 align =rep('c', 5))  %>%
    kable_styling(bootstrap_options = c("striped", "hover"),font_size = 10) %>%
  add_footnote( c("Femenino= Feminine" ,"Masculino= Masculine"), notation = "symbol")
...
```{r inicio consumo, echo=T, paged.print=TRUE}
