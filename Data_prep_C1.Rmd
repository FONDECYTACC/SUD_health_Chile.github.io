...
```{r echo=F, paged.print=TRUE}
    ...
    knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                   caption="Table 7. Duplicated cases", col.names= c(" ", " Frequencies", "Percentage"), 
                 align =rep('c', 3))  %>%
    kable_styling(bootstrap_options = c("striped", "hover"),font_size = 10)
```
...
```{r bunch_o_figs2, fig.height=4, fig.width=8}
    par(mfrow = c(1,3)) 
...
```{r bunch_o_figs3, fig.height=4, fig.width=8}
    par(mfrow = c(1,2)) 
...
```{r echo=F, paged.print=TRUE}
summarytools::ctable(CONS_C1_df$embarazo, CONS_C1_df$Sexo, prop = "c",totals = FALSE)
```
...
```{r echo=F, paged.print=TRUE}
 print(summarytools::ctable(CONS_C1_df$identidad.de.genero, CONS_C1_df$Sexo, prop = "c",totals = FALSE))
```
<br>
&nbsp;
...
```{r echo=F, paged.print=TRUE, message=FALSE}
    CONS_C1_df %>%
...
There are `r CONS_C1_df %>% dplyr::filter(dias_trat<1) %>% dplyr::summarise(n())` values 0 or negative.
