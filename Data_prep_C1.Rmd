...
**UPDATE: 2020-01-17**. Thanks to SENDA's experts, they provided us with information about a case with missing values in dates of treatment. This date had to be formatted because it was bad labeled.  `CONS_C1_df[CONS_C1_df$row=="88379","fech_ing"] <- parse_date_time(str_trim("09/07/2015"), orders = c("%d/%m/%Y"),exact=T)`.
**UPDATE: 2020-01-17**, this date had to be formatted because it was bad labeled.  `CONS_C1_df[CONS_C1_df$row=="79428","fech_ing"] <- parse_date_time(str_trim("09/07/2015"), orders = c("%d/%m/%Y"),exact=T)`.
**UPDATE: 2020-01-17**, this date had to be formatted because it was bad labeled.  `CONS_C1_df[CONS_C1_df$row=="88379","fech_egres"] <- parse_date_time(str_trim("11/04/2016"), orders = c("%d/%m/%Y"),exact=T)`.
**UPDATE: 2020-01-17**, this date had to be formatted because it was bad labeled.  `CONS_C1_df[CONS_C1_df$row=="79428","fech_egres"] <- parse_date_time(str_trim("11/04/2016"), orders = c("%d/%m/%Y"),exact=T)`. Still, there are `r table(is.na(CONS_C1_df$fech_egres))[2]` cases with NULL dates of
discharge.
 <br>
***
***
...
Every of the `r CONS_C1_df %>% dplyr::filter(tipo_de_programa=="Programa Alcohol") %>% dplyr::group_by(tipo_de_plan) %>% dplyr::summarise(n())` cases in the alcohol program should be in the general population plan.
...
- **Why were there men and women with female gender identity that are in the general population?, (Basic outpatient , intensive outpatient , Residential and Outpatient 2)**

- **Should the 2 men and 12 women in M-PAI but with masculine gender identity be in the general population (PG PAI)?**

- **If there was a woman with masculine gender identity in M-PR, should not be in the general population (PG_PR)?**
