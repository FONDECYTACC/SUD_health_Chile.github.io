...
We used a neural network model to obtain a definitive value of the `r CONS_C1_df_dup_FEB_2020_prev25 %>% dplyr::filter(hash_key %in% as.character(unlist(sexo_casos_pendientes_3["hash_key"])))%>% nrow()/2` users that had two different sexes entered into the system.
...
CONS_C1_df_dup_FEB_2020_prev24_test_nn<- CONS_C1_df_dup_FEB_2020_prev24 %>% 
        ...
        result_sex_all_cv_pred<- predict(neuralnet_sex, pred_data = CONS_C1_df_dup_FEB_2020_prev25_test_nn)
```

<br>

The model was trained using the rest of the dataset with valid attributes on the variable sex (n= `r formatC(nrow(CONS_C1_df_dup_FEB_2020_prev25_train_nn),big.mark = ",")`). We generated a total of 10 cross-validations, with alternatives of 1 to 5 layers, and a threshold of 0, .5 and 1 in every model (10x5x3). The model that obtained the highest area under the curve  (AUC (mean)= `r round(nn_sex_par[1,"auc (mean)"],2)`) had a size of `r as.numeric(nn_sex_par[1,"size"])` `r ifelse(as.numeric(nn_sex_par[1,"size"])<2,"layer","layers")`, and a threshold for weights of `r as.numeric(nn_sex_par[1,"decay"])` (L2 penalty). THe following figure shows the neural network structure Due to the great amount of categories, we generated a garson plot of importance of variables into the weight.

<br>

```{r fig7a_image-decision_tree_distinct_values_user_invariant_nat,fig.height=4, fig.width=8, echo=T, fig.align='center', fig.pos='H', fig.cap="Figure 7a. Neural Network of Sex", message=FALSE}
...
```{r fig7a_image-decision_tree_distinct_values_user_invariant_nat,fig.height=4, fig.width=8, echo=T, fig.align='center', fig.pos='H', fig.cap="Figure 7b. Garson Plot of Variable Importance", message=FALSE}
#sexo_casos_pendientes_3
...
#sexo_casos_pendientes_3
  ...
  fech_nac_edad_al_ing_replace <-
                  result_sex_all_cv_pred%>%
                     cbind(CONS_C1_df_dup_FEB_2020_prev25_test_nn[c("row","hash_key","sexo_2")])%>%
                    ...
                    dplyr::mutate(sexo_2_corr_final=ifelse(sexo_2_corr==sexo_2_corr2,sexo_2_corr,NA_character_))
...
We replaced  cases that in both 
...
CONS_C1_df_dup_FEB_2020_prev25_nacionalidad <- CONS_C1_df_dup_FEB_2020_prev25 %>% group_by(hash_key) %>% dplyr::mutate(nacionalidad_por_hash=n_distinct(nacionalidad)) %>% ungroup() %>% dplyr::filter(nacionalidad_por_hash>1) %>% dplyr::distinct(hash_key)
CONS_C1_df_dup_FEB_2020_prev25 %>% 
