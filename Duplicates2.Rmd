...
# 117,387 de MAY 2020, v/s 118,385 de MAR 2020
```
...
```{r comprobacion de overlap final, include=T, echo=T, eval=F, cache=T}
...
There may be differences between the primary substance and the route of administration answered by users. For example, `r CONS_C1_df_dup_FEB_2020_prev21 %>% dplyr::filter(sus_principal=="Cocaína", via_adm_sus_prin!="Intranasal  ( aspiración de polvo por la nariz)") %>% nrow() %>% formatC(, format="f", big.mark=",", digits=0)` cases did not snort Cocaine, ¿so what is the correct route of administration of each primary substance?. A variable called `via_adm_sus_prin_act`.

<br>

```{r  tab7_ruta_adm_sus_prin, include=T, echo=T, eval=F, cache=T}
CONS_C1_df_dup_FEB_2020_prev21 %>%
...
Some cases did not have a primary diagnostic in DSM-IV notation, but have a secondary or tertiary `r CONS_C1_df_dup_FEB_2020_prev22 %>% dplyr::filter(is.na(dg_trs_psiq_dsm_iv_or) & !is.na(x2_dg_trs_psiq_dsm_iv_or)) %>% dplyr::select(dg_trs_psiq_dsm_iv_or,x2_dg_trs_psiq_dsm_iv_or,x3_dg_trs_psiq_dsm_iv_or) %>% nrow() %>%  formatC(, format="f", big.mark=",", digits=0)`).

<br>

```{r  dg_replace_dsm, include=T, echo=T, eval=T, cache=T}
dg_replace_dsm<- CONS_C1_df_dup_FEB_2020_prev22 %>% dplyr::filter(is.na(dg_trs_psiq_dsm_iv_or) & !is.na(x2_dg_trs_psiq_dsm_iv_or)) %>% dplyr::select(row) %>%unlist()
dg_replace_dsm<- as.character(dg_replace_dsm)

CONS_C1_df_dup_FEB_2020_prev22 %>% 
  dplyr::mutate(obs=case_when(row %in% as.character(as.vector(unlist(as.data.table(unlist(dg_replace_dsm)))))~glue::glue("{obs};2.5.01.Replaced empty DSM-IV with second diagnostic"),TRUE~obs))%>%
...
`r CONS_C1_df_dup_FEB_2020_prev23 %>% dplyr::filter(is.na(dg_trs_psiq_cie_10_or) & !is.na(x2_dg_trs_psiq_cie_10_or)) %>% dplyr::select(dg_trs_psiq_cie_10_or,x2_dg_trs_psiq_cie_10_or,x3_dg_trs_psiq_cie_10_or) %>% nrow() %>%  formatC(, format="f", big.mark=",", digits=0)` cases did not have a primary diagnostic for CIE-10 but had a secondary.

<br>

Also, some cases did not have another substances (1), but had another substances (2) (n= `r CONS_C1_df_dup_FEB_2020_prev23 %>% dplyr::filter(is.na(otras_sus1) & !is.na(otras_sus2)) %>% dplyr::select(otras_sus1,otras_sus2,otras_sus3) %>% nrow() %>%  formatC(, format="f", big.mark=",", digits=0)`).

<br>

```{r  dg_replace_substances, include=T, echo=T, eval=T, cache=T}
replace_subs<- CONS_C1_df_dup_FEB_2020_prev23 %>% dplyr::filter(is.na(otras_sus1) & !is.na(otras_sus2)) %>% dplyr::select(row) %>% unlist()
...
CONS_C1_df_dup_FEB_2020_prev23 %>%
  dplyr::mutate(obs=case_when(hash_key %in% as.character(as.vector(unlist(as.data.table(unlist(CONS_C1_df_dup_FEB_2020_prev23_sex)))))~glue::glue("{obs};2.6.01.HASH w/ more than one distinct Sex"),TRUE~obs))%>%
...
#View(CONS_C1_df_dup_FEB_2020_prev24 %>% dplyr::group_by(obs) %>% summarise(n=n()) %>% dplyr::filter(n,grepl('2.6.0', obs)))
```
...
- Date of Birth (fech_nac) (`r CONS_C1_df_dup_FEB_2020_prev24 %>% group_by(hash_key) %>% dplyr::mutate(fech_nac_por_hash=n_distinct(fech_nac)) %>% ungroup() %>% dplyr::filter(fech_nac_por_hash>1) %>% nrow() %>%  formatC(, format="f", big.mark=",", digits=0)`) (must note that these different dates of birth did not impact on age). In cases with nothing than 2 distinct dates of birth with datasets from the same year, we replace the date of birth with the mean between both dates.

```{r tab9_pers_info_fech_nac,eval=T, echo=T, paged.print=TRUE}
...
dplyr::select(row, hash_key, ano_bd, id_mod, sexo, fech_nac,ano_nac,edad,edad_al_ing, fech_ing, fech_egres,fech_egres_imp, tipo_de_plan, senda)%>%
      head(40)%>%
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 9. HASHs with more than one Date of Birth (First 40 Cases)",
                 align =rep('c', 101))  %>%
   kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
  kableExtra::scroll_box(width = "100%", height = "375px")
#_______________________
...
CONS_C1_df_dup_FEB_2020_prev24 %>%
  dplyr::mutate(obs=case_when(hash_key %in% as.character(as.vector(unlist(as.data.table(unlist(CONS_C1_df_dup_FEB_2020_prev24_fech_nac)))))~glue::glue("{obs};2.6.02.HASH w/ more than one distinct Date of Birth"),TRUE~obs))%>%  
...
#View(CONS_C1_df_dup_FEB_2020_prev25 %>% dplyr::group_by(obs) %>% summarise(n=n()) %>% dplyr::filter(n,grepl('2.6.02.HASH w/ more than one distinct Date of Birth', obs)))
...
```{r tab10_pers_info_nationality, eval=T,echo=T, paged.print=TRUE}
...
dplyr::select(row, hash_key, ano_bd, nacionalidad, etnia, sexo, fech_nac,edad_al_ing, fech_ing, fech_egres, tipo_de_plan, senda)%>%
    head(40)%>%
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 10. HASHs with more than one Nationality (First 40 Cases)",
                 align =rep('c', 101))  %>%
   kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
  kableExtra::scroll_box(width = "100%", height = "375px")
#
...
CONS_C1_df_dup_FEB_2020_prev25 %>%
  dplyr::mutate(obs=case_when(hash_key %in% as.character(as.vector(unlist(as.data.table(unlist(CONS_C1_df_dup_FEB_2020_prev25_nat)))))~glue::glue("{obs};2.6.03.HASH w/ more than one distinct Nationality"),TRUE~obs))%>%  
...
dplyr::select(row, hash_key, ano_bd, nacionalidad, etnia)%>%
    head(40)%>%
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 11. HASHs with more than one Ethnicity (First 40 Cases)",
                 align =rep('c', 101))  %>%
   kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
  kableExtra::scroll_box(width = "100%", height = "375px")
#_______________________
...
CONS_C1_df_dup_FEB_2020_prev26 %>%
  dplyr::mutate(obs=case_when(hash_key %in% as.character(as.vector(unlist(as.data.table(unlist(CONS_C1_df_dup_FEB_2020_prev26_etnia)))))~glue::glue("{obs};2.6.04.HASH w/ more than one distinct Ethnic Group"),TRUE~obs))%>%  
...
```{r tab12X_pers_info_edad_ini_cons,eval=F, echo=T, paged.print=TRUE}
...
dplyr::select(row, hash_key, ano_bd, edad_ini_cons, edad, fech_nac,edad_al_ing, fech_ing, fech_egres_imp, tipo_de_plan, senda)%>%
    head(40)%>%
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 12. HASHs with more than one Age of Onset of Drug Use (First 40 Cases)",
                 align =rep('c', 101))  %>%
   kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
  kableExtra::scroll_box(width = "100%", height = "375px")
#_______________________
...
```{r tab13X_pers_info_edad_ini_sus_prin,eval=F, echo=T, paged.print=TRUE}
...
dplyr::select(row, hash_key, ano_bd, edad_ini_cons, edad, fech_nac,edad_al_ing, fech_ing, fech_egres_imp, tipo_de_plan, senda)%>%
    head(40)%>%
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 13. HASHs with more than one Age of Onset of Drug Use of Primary Substance (First 40 Cases)",
                 align =rep('c', 101))  %>%
   kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
  kableExtra::scroll_box(width = "100%", height = "375px")
#_______________________
...
- Starting Substance (sus_ini) (`r CONS_C1_df_dup_FEB_2020_prev27 %>% group_by(hash_key) %>% dplyr::mutate(sus_ini_por_hash=n_distinct(sus_ini)) %>% ungroup() %>% dplyr::filter(sus_ini_por_hash>1) %>% nrow() %>%  formatC(, format="f", big.mark=",", digits=0)`). For users that had only two treatments but a different starting substance, we added a second variable called `sus_ini_2` that contains a second starting substance.

<!---# considerar que no sea mayor a la edad de inicio de consumo --->

```{r tab12_pers_info_sus_ini, eval=T, echo=T, paged.print=TRUE}
...
CONS_C1_df_dup_FEB_2020_prev27 %>% 
 ...
 dplyr::arrange(hash_key) %>%
  dplyr::select(row, hash_key, ano_bd, edad_ini_cons, edad, fech_nac,edad_al_ing, fech_ing, fech_egres_imp, tipo_de_plan, senda)%>%
    head(40)%>%
    knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 12. HASHs with more than one Starting Substance (First 40 Cases)",
                 align =rep('c', 101))  %>%
   kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
  kableExtra::scroll_box(width = "100%", height = "375px")
#_______________________
...
CONS_C1_df_dup_FEB_2020_prev27 %>%
  dplyr::mutate(obs=case_when(hash_key %in% as.character(as.vector(unlist(as.data.table(unlist(CONS_C1_df_dup_FEB_2020_prev27_sus_ini)))))~glue::glue("{obs};2.6.05.HASH w/ more than one distinct Starting Substance"),TRUE~obs))%>%  
...
```{r fig_brief_summary, echo=T, fig.align='center', message=FALSE, fig.caption="Figure 5. Diagram of Process of Data Tidying and Cleaning"}
