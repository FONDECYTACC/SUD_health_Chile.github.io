...
if(no_mostrar==1){
ggsave(paste0(gsub("SUD_CL.*","",path),"/_mult_state_ags/loveplot_bal32_2022.eps"), loveplot_bal3, height=8, width=8)
...
```{r fig5a mstate_models_3s, echo=T, fig.align='center', message=FALSE, fig.caption="Figure 17a. Tree Structure for the Multistate System, Five-states model", error=T, fig.height=6, fig.width=12, eval=T, dpi=320}
library(Epi)
...
```{r trans_matrix1a,eval=T, echo=T, paged.print=TRUE}                                 
tab9_f<-
...
```{r trans_matrix1b,eval=T, echo=T, paged.print=TRUE}                                 
tab9_fb<-
...
```{r add_perc_in_res_a,eval=T, echo=T, paged.print=TRUE}                                 
ms_d_match_surv_res<-
...
```{r add_perc_in_res_b,eval=T, echo=T, paged.print=TRUE}                              
...
```{r ph1_b,eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.width=10, fig.cap="Figure 18b. LOG CUMULATIVE HAZARD VS LOG TIME PLOT (w/o covars) (9 states)", dpi=320, fig.align="center"}                                 
...
```{r ph2,eval=T, echo=T, paged.print=TRUE, fig.height=13, fig.width=10, fig.cap="Figure 18c. CUMULATIVE HAZARD PLOT: -LOG(KM SURVIVAL) (w/o covars)", dpi=320, fig.align="center"}      
layout(matrix(1:4, nc = 2, byrow = F))
...
```{r ph2_b,eval=T, echo=T, paged.print=TRUE, fig.height=13, fig.width=10, fig.cap="Figure 18d. CUMULATIVE HAZARD PLOT: -LOG(KM SURVIVAL) (w/o covars) (9 states)", dpi=740, fig.align="center"}      
...
```{r cox_zph_a, eval=T, echo=T, warning=FALSE, paged.print=TRUE}
...
```{r cox_zph_b, eval=T, echo=T, warning=FALSE, paged.print=TRUE}
cox_zph_trans2<-data.frame()#rn="0",chisq=1,df=1,p=1
for(i in c(1:max(trans_matrix2,na.rm=T))){
  
cox_zph_trans2<-dplyr::bind_rows(cox_zph_trans2,cbind.data.frame(trans=i,data.table::data.table(cox.zph(coxph(Surv(Tstart,Tstop,status)~factor(tipo_de_plan_res_1)+Tstart, data=subset(ms_d_match_surv_oct_2022, trans==i),method = "efron"))$table, keep.rownames = T))
)
}

for(i in c(1:max(trans_matrix2,na.rm=T))){
  print(paste0("Transition ", i))
    print(broom::tidy(coxph(Surv(Tstart,Tstop,status)~factor(tipo_de_plan_res_1)+yr_arrival, data=subset(ms_d_match_surv_oct_2022, trans==i) %>% dplyr::mutate(yr_arrival=Tstart/365.25),method = "efron"), exponentiate=T, conf.int=T))
}
