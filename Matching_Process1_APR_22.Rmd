...
if (grepl("CISS Fondecyt",path)==T){
    ...
    setwd("G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/SUD_CL/");load("E:/Mi unidad/Alvacast/SISTRAT 2019 (github)/8.RData")
  } else {
    setwd("~/");load("~/8.RData");path.expand("~/8.RData")
  }

rm(list=setdiff(ls(), c("CONS_C1_df_dup_SEP_2020_match","CONS_C1_df_dup_SEP_2020","CONS_C1","path", "irrs"))) 
```
...
#ACTUALIZADO install.packages(paste0(gsub("\\\\","/",Sys.getenv("GUROBI_HOME")),"/R/gurobi_9.5-1.zip"), repos = NULL, type="source")
#BUSCAR EN C:\gurobi si está actualizado
...
We selected treatments at baseline for each user, leaving `r CONS_C1_df_dup_SEP_2020 %>% janitor::tabyl(dup) %>% slice(1) %>% dplyr::select(n) %>% as.numeric(n)%>%  format(big.mark=",")` observations. Then, we distinguished between residential (n= `r CONS_C1_df_dup_SEP_2020 %>% dplyr::filter(dup==1, tipo_de_plan_2 %in% c("PG-PR","M-PR")) %>% nrow() %>%  format(big.mark=",")`) and ambulatory (n= `r CONS_C1_df_dup_SEP_2020 %>% dplyr::filter(dup==1, tipo_de_plan_2 %in% c("PG-PAI","M-PAI","PG-PAB","M-PAB")) %>% nrow() %>%  format(big.mark=",")`) treatments. We imputed cases that did not have a defined treatment assigned (n=`r CONS_C1_df_dup_SEP_2020 %>% dplyr::filter(dup==1, is.na(tipo_de_plan_2)) %>% nrow() %>% format(big.mark=",")`).
...
We started looking over the missing values in the age at admission (n= `r missing.values %>% dplyr::select(edad_al_ing) %>% as.numeric()`). Since `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss %>% dplyr::group_by(hash_key) %>%  dplyr::mutate(miss_edad_al_ing=ifelse(is.na(edad_al_ing),1,0),sum_miss_edad_al_ing=sum(miss_edad_al_ing,na.rm=T)) %>% dplyr::ungroup() %>% dplyr::filter(sum_miss_edad_al_ing>0, duplicates_filtered>1) %>% nrow()==0," there were not cases with more than one treatment that did not have an admission age, we did not have to impute taking into account serial dependencies in the dates of treatment.","**there were some users that had more than one treatment, it is necessary to analyze serial dependencies into the dates that were candidates to imputation**.")`
...
`r ifelse(anyNA(CONS_C1_df_dup_SEP_2020_match_miss0$edad_al_ing),paste0("There are still ",CONS_C1_df_dup_SEP_2020_match_miss0%>% dplyr::filter(is.na(edad_al_ing)) %>% nrow()," missing cases."),"After the imputation, there were no missing cases left.")`

<br>

### Substance Use Onset Age

Another variable worth imputing is the Substance Use Onset Age (n= `r missing.values %>% dplyr::select(edad_ini_cons) %>% as.numeric() %>%  format(big.mark=",")`).
...
There were `r CONS_C1_df_dup_SEP_2020_match_miss1 %>% dplyr::filter(is.na(edad_ini_cons)) %>% nrow()` cases of imputed substance use onset ages that did not fulfill the conditions necessary to replace the missing values with the imputed ones.
...
As a result of the imputations, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss2 %>% dplyr::filter(is.na(sus_ini_mod_mvv)) %>% nrow()==0,paste0(" there were no missing values."),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss2 %>% dplyr::filter(is.na(sus_ini_mod_mvv)) %>% nrow(),")."))`

<br>

### Primary Substance at Admission Usage Frequency

Another variable that is worth imputing is the Primary Substance at Admission Usage Frequency (n= `r CONS_C1_df_dup_SEP_2020_match_miss %>% dplyr::filter(is.na(freq_cons_sus_prin)) %>% nrow()`). In case of ties, we selected the imputed values with the value with the most frequent substance use pattern. 
...
As a result of the imputations, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss3 %>% dplyr::filter(is.na(freq_cons_sus_prin)) %>% nrow()==0,paste0(" there were no missing values."),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss3 %>% dplyr::filter(is.na(freq_cons_sus_prin)) %>% nrow(),")."))`

<br>

### Educational Attainment

Another variable that is worth imputing is the Educational Attainment (n= `r CONS_C1_df_dup_SEP_2020_match_miss %>% dplyr::filter(is.na(escolaridad_rec)) %>% nrow()`). We followed the rules stated in the `Duplicates4` process ([link](https://fondecytacc.github.io/SUD_health_Chile.github.io/Duplicates4)). We were particularly cautious to impute attainments that would follow a progression from primary school to more than high school. For this purpose, we first looked over the actual values per user, filling intermediate gaps in educational attainment of users with intermediate null values (a), we overcame the difficulty of the incorrect imputations by logically selecting them if there were any.
...
We looked over possible imputations for region of the center (n=`r CONS_C1_df_dup_SEP_2020_match_miss %>% dplyr::filter(is.na(nombre_region)) %>% nrow()`) and type of the center (public or private) (n=`r CONS_C1_df_dup_SEP_2020_match_miss %>% dplyr::filter(is.na(tipo_centro_pub)) %>% nrow()`).
...
It was impossible to impute region of the center in `r as.numeric(table(is.na(CONS_C1_df_dup_SEP_2020_match_miss6$nombre_region)))[2]` cases due to ties in the different imputed values. In the case of public or private center, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss6 %>% dplyr::filter(is.na(tipo_centro_pub)) %>% nrow()==0,paste0("all values were imputed"),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss6 %>% dplyr::filter(is.na(tipo_centro_pub)) %>% nrow(),")"))`
...
if(nrow(CONS_C1_df_dup_SEP_2020_match_miss7)-nrow(CONS_C1_df_dup_SEP_2020_match_miss6)>0){
  warning("Some rows were added in the imputation")}
```
As a result of the imputations, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss7 %>% dplyr::filter(is.na(dg_trs_cons_sus_or)) %>% nrow()==0,paste0(" there were no missing values."),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss7 %>% dplyr::filter(is.na(dg_trs_cons_sus_or)) %>% nrow(),")."))`
...
As a result of the imputations, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss9 %>% dplyr::filter(is.na(evaluacindelprocesoteraputico)) %>% nrow()==0,paste0(" there were no missing values."),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss9 %>% dplyr::filter(is.na(evaluacindelprocesoteraputico)) %>% nrow(),")."))`
...
As a result of the process of imputation, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss10 %>% dplyr::filter(is.na(tipo_de_plan_res)) %>% nrow()==0,paste0("all values were imputed"),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss10 %>% dplyr::filter(is.na(tipo_de_plan_res)) %>% nrow(),")."))`

<br>

### Tenure status of households

Another variable that is worth imputing is the Tenure status of households (n= `r CONS_C1_df_dup_SEP_2020_match_miss %>% dplyr::filter(is.na(tenencia_de_la_vivienda_mod)) %>% nrow() %>% format(big.mark=",")`). In case of ties, we selected the imputed values with the value with the minimum involvement. In case of ties, we kept what we thought was the most vulnerable value (discarding "Owner" or "Renting" values).

<br>

```{r mice10_tenencia_vivienda, eval=T, echo=T, paged.print=TRUE}
...
As a result of the imputations, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss11 %>% dplyr::filter(is.na(tenencia_de_la_vivienda_mod)) %>% nrow()==0,paste0(" there were no missing values."),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss11 %>% dplyr::filter(is.na(tenencia_de_la_vivienda_mod)) %>% nrow(),")."))`
...
```{r mice12_main_subs,eval=T, echo=T, paged.print=TRUE}
# Ver distintos valores propuestos para sustancia de inciio
...
As a result of the imputations, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss12 %>% dplyr::filter(is.na(sus_principal_mod)) %>% nrow()==0,paste0(" there were no missing values."),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss12 %>% dplyr::filter(is.na(sus_principal_mod)) %>% nrow(),")."))`
...
Considering that some missing values were not able to be imputed (due to ties in the candidate values for imputation or inconsistent values for imputations) (`r dim(CONS_C1_df_dup_SEP_2020_match_miss_after_imp_descartados)[1]`, users=`r length(unique(CONS_C1_df_dup_SEP_2020_match_miss_after_imp_descartados$hash_key))`), we ended the process with a total of `r nrow(CONS_C1_df_dup_SEP_2020_match_miss_after_imp_conservados) %>%  format(big.mark=",")` complete cases (users=`r length(unique(CONS_C1_df_dup_SEP_2020_match_miss_after_imp_conservados$hash_key))%>%  format(big.mark=",")`).  
...
```{r  prueba, warning=FALSE, fig.align = "center", message=F, fig.height=7, cache=T, eval=T, fig.cap="Figures 9-12. Empirical Cumulative Distribution Functions on the Matched Sample", results='asis'}
...
```{r  tab3_match1b, echo=T, warning=FALSE, fig.align = "center", message=F, fig.height=10, cache=T, eval=T}
...
We allowed to tolerate `r smd_df2[order(smd_df2$smd, decreasing = TRUE), ][[1]][[1]]` (SMD=`r as.numeric(dplyr::filter(smd_df_prev_match_after_match,grepl("Admission Date", unlist_var_names) & grepl("After", variable))[4])`), because the date of admission not necessarily had to be strictly balanced, assuming that not every user had to be admitted to treatment in exact dates.
...
The incidence rate of readmission was `r round(as.numeric(irrs_early_drop$estimate[1]),2)` (95% IC `r round(irrs_early_drop$conf.int[1],2)`-`r round(irrs_early_drop$conf.int[2],2)`) in users that had at least an early dropout, compared to users that did not have a physical condition at baseline (p`r ifelse(sprintf("%1.3f",irrs_early_drop$p.value)=="0.000","<0.001",paste0("= ", sprintf("%1.3f",irrs_early_drop$p.value)))`).

<br>

```{r irrs_health_conditions2,eval=T, echo=T, paged.print=TRUE}                                 
...
Excluding censored cases, the incidence rate of readmission was significantly higher (`r round(as.numeric(comp_status_readmission_setting$estimate[1]),2)`, 95% IC `r round(comp_status_readmission_setting$conf.int[1],2)`-`r round(comp_status_readmission_setting$conf.int[2],2)`) in users that had a Discharge without clinical advice, compared to users that had a Therapeutic discharge (p`r ifelse(sprintf("%1.3f",comp_status_readmission_setting$p.value)=="0.000","<0.001",paste0("= ", sprintf("%1.3f",comp_status_readmission_setting$p.value)))`).
...
The incidence rate of readmission was `r round(as.numeric(irrs_res_plan$estimate[1]),2)` (95% IC `r round(irrs_res_plan$conf.int[1],2)`-`r round(irrs_res_plan$conf.int[2],2)`) in users that had a residential plan, compared to users that had an ambulatory plan at baseline (p`r ifelse(sprintf("%1.3f",irrs_res_plan$p.value)=="0.000","<0.001",paste0("= ", sprintf("%1.3f",irrs_res_plan$p.value)))`).

<br>


```{r fig13_survplot_tipo_de_plan_res, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 13. Cum. Hazards to Experience Readmission to SUD Treatment, by Treatment Setting at Baseline", error=T,fig.width=8}
...
The incidence rate of readmission was `r round(as.numeric(irrs_min_ach$estimate[1]),2)` (95% IC `r round(irrs_min_ach$conf.int[1],2)`-`r round(irrs_min_ach$conf.int[2],2)`) in users that had a minimum achievement of the therapeutic goals, compared to the rest of users at baseline (p`r ifelse(sprintf("%1.3f",irrs_min_ach$p.value)=="0.000","<0.001",paste0("= ", sprintf("%1.3f",irrs_min_ach$p.value)))`).
...
```{r surv_tab_total, warning=FALSE, echo=T, eval=T}
d_match_surv<-
CONS_C1_df_dup_SEP_2020_match_miss_after_imp_conservados %>% 
...
# try(attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$sus_principal_mod,"label")<-"Primary Substance at Admission")
# attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$sus_ini_mod_mvv,"label")<-"First Substance Used"
# attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$estado_conyugal_2,"label")<-"Marital Status"
# attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$escolaridad_rec,"label")<-"Educational Attainment"
# attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$edad_ini_cons,"label")<-"Substance Use Onset Age"
...
# attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$tipo_de_plan_res,"label")<-"Residential Setting"
# attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$tipo_centro_pub,"label")<-"Public Center"
# attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$condicion_ocupacional_corr,"label")<-"Employment Status"
# try(attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$tenencia_de_la_vivienda_mod,"label")<-"Tenure status of household")
# try(attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$edad_al_ing,"label")<-"Admission Age")
# try(attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$fech_ing_num,"label")<-"Admission Date")
# try(attr(CONS_C1_df_dup_SEP_2020_match_not_miss2$origen_ingreso_mod,"label")<-"Treatment Admission Motive")
...
```{r fig5 mstate_models_3s, echo=T, fig.align='center', message=FALSE, fig.caption="Figure 17. Tree Structure for the Multistate System, Five-states model", error=T, fig.height=6, fig.width=12, eval=T, dpi=320}
library(Epi)
...
To the first and second states, we subtracted one day if it overlaps with the date of discharge. For the third and the following states, we added one day in case of overlapping dates due to continuous treatments. We left `r d_match_surv %>% dplyr::filter(dup>=5) %>% nrow()` entries of `r d_match_surv %>% dplyr::filter(dup>=5) %>% dplyr::distinct(id) %>% nrow()` patients that had more readmissions.
...
```{r msprep_1_oct_2022, eval=T, echo=T, paged.print=TRUE}                                 
### diff_bet_treat is the variable that includes time-to-readmission
### AGS: Starts in 0, excepting left truncated cases
### variables should start with time_ & status_
### Transform to years once generated
...
## 2021-05-06, CENSORED TIME IS NOT THE DIFFERENCE BETWEEN THE TIME OF CENSORSIP AND THE TIME OF THE LAST EVENT, IS THE TOTAL DIFFERENCE. THE SUM OF DAYS UNTIL THE FOLLOWUP TIME
  ...
  dplyr::mutate( 
  Readmission_time= dplyr::case_when(
        Readmission_status==1 & TD_1==1~ as.numeric(fech_ing_num_2-(TD_time+fech_ing_num_1)),
        Readmission_status==1 & DWCA_1==1~ as.numeric(fech_ing_num_2-(DWCA_time+fech_ing_num_1)),
        Readmission_status==0~as.numeric(as.Date("2019-11-13"))-fech_ing_num_1)) %>% 
  dplyr::mutate( 
  Readmission2_time= dplyr::case_when(
        Readmission2_status==1~as.numeric(fech_ing_num_3-fech_ing_num_1),
  ...
  #OCT 2022    
  dplyr::mutate(Readmissionb_time= dplyr::case_when(DWCA_1==1~Readmission_time,T~as.numeric(as.Date("2019-11-13"))-fech_ing_num_1), Readmission2b_time= dplyr::case_when(DWCA_1==1~Readmission2_time,T~as.numeric(as.Date("2019-11-13"))-fech_ing_num_1),Readmission3b_time= dplyr::case_when(DWCA_1==1~Readmission3_time,T~as.numeric(as.Date("2019-11-13"))-fech_ing_num_1)) %>% dplyr::mutate(Readmission_time= dplyr::case_when(TD_1==1~Readmission_time,T~as.numeric(as.Date("2019-11-13"))-fech_ing_num_1), Readmission2_time= dplyr::case_when(TD_1==1~Readmission2_time,T~as.numeric(as.Date("2019-11-13"))-fech_ing_num_1),Readmission3_time= dplyr::case_when(TD_1==1~Readmission3_time,T~as.numeric(as.Date("2019-11-13"))-fech_ing_num_1)) %>% 
  dplyr::mutate(Readmissionb_status= dplyr::case_when(DWCA_1==1~Readmission_status,T~0), Readmission2b_status= dplyr::case_when(DWCA_1==1~Readmission2_status,T~0), Readmission3b_status= dplyr::case_when(DWCA_1==1~Readmission3_status,T~0)) %>% dplyr::mutate(Readmission_status=  dplyr::case_when(TD_1==1~ Readmission_status,T~0), Readmission2_status= dplyr::case_when(TD_1==1~ Readmission2_status,T~0), Readmission3_status= dplyr::case_when(TD_1==1~ Readmission3_status,T~0)) %>% 
...
#OCT 2022- format into cumulative time
dplyr::mutate( Readmission3b_time=dplyr::case_when(Readmission3b_status==1~ Readmission3b_time+Readmission2b_time+ Readmissionb_time+ DWCA_time, T~Readmission3b_time),
               Readmission2b_time=dplyr::case_when(Readmission2b_status==1~ Readmission2b_time+ Readmissionb_time+ DWCA_time, T~Readmission2b_time),
               Readmissionb_time=dplyr::case_when(Readmissionb_status==1~ Readmissionb_time+ DWCA_time, T~Readmissionb_time)) %>% 
  
  dplyr::mutate( Readmission3_time=dplyr::case_when(Readmission3_status==1~ Readmission3_time+Readmission2_time+ Readmission_time+ TD_time, T~Readmission3_time),
               Readmission2_time=dplyr::case_when(Readmission2_status==1~ Readmission2_time+ Readmission_time+ TD_time, T~Readmission2_time),
               Readmission_time=dplyr::case_when(Readmission_status==1~ Readmission_time+ TD_time, T~Readmission_time)) %>% 
...
d_match_surv_msprep_oct_2022 %>% 
  dplyr::filter(duplicates_filtered==max(duplicates_filtered)) %>% 
      knitr::kable(format= "html", format.args= list(decimal.mark= ".", big.mark= ","),
               caption="Table 8b. Data in Wide, Five-states (example of patients with >= 5 admissions)",
               align= c("c",rep('c', 5)))%>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size= 13)%>% 
  kableExtra::add_footnote("Note= Proportions from the initial state") %>% 
  kableExtra::kable_classic() %>% 
...
#ms_d_match_surv_oct_2022 %>% dplyr::filter(trans==4& is.infinite(time))
#d_match_surv_msprep_oct_2022 %>% dplyr::filter(id%in%c(5,7))

#mstate::events(ms_d_match_surv_oct_2022)
...
if(no_mostrar==1){
jpeg(paste0(gsub("SUD_CL/Matching_Process_APR_22.Rmd","",path),"_mult_state_ags/ph_test2.jpg"), height=7, width= 13, res= 320, units = "in")
...
```{r cox_zph, eval=T, echo=T, warning=FALSE, paged.print=TRUE}
...
cox_zph_trans_cont %>% 
    knitr::kable(format= "html", format.args= list(decimal.mark= ".", big.mark= ","),
               caption="Table 10. Schoenfeld's global test to test the proportional hazards assumption",
               align= c("c",rep('c', 5)))%>%
