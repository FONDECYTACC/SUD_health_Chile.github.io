...
```{r paso1_dates, fig.align='center', fig.caption="Figure. Absolute differences between SENDA and POs date of birth", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
...
Base_fiscalia_v3 %>% 
    ...
    dplyr::select(rut_enc_saf,gls_tipo_sujeto_vic,fec_nacimiento_simple,fec_comision_simple, termino_relacion_simple,fech_nac, edad_al_ing, edad_ini_cons,contains("rpa"), obs) %>% 
...
```{r paso1_dates2, fig.align='center', fig.caption="Figure. SENDA patients with differences in the dates of birth of SENDA DB. Number of distinct dates of comission of crimes", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
Base_fiscalia_v3 %>% 
    ...
    labs( caption="Note. 11,511 patients, 90,037 entries", x="Number of distinct dates by patient")+
...
Base_fiscalia_v3 %>% 
    ...
    dplyr::mutate(n_dis_fec=n_distinct(fec_comision)) %>% 
    dplyr::ungroup() %>% 
    dplyr::summarise(n=n(), min=min(n_dis_fec, na.rm=T), p10= quantile(n_dis_fec,.1,na.rm=T),p25=quantile(n_dis_fec,.25,na.rm=T),p50=quantile(n_dis_fec,.5,na.rm=T),p75=quantile(n_dis_fec,.75,na.rm=T),p90=quantile(n_dis_fec,.9,na.rm=T), max=max(n_dis_fec,na.rm=T))
```
...
```{r bpmn, echo=T, cache= T, paged.print=TRUE, eval=F, error=T, dpi=320, fig.align="center",fig.cap="Figure . Decision Tree to correct dates of birth"}
...
paste0("No changes needed\n (n= ",format(length(unique(Base_fiscalia_v3$rut_enc_saf))-nrow(fech_nac_fisc_vs_senda),big.mark=","),";\n",
       scales::percent((length(unique(Base_fiscalia_v3$rut_enc_saf))-nrow(fech_nac_fisc_vs_senda))/length(unique(Base_fiscalia_v3$rut_enc_saf))),"; ",
       scales::percent((length(unique(Base_fiscalia_v3$rut_enc_saf))-nrow(fech_nac_fisc_vs_senda))/length(unique(CONS_C1_df_dup_SEP_2020$hash_key))),")")


paste0("as.numeric(as.Date('2019-11-13')-as.Date('2003-11-13'))/365.25 = ",as.numeric(as.Date('2019-11-13')-as.Date('2003-11-13'))/365.25," years")

paste0("1.Cases with less than 16 years old when they committed the crime: ",
fech_nac_fisc_vs_senda %>% 
...
```{r paso1_sex, fig.align='center', fig.caption="Figure 16.Summary of Process of Data Cleaning and Standardization, Imputation & Matching", echo=T, cache= T, paged.print=TRUE, eval=T, error=T, dpi=320, fig.align="center"}
...
Base_fiscalia_v3 %>% 
    ...
    dplyr::select(rut_enc_saf,sexo,sexo, sexo_2,embarazo, centro_muj) %>% 
