...
```{r 0, include=T,warning=F, message=F, results="hide"}
coxphw_pwp_int<-
...
timecox_pwp_const1<- 
  timecox(Surv(time,status) ~ tipo_de_plan_res_1*strata(trans)+ 
            TD_1+ const(TD_2)+ TD_3+ const(TD_4)+ cluster(id) + strata(trans), Nit = 100,
          data = ms_d_match_surv, n.sim=3000, robust = 1, weighted.test = 1)
```
</div>
...
<div style="border: 1px solid #ddd; padding: 5px; overflow-y: scroll; height:350px; overflow-x: scroll; width:100%">
```{r fourth_try,eval=T, echo=T, paged.print=TRUE, error=T}      
...
timecox_pwp_const2_opc2<- 
  ...
  #         data = ms_d_match_surv, n.sim=3000, robust = 1, weighted.test = 1)
  timecox(Surv(time,status) ~ const(tipo_de_plan_res_1)*strata(trans)+ 
            const(TD_1)+ const(TD_2)+ const(TD_3)+ const(TD_4)+ cluster(id), Nit = 100,
          data = ms_d_match_surv, n.sim=3000, robust = 1, weighted.test = 1)
```
</div>


```{r fourth_try2,eval=T, echo=T, paged.print=TRUE, error=T}    
