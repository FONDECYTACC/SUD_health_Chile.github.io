...
if (grepl("CISS Fondecyt",path)==T){
    setwd("C:/Users/CISS Fondecyt/Mi unidad/Alvacast/SISTRAT 2019 (github)/SUD_CL/");load("C:/Users/CISS Fondecyt/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_15_apr22.RData")
  } else if (grepl("andre",path)==T){
    setwd('C:/Users/andre/Desktop/SUD_CL/');load("E:/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_15_apr22.RData")
  } else if (grepl("E:",path)==T){
    setwd("E:/Mi unidad/Alvacast/SISTRAT 2019 (github)/SUD_CL/");load("E:/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_15_apr22.RData")
  } else if (grepl("G:",path)==T){
    setwd("G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/SUD_CL/");load("E:/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_15_apr22.RData")
  } else {
    setwd("~/");load("~/mult_state_15_apr22.RData");path.expand("~/mult_state_15_apr22.RData")
  }
```
...
```{r msfit2.5_4s, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 2b. Estimate of Cumulative Hazards (Semi-Markov), Five-states", fig.align="center", error=T, dpi=320}
...
```{r mssample_plot, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 3. Estimate of State Occupancies at Baseline, Five-states model (simulated approach)", fig.align="center", error=T}
...
LMAJ(pr1_sel, #An "msdata" object, as for instance prepared by link{msprep}
                  from=4, ##Either a single state or a set of states in the state space 1,...,S
                  s=365*9,  #The prediction time point s from which transition probabilities are to be obtained
                  method ="aalen" #The method for calculating variances, as in probtrans
                  )
#if (nrow(mf) ==0) stop("No (non-missing) observations")
...
```{r lmaj_plot, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 4a. State Occupancies, landmark Aalen-Johansen estimator (At 3 years, from State 1)", fig.align="center", error=T}
...
lmaj_plot<-
rbind(cbind(pr="res",lmaj_res_1_1095,transo=1),
      cbind(pr="out",lmaj_out_1_1095,transo=1)) %>% 
  ...
  scale_x_continuous(breaks=seq(0,max(c(msf0$Haz$time,msf1$Haz$time),na.rm=T),by=max(c(msf0$Haz$time,msf1$Haz$time),na.rm=T)/12),
                     label=round(seq(0,max(c(msf0$Haz$time,msf1$Haz$time),na.rm=T),by=max(c(msf0$Haz$time,msf1$Haz$time),na.rm=T)/12)/365.25,0))+
  xlab("Years") + 
  ylab("") + 
  theme_minimal(base_size = 18)+
  theme(legend.position = c(0.75,.18))
...
```{r lmaj_plot2, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 4b. State Occupancies, landmark Aalen-Johansen estimator (at s=3 years)", fig.align="center", error=T}
#A data frame containing estimates and associated standard errors of the transition probabilities P(X(t)=k | X(s) in from) with s and from the arguments of the function.
...
lmaj_plot2<-
rbind(cbind(pr="res",lmaj2_res_1_1095,transo=1),
      cbind(pr="out",lmaj2_out_1_1095,transo=1)) %>% 
  melt(id.vars=c("pr","transo","time")) %>% 
  dplyr::rename("state_from"="transo") %>% 
  dplyr::mutate(state_to=stri_sub(variable,-1,-1)) %>%
    dplyr::mutate(setting= dplyr::case_when(grepl("out",pr)~"Ambulatory",
  ...
  geom_step(size=1, alpha=.65) + 
  geom_ribbon(aes(ymin=lo_ci, ymax=up_ci, fill=setting), alpha=.5)+#, fill=setting
  facet_wrap(state_to~., ncol=1, labeller = as_labeller(states_trans_lab)) + #scales="free_y",
  scale_y_continuous(labels = scales::percent_format(accuracy = .1))+
  #facet_wrap(state~., ncol=1, scales="free_y") + #type_of_program
  scale_fill_manual(name= "Type of\nprogram",values=c("gray35","gray75"), labels=c("Ambulatory","Residential"))+
  scale_linetype_manual(name= "Type of\nprogram",values=c(1,4), labels=c("Ambulatory","Residential")) +
...
if(no_mostrar==1){
jpeg(paste0(jpg_path,"_mult_state_ags/lmaj_st_occ_apr22.jpg"), height=20, width= 10, res= 320, units = "in")
lmaj_plot2
dev.off()
}
```
...
lmaj_ci_df<-
  rbind(cbind(pr="res",lmaj2_res_1_1095,transo=1),
  cbind(pr="out",lmaj2_out_1_1095,transo=1)) %>% 
  rbind(cbind(pr="res",lmaj2_res_2_1095,transo=2),
  cbind(pr="out",lmaj2_out_2_1095,transo=2)) %>% 
  rbind(cbind(pr="res",lmaj2_res_3_1095,transo=3),
  cbind(pr="out",lmaj2_out_3_1095,transo=3)) %>% 
  rbind(cbind(pr="res",lmaj2_res_4_1095,transo=4),
  cbind(pr="out",lmaj2_out_4_1095,transo=4)) %>% 
...
lmaj_ci_df2<-
  ...
  rbind(cbind(pr="res",lmaj2_res_2,transo=2),
  cbind(pr="out",lmaj2_out_2,transo=2)) %>% 
  rbind(cbind(pr="res",lmaj2_res_3,transo=3),
  cbind(pr="out",lmaj2_out_3,transo=3)) %>% 
  dplyr::mutate(lo_pstate1=pmax(0,1-(1-pstate1)* + exp(qnorm(0.975)*se1/(1-pstate1)))) %>% 
  dplyr::mutate(up_pstate1=1-(1-pstate1)* + exp(-qnorm(0.975)*se1/(1-pstate1))) %>% 
  dplyr::mutate(lo_pstate2=pmax(0,1-(1-pstate2)* + exp(qnorm(0.975)*se2/(1-pstate2)))) %>% 
  dplyr::mutate(up_pstate2=1-(1-pstate2)* + exp(-qnorm(0.975)*se2/(1-pstate2))) %>% 
...
lmaj_ci_df2 %>% 
    ...
    dplyr::filter(time %in% c(90, 365, 1095, 1826,1827)) %>%
    dplyr::mutate(time=factor(time, levels=c(90, 365, 1095, 1826,1827),
                              labels=c("~90days/3 months", "~1 year", "~3 years", "~5 years","~5 years"),ordered=T)) %>% 
    dplyr::mutate(comb=paste0(state_from,"_",state_to)) %>% 
    dplyr::filter(comb %in% c("1_2","1_2","1_3","1_4","1_5")) %>% 
...
if(no_mostrar==1){
jpeg(paste0(jpg_path,"_mult_state_ags/probtrans_states_trans_apr22_3.jpg"), height=10, width= 15, res= 320, units = "in")
lmaj_ci_df_plot2
dev.off()
}
...
```{r probtrans_plot, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.width=10, fig.cap="Figure 5. Estimate of State Occupancies at Baseline, Five-states model (Markov)", fig.align="center", error=T}
...
ab_pmatrix_probtrans<-
  ...
  #pstate1–pstate3 are close to the first rows of the matrices returned by pmatrix.fs
  reshape2::melt(id.vars="time") %>% #janitor::tabyl(variable)
  dplyr::mutate(start=stringi::stri_sub(variable,-1,-1)) %>% 
  dplyr::mutate(state_to=stringi::stri_match_first_regex(variable, "(.*?)\\.")[,2]) %>%
  dplyr::mutate(state_to=gsub("pstate", "", state_to), state_to=as.numeric(state_to)) %>% 
  dplyr::mutate(setting= dplyr::case_when(grepl(".out",variable)~"Ambulatory",
                                               grepl(".res",variable)~"Residential",
...
fig_ab_pmatrix_probtrans<-
  #janitor::tabyl(start)
  ggplot(ab_pmatrix_probtrans, aes(time, value, color= factor(state_to), linetype =setting))+
    geom_step(size=1, alpha=.65) + 
    scale_y_continuous(labels = scales::percent_format(accuracy = 2))+
    facet_wrap(start~., ncol=2, scales="free_y", labeller = as_labeller(states_trans_lab)) + 
    #facet_wrap(state~., ncol=1, scales="free_y") + #type_of_program
    scale_color_manual(name = "Starting from\nState", values=c26, labels= as.character(c(1:10))) + # "#A65100" #"Cox",
    scale_linetype_manual(name= "Type of\nprogram",values=c(1,4), labels=c("Ambulatory","Residential")) +
   scale_x_continuous(breaks=seq(0,max(c(msf0$Haz$time,msf1$Haz$time),na.rm=T),by=365.25),
                     label=seq(0,round(max(c(msf0$Haz$time,msf1$Haz$time),na.rm=T)/365.25),by=1))+
    xlab("Years") + 
    ylab("") + 
    theme_minimal()+
...
if(no_mostrar==1){
jpeg(paste0(jpg_path,"_mult_state_ags/probtrans_states_apr22.jpg"), height=10, width= 10, res= 320, units = "in")
fig_ab_pmatrix_probtrans
dev.off()
}
```

```{r probtrans_plot2, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 6. Estimate of Transition Probabilities to the following state, Five-states model (Markov)", fig.align="center", error=T}
myplots<- vector("list")
...
probtrans_df<-
rbind(cbind(subset(myplots$pt_0_1$data,state_num %in% c(2))[,c("time","state","prob","CI_low","CI_upp")],res=0, st=1),
      cbind(subset(myplots$pt_1_1$data,state_num %in% c(2))[,c("time","state","prob","CI_low","CI_upp")],res=1, st=1),
      cbind(subset(myplots$pt_0_2$data,state_num %in% c(3))[,c("time","state","prob","CI_low","CI_upp")],res=0, st=2),
      cbind(subset(myplots$pt_1_2$data,state_num %in% c(3))[,c("time","state","prob","CI_low","CI_upp")],res=1, st=2),
      ...
      cbind(subset(myplots$pt_0_4$data,state_num %in% c(5))[,c("time","state","prob","CI_low","CI_upp")],res=0, st=4),
      cbind(subset(myplots$pt_1_4$data,state_num %in% c(5))[,c("time","state","prob","CI_low","CI_upp")],res=1, st=4)
)
...
probtrans_df %>% 
 ...
 ggplot()+
    geom_step(aes(time, prob, color= res, fill=res, linetype =res),size=1, alpha=.75) + 
    geom_ribbon(aes(x=time, fill=res, color= res, linetype =res, ymin=CI_low, ymax=CI_upp), alpha=.35)+
    scale_y_continuous(labels = scales::percent_format(accuracy = 2))+
    facet_wrap(st~., ncol=1, scales="free_y", labeller = as_labeller(states_trans_lab)) + 
    #facet_wrap(state~., ncol=1, scales="free_y") + #type_of_program
    scale_color_manual(name = "Exit\nState", values=c("gray30","gray70"), labels=c("Ambulatory","Residential")) +
...
if(no_mostrar==1){
jpeg(paste0(jpg_path,"_mult_state_ags/probtrans_states_trans_apr22.jpg"), height=20, width= 10, res= 320, units = "in")
probtrans_df_plot
dev.off()
}
```
...
invisible(c("Produce datos e intervalos"))
set.seed(2125)
P23 <- patp(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", 
             h=2, j=3, s=0, B=1000, LMAJ=T, cband=T)
...
invisible(c("No produce dato alguno"))
set.seed(2125) 
...
set.seed(2125)
P12_90 <- patp(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", 
             h=1, j=2, s=90, B=1000, LMAJ=T, cband=T)
invisible(c("Entrega intervalos"))
set.seed(2125)
P23_90 <- patp(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", 
             h=2, j=3, s=90, B=1000, LMAJ=T, cband=T)
invisible(c("Entrega intervalos"))
set.seed(2125) 
P34_90 <- patp(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", 
             h=3, j=4, s=90, B=1000, LMAJ=T, cband=T)
invisible(c("Entrega error"))
set.seed(2125)
P45_90 <- patp(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", 
             h=4, j=5, s=90, B=1000, LMAJ=T, cband=T)
invisible(c("Entrega error"))
...
tryCatch({P12_clus_0$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
# 0 
...
tryCatch({P23_clus_0$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P34_clus_0$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P45_clus_0$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P12_clus_90$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#  0  
...
tryCatch({P23_clus_90$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#.816
...
tryCatch({P34_clus_90$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P45_clus_90$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P12_clus_365$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#  0 
...
tryCatch({P23_clus_365$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
# 0.75
...
tryCatch({P34_clus_365$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P45_clus_365$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P12_clus_1095$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#0.08 
...
tryCatch({P23_clus_1095$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#0.11 
...
tryCatch({P34_clus_1095$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P45_clus_1095$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#Error
...
tryCatch({P12_clus_1826$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#0.191 
...
tryCatch({P23_clus_1826$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
#0.093
...
tryCatch({P34_clus_1826$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
...
tryCatch({P45_clus_1826$pval},error=function(error_message){warning(paste0("Error"));print("");message(error_message)})
```
</div>
...
bind_rows(cbind.data.frame(P12,at=0,t=12),
                 ...
                 #cbind.data.frame(P12_1826,at=1826,t=12)
                                  ) %>% 
