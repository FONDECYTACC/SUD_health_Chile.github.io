...
df<- data.frame()
  for (i in names(Filter(is.factor,codebook_data))) { 
      ...
      df_titulo<-data.frame(variable=paste0(i),export=NA,cat=NA, N=9999999)
...
df<-
  ...
  #15. Pararse en n_missing y generar una columna a la izquierda, pegando el siguiente formula: =SI(SI.ERROR(BUSCARV(CONCATENAR(A4,"_",CONTAR.SI($A$4:$A4,A4)-1),para_pegar_cat_factores!F:H,3,0),"")=999,"",SI.ERROR(BUSCARV(CONCATENAR(A4,"_",CONTAR.SI($A$4:$A4,A4)-1),para_pegar_cat_factores!F:H,3,0),""))
