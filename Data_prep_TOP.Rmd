...
We replaced SENDA's ID, age, ID and date of birth to those cases, to get a much cleaner dataset of valid cases. **One observation that come from this transformation, is that many of TOPs applications that had missing values in one value, had missing values in many other more. Possibly, there were duplicated events, and some of them were invalid. If this is true, we only need to discard events with incomplete data instead of replacing them with valid information. But first we need to check whether this happens frequently or not. This have to be contrasted in the next stages of data preparation**.
