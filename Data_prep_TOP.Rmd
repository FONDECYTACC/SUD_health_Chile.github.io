...
As seen in Table 1, C1 dataset had more variables than the TOPs dataset. One of the variables that appear in both datasets is age, sex, the center of treatment, the region of the center and type of center, primary substances, date of admission to treatment, and type of plan of the treatment. Considering the above, we might replace missing or invalid information of C1 to TOP.

### 2. Change Dates of Admission, Aplication of TOP and Birth

As can be seen in Table 2, we brought a new format to every date to show first a 4-digit year, 2-digit month, and 2-digit day. This format also matches the format of these variables in the C1 dataset. Additionally, we masked SENDA’s IDs to avoid the eventual identification of users from third parties.
...
- Replace them from C1 dataset, using the date of application and the stage of application as a reference to determine the closest date of admission (2)

<br>

First, we identified applications of the TOP that were applied for the same stage (TOP at Admission) and the same user on the same dates of application. These cases could replace missing dates of admission.
...
dplyr::left_join(CONS_TOP_df_dup_fech_ing_adm, by = c("HASH_KEY", "fech_ap_top","row_leftjoin"), suffix = c("", ".y")) %>% #names() #junto con la BD que hice, sólo tomo una.
  dplyr::mutate(OBS= ifelse(is.na(fech_ing) & !is.na(fech_ing.y), "1.1.Replace miss date admission w TOPs w same stage & user","")) %>%
...
In Table 4, we may see how many dates could be replaced by the dates in C1. We replaced these dates by selecting the applications correspondent to the stage of admission that had missing data. Then, we paired them with cases in C1 with the same HASH and the same date of admission (in which the date of application of the TOP for admission coincides with the date of admission in C1). Finally, we replaced it with this corresponding date.
...
CONS_TOP_df_dup_ENE_2020_prev0 %>% 
  ...
  dplyr::mutate(OBS=ifelse((is.na(fech_ing))&(TOP=="Ingreso")&(!is.na(fech_ing_C1)),glue::glue("{OBS};1.2.Replaced missing dates of admission from C1"),OBS))%>% 
...
**And what about another important event that is present along the C1 dataset?**. Despite that the C1 datast had 2 types of events, unfortunately, no other dates of admission could be replaced by the dates in C1. We filtered the applications correspondent to the event of discharge that had missing data in the date of admission (n= `r CONS_TOP_df_dup_ENE_2020_prev1 %>% dplyr::filter(TOP=="Egreso", is.na(fech_ing)) %>% nrow()`). Then, we paired them with cases in C1 with the same HASH and the same date of discharge (in which the date of application of the TOP for discharge coincides with the date of discharge in C1), ending with `r CONS_TOP_df_dup_ENE_2020_prev1 %>% dplyr::filter(TOP=="Egreso", is.na(fech_ing)) %>% dplyr::inner_join(dplyr::select(CONS_C1_df,HASH_KEY, fech_ing, fech_egres) %>%   mutate(fech_egres=as.Date(as.character(fech_egres))), by=c("HASH_KEY"="HASH_KEY", "fech_ap_top"="fech_egres"), suffix=c(".TOP",".C1")) %>% as.data.frame() %>% nrow()` cases to match.

<br>

Another alternative could be the match by coincing HASHs and years at the time of admission. However, we require to have valid dates of birth in the TOP datasetm to calculate years at a given time. There were `r CONS_TOP_df_dup_ENE_2020_prev1 %>% dplyr::filter(!is.na(fech_nac)) %>% dplyr::select(fech_nac) %>% dplyr::filter(fech_nac<"1929-03-19"|fech_nac>"2001-11-01")  %>% nrow()` dates of birth that might represent invalid values along the dataset, depending on contextual factors (such as the time in which the user was interviewed). That is why we decided to **get the age at the time of application of the TOP as a valid filter of valid dates of birth**. This criterion aims to bring consistency over both datasets in terms of admitted ages into programs. In the following table, these cases are presented in depth. 
...
An important amount of cases were available in the TOP dataset, permitting to replace invalid ages with the right information. Must note that `r edad_adolescentes_17` cases had around 17 years of the `r edad_adolescentes_menores_18` cases that were not adults at the time of the application of TOP.  The next table presents those HASHs that had other registries with a valid age.
...
As seen in Table 6, only `r dates_replace_invalid_value` `r ifelse(dates_replace_invalid_value>1,"values","value")` could be replaced. We replaced SENDAs ID, age, ID and date of birth to `r ifelse(dates_replace_invalid_value>1,"to those cases","to this case")`, to get a much cleaner dataset of valid cases. 

<div class = "blue">
**One observation that come from these transformations, is that many of TOPs applications that had missing values in one value, had missing values in many other more. Possibly, there were duplicated events, and some of them were invalid. If this is true, we only need to discard events with incomplete data instead of replacing them with valid information. But first we need to check whether this happens frequently or not. This have to be contrasted in the next stages of data preparation**.
</div>
...
#Join datasets 
    dplyr::mutate(OBS=ifelse((Edad_at_ap<18 & !is.na(Edad_at_ap.corr_age_adm_top))|(Edad_at_ap>90 & !is.na(Edad_at_ap.corr_age_adm_top)),glue::glue("{OBS};1.3.Replaced invalid age at TOP application"),OBS))%>% 
    dplyr::mutate(OBS=ifelse(Edad_at_ap<18|Edad_at_ap>90,glue::glue("{OBS};1.3.Invalid Age at the Time of Application of TOP"),OBS))%>% 
...
### 3. Inconsistencies between SENDAs ID and HASH Key
...
The official individual IDs (RUT) were masked into HASH keys by an informatics professional (For more information about this process, visit the [encryption phase](https://fondecytacc.github.io/SUD_health_Chile.github.io/Encript.html)). We checked whether they were consistent with each SENDAs ID and do not depend on other factors in their identification (eg., individual ID was not well processed), by searching for more than one HASH Key in each SENDAs ID. There were `r nrow(ids_more_one_hash_TOP)` IDs that had more than one HASH key. These `r nrow(ids_more_one_hash_TOP)` IDs affect 183 registries.
...
**These inconsistent HASHs have been sent to SENDAs professional, who made clear that these cases (every one of them also present in C1 dataset) corresponded to users with different RUTs, so their HASHs should be different**.
...
One of them is the date of application of the TOP questionnaire. As can be seen in Table 9, `r prop.table(table(duplicated(CONS_TOP_df_dup_ENE_2020_prev2[,c("HASH_KEY","fech_ap_top")])))[1] %>% round(3)*100`% of the registries are unique combinations of date of application of the TOP questionnaire and HASH key.
...
- SENDAs ID (ID) : The HASH Key identifier was the one recommended by SENDAs Professionals.
- SENDAs ID (masked characters 5 & 6) (id_mod): The HASH Key identifier was the one recommended by SENDAs Professionals.
- Year of the Dataset (Source) (ano_bd) (TABLE): Let us detect cases that repeat among different yearly datasets.
- Events in the Dataset/ Row Number (row): We excluded this identifier because it is a unique number of each event that we arbitrarely generated.
...
- Age at the Time of Admission (Edad_al_ing): Not included because depends mainly on already present variables (Date of Birth and Date of Admission)
- Age at the Time of Application (Edad_at_ap): Not included because depends mainly on already present variables (Date of Birth and Date of Application of TOP)
...
dplyr::mutate(OBS=ifelse(dup_todo_TOP>1,glue::glue("{OBS};1.4.Had a duplicated events with almost every variable in common"),OBS))%>% 
...
as.data.table(CONS_TOP_df_dup_ENE_2020_prev4)[, dup_contents_TOP := .N, by = names_top2] %>%
  dplyr::mutate(OBS=ifelse(dup_contents_TOP>1,glue::glue("{OBS};1.5.Had a duplicated event with relevant variables in common"),OBS))%>% 
  data.table::as.data.table() %>%
...
As seen in Table 14, a small fraction of the combination of dates and HASHs was not present in C1 Dataset. Many of them due to missing variables in the date of admission. There are `r n_cases_hash1` cases of `r n_cases_hash2` distinct HASHs that were not included in C1 dataset.
...
CONS_TOP_df_dup_ENE_2020_prev6 %>%
  dplyr::mutate(OBS=ifelse(Plan.de.Tratamiento %in% c("M-PAI2","M-PR2","PG PAI 2","Otro","CALLE"),paste0(OBS,";","1.6. Collapsed Treatment Plans"),OBS))%>%
...
CONS_C1_df_dup_ENE_2020_unique_gender<- dplyr::select(CONS_C1_df_dup_ENE_2020,hash_key, identidad_de_genero) %>% dplyr::filter(!is.na(identidad_de_genero)) %>% group_by(hash_key) %>% dplyr::mutate(row_join=row_number()) %>%ungroup() %>% dplyr::filter(row_join==1) %>% data.table::as.data.table(.)

CONS_TOP_df_dup_ENE_2020_prev7 %>%
    dplyr::left_join(CONS_C1_df_dup_ENE_2020_unique_gender, by=c("HASH_KEY"="hash_key")) %>%
    dplyr::mutate(OBS= ifelse(identidad_de_genero=="Masculino" & Sexo=="Hombre" & tipo_de_plan %in% c("M-PAI", "M-PR", "M-PAB"), paste0(OBS,";","1.7. Standardized Plans By Sex & Gender Id"),OBS))%>%
...
#    data.table::as.data.table() %>%
#  print()
  dplyr::select(-dup_todo_TOP,-dup_contents_TOP,-row_join) %>%
  as.data.frame() %>%
...
CONS_TOP_df_dup_ENE_2020_prev8 %>% 
  ...
  dplyr::mutate(sus_prin1= dplyr::recode(sus_prin1,
                                              ...
                                              "Hongos"="Alucinógenos")) %>%
dplyr::mutate(OBS=ifelse(sus_prin1=="SIN CONSUMO",paste0(OBS,";","1.8.Primary Substance1, Invalid due to No Consumption"),OBS))%>%    
dplyr::mutate(sus_prin1= dplyr::na_if(sus_prin1, "SIN CONSUMO")) %>%
  ...
  dplyr::mutate(sus_prin2= dplyr::recode(sus_prin2,
                                              ...
                                              "Esteroides Anabólicos"="Otros",
                                              "Hongos"="Alucinógenos")) %>%
dplyr::mutate(OBS=ifelse(sus_prin2=="SIN CONSUMO",paste0(OBS,";","1.8.Primary Substance2, Invalid due to No Consumption"),OBS))%>%
dplyr::mutate(sus_prin2= dplyr::na_if(sus_prin2, "SIN CONSUMO")) %>%
  ...
  dplyr::mutate(sus_prin3= dplyr::recode(sus_prin3,
                                              ...
                                              "Esteroides Anabólicos"="Otros",
                                              "Hongos"="Alucinógenos")) %>%
dplyr::mutate(OBS=ifelse(sus_prin3=="SIN CONSUMO",paste0(OBS,";","1.8.Primary Substance3, Invalid due to No Consumption"),OBS))%>%
