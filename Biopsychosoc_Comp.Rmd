...
rm(list=setdiff(ls(), c("CONS_C1_df_dup_JUN_2020", "CONS_TOP_df_dup_ENE_2020_prev9")))
...
CONS_C1_df_dup_JUN_2020_match_top<-
  CONS_C1_df_dup_JUN_2020%>%
...
Of the total entries in TOP dataset (n= `r nrow(CONS_C1_df_dup_JUN_2020) %>% formatC(, format="f", big.mark=",", digits=0)`), we did not considered 2,845 entries in TOP dataset had more than one application in the same date (`r scales::percent(2845/nrow(CONS_C1_df_dup_JUN_2020))`). We selected `r nrow(CONS_C1_df_dup_JUN_2020_match_top) %>% formatC(, format="f", big.mark=",", digits=0)` cases that had no differences between the date of application and the date of admission to treatment. Additionally, these applications must had been administered at admission (excluding the 0,6% of applications that were administered at other stage of treatment). Finally, we discarded cases with null values in the variables of interest (`r 6948-6718` entries). 
...
Furthermore, we were interested in co-occurrent physical and psychiatric conditions among substance users. Consequently, we generated a variable called  `dg_trs_psiq`, composed of users without a diagnosed psychiatric condition (0), in study (1), and those diagnosed with a psychiatric condition (2). Also, we included `dg_trs_fisico`, which categorizes between users without a diagnosed physical condition (0), in study (1), and those diagnosed with a physical condition (2).
...
CONS_C1_df_dup_JUN_2020_match_top%>%
  dplyr::select(compromiso_biopsicosocial,total_oh,dosis_oh, total_thc, dosis_thc, total_pbc, dosis_pbc, total_coc, dosis_coc, total_bzd, dosis_bzd, total_otra, dosis_otra, total_vif,  total_transgresion, salud_psicologica, total_trabajo, total_educacion, salud_fisica, calidad_vida,  hurto, robo,  venta_drogas, rina,  otro, lugar_vivir, vivienda,origen_ingreso,motivodeegreso_mod,hash_key,dg_trs_psiq_cie_10_or,diagnostico_trs_fisico) %>%
   #dplyr::mutate(compromiso_biopsicosocial=as.numeric(compromiso_biopsicosocial))%>%
    ...
    dplyr::mutate(abandono_temp=dplyr::case_when(motivodeegreso_mod=="Abandono Temprano"~1,TRUE~0),
                  alta_adm= dplyr::case_when(grepl("trativa",as.character(motivodeegreso_mod))~1,TRUE~0))%>%
    ...
    dplyr::mutate(dg_trs_fisico= replace_na(diagnostico_trs_fisico, 0))%>%
    dplyr::mutate(dg_trs_fisico= dplyr::case_when(grepl("En estudio",as.character(diagnostico_trs_fisico))~1,
                                                  ...
                                                  TRUE~2))%>%
    dplyr::mutate(dg_trs_psiq= replace_na(dg_trs_psiq_cie_10_or, 0))%>%
    dplyr::mutate(dg_trs_psiq= dplyr::case_when(grepl("En estudio",as.character(dg_trs_psiq))~1,
                                                  grepl("Sin trastorno",as.character(dg_trs_psiq))~0,    
                                                  TRUE~2))%>%
    dplyr::mutate(dg_trs_fisico=as.factor(dg_trs_fisico))%>%
    dplyr::mutate(dg_trs_psiq=as.factor(dg_trs_psiq))%>%
    dplyr::mutate(dg_trs_psiq_cie_10_or=as.factor(dg_trs_psiq_cie_10_or))%>%
    dplyr::select(-origen_ingreso)%>%
    dplyr::mutate(salud_mean_z=scale2(salud_mean))%>%
    dplyr::mutate(dosis_mean_z=scale2(dosis_mean,na.rm=T))%>%
    dplyr::mutate(total_mean_z=scale2(total_mean))%>%
    dplyr::mutate(vivienda_mean_z=scale2(vivienda_mean))%>%
    dplyr::mutate(total_vif_z=scale2(total_vif))%>%
    ...
    dplyr::mutate(mayor_comp_biopsicsoc=dplyr::case_when(compromiso_biopsicosocial=="Severo"~1,TRUE~0))%>%
    dplyr::mutate(mayor_comp_biopsicsoc=as.logical(mayor_comp_biopsicsoc))%>%
    dplyr::mutate(menor_comp_biopsicsoc=dplyr::case_when(compromiso_biopsicosocial=="Leve"~1,TRUE~0))%>%
  ...
  dplyr::filter(!is.na(dosis_mean))%>%
  dplyr::select(compromiso_biopsicosocial,total_oh,dosis_oh, total_thc, dosis_thc, total_pbc, dosis_pbc, total_coc, dosis_coc, total_bzd, dosis_bzd, total_otra, dosis_otra, total_vif,  total_transgresion, salud_psicologica, total_trabajo, total_educacion, salud_fisica, calidad_vida,  hurto, robo,  venta_drogas, rina,  otro, lugar_vivir, vivienda,dg_trs_psiq,dg_trs_fisico,everything())

skimr::skim(CONS_C1_df_dup_JUN_2020_match_top_sel%>% dplyr::select(-hash_key))%>%
  tibble::as_tibble() %>%
...
foreign::write.dta(CONS_C1_df_dup_JUN_2020_match_top_sel, "G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/c1_top.dta")

#CONS_C1_df_dup_JUN_2020 CONS_C1_df_dup_JUN_2020_match_top CONS_TOP_df_dup_ENE_2020_prev9_match CONS_TOP_df_dup_ENE_2020_prev9
#sólo match
comb_datasets_a_n<-CONS_C1_df_dup_JUN_2020%>%
    dplyr::mutate(concat=paste0(hash_key,"_",as.character(fech_ing)))%>%
    dplyr::left_join(CONS_TOP_df_dup_ENE_2020_prev9_match,by="concat", suffix=c("","_TOP"))%>%
    dplyr::filter(!is.na(hash_key_TOP))%>%
    dplyr::select(-hash_key_TOP)%>% 
    nrow()
comb_datasets_a_users<-CONS_C1_df_dup_JUN_2020%>%
    dplyr::mutate(concat=paste0(hash_key,"_",as.character(fech_ing)))%>%
    dplyr::left_join(CONS_TOP_df_dup_ENE_2020_prev9_match,by="concat", suffix=c("","_TOP"))%>%
    dplyr::filter(!is.na(hash_key_TOP))%>%
    dplyr::select(-hash_key_TOP)%>% 
...
comb_datasets_b_n<-CONS_C1_df_dup_JUN_2020%>%
    dplyr::mutate(concat=paste0(hash_key,"_",as.character(fech_ing)))%>%
    dplyr::left_join(CONS_TOP_df_dup_ENE_2020_prev9_match,by="concat", suffix=c("","_TOP"))%>%
    dplyr::filter(!is.na(compromiso_biopsicosocial), !is.na(hash_key_TOP))%>%
    dplyr::select(-hash_key_TOP)%>% 
    nrow()
comb_datasets_b_users<-CONS_C1_df_dup_JUN_2020%>%
    dplyr::mutate(concat=paste0(hash_key,"_",as.character(fech_ing)))%>%
    dplyr::left_join(CONS_TOP_df_dup_ENE_2020_prev9_match,by="concat", suffix=c("","_TOP"))%>%
    dplyr::filter(!is.na(compromiso_biopsicosocial), !is.na(hash_key_TOP))%>%
    dplyr::select(-hash_key_TOP)%>% 
...
```{r fig2, fig.width = 7, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 2. Biopsychosocial Compromise", error=T, eval=T}
CONS_C1_df_dup_JUN_2020_match_top_sel %>%
     dplyr::group_by(compromiso_biopsicosocial)%>%
     dplyr::mutate(n=n(),percent=round(n/sum(n),2))%>% 
      dplyr::mutate(compromiso_biopsicosocial=case_when(compromiso_biopsicosocial=="Moderado"~"Moderate",
                                                        compromiso_biopsicosocial=="Leve"~"Mild",
                                                        TRUE~"Severe"))%>%
     dplyr::ungroup()%>%
 ggplot(aes(x=1,y=compromiso_biopsicosocial,fill=compromiso_biopsicosocial))+
     geom_col(width=1)+
     scale_fill_manual(name ="Biopsychosocial Compromise",values = c("Severe" = "coral2", "Moderate" = "darkorange","Mild"= "darkolivegreen")) +
     coord_polar(theta="y",start=1.2)+
     #geom_text(aes(label=percent, group=compromiso_biopsicosocial),position= position_stack(vjust=.5))+
...
mix_cor<- psych::mixedCor(data=CONS_C1_df_dup_JUN_2020_match_top_sel%>%dplyr::mutate(compromiso_biopsicosocial=as.numeric(compromiso_biopsicosocial)),c=2:20, #total_oh,dosis_oh total_thc dosis_thc total_pbc dosis_pbc total_coc dosis_coc total_bzd dosis_bzd  total_otra dosis_otra total_vif  total_transgresion salud_psicologica total_trabajo total_educacion salud_fisica calidad_vida 
...
#mixedCor(data=CONS_C1_df_dup_JUN_2020_match_top_sel,c=c("total_oh","dosis_oh", "total_thc", "dosis_thc", "total_pbc", "dosis_pbc", "total_coc", "dosis_coc", "total_bzd", "dosis_bzd", "total_otra", "dosis_otra", "total_vif",  "total_transgresion", "salud_psicologica", "total_trabajo", "total_educacion", "salud_fisica", "calidad_vida"), p="compromiso_biopsicosocial",d=c("hurto", "robo",  "venta_drogas", "rina",  "otro", "lugar_vivir", "vivienda"), smooth=TRUE,correct=.5,global=TRUE,ncat=8,use="pairwise",method="pearson",weight=NULL)

#subscript out of bounds
#set.seed(123)
#par(mar=c(1,1,1,1))
#comp_biopsicosoc_mat_ci<-mycor.ci(x=CONS_C1_df_dup_JUN_2020_match_top_sel%>%dplyr::mutate(compromiso_biopsicosocial=as.numeric(compromiso_biopsicoso#cial)), 
#                                  method="spearman", 
#                                  poly=TRUE, 
#                                  cvars=2:20, 
#                                  pvars=1,
...
Also we included an heterogeneus matrix of correlations, consisting of polychoric, biserial and pearson correlations [@Fox2019]. We mostly selected transformed variables in the following matrix.
...
hetcor_mat<-hetcor(CONS_C1_df_dup_JUN_2020_match_top_sel[c("compromiso_biopsicosocial","lugar_vivir","vivienda","abandono_temp","alta_adm","transg_norm","salud_mean","dosis_mean","total_mean","trabajo_edu_mean","vivienda_mean","polycons", "menor_salud", "menor_trabajo_edu", "mayor_dosis", "mayor_total","transg_norm_bin")], ML = T, std.err = T, use="complete.obs", bins=4, pd=TRUE)
#CONS_C1_df_dup_JUN_2020_match_top_sel%>% select(contains("dosis"))%>% hist()
...
ocdMelt <- melt(CONS_C1_df_dup_JUN_2020_match_top_sel[c('compromiso_biopsicosocial','hurto',"robo","venta_drogas","rina","otro","lugar_vivir","vivienda","polycons","vif","transg_norm_bin")], id=c('compromiso_biopsicosocial'))
...
# plot
ggplot(ocdMelt, aes(Outcome_Measure,perc))+
...
library(ggplot2)
# First we need to restructure the data into long format:
ocdMelt <- melt(CONS_C1_df_dup_JUN_2020_match_top_sel[c('compromiso_biopsicosocial',"salud_mean_z","dosis_mean_z", "total_mean_z","total_vif_z")], id=c('compromiso_biopsicosocial'))
names(ocdMelt) <- c('Group', 'Outcome_Measure', 'Frequency')
# plot
...
mvn_test<-MVN::mvn(CONS_C1_df_dup_JUN_2020_match_top_sel[c("salud_mean_z","total_mean_z","dosis_mean_z","total_vif_z")], mvnTest = "mardia", covariance = TRUE, tol = 1e-25, alpha = 0.5,scale = T, desc = TRUE, transform = "none", R = 1000,
...
mvn_test$multivariateNormality%>%
  ...
  knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 2. Multivariate Normality Test",
                 align =rep('c', 101),
               col.names=c("Test", "Statistic", "P Value", "Normality"))%>%
   kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
  kableExtra::add_footnote( c("Note. Using z-scores distributions of Average Health, Average  and "), notation = "none") %>%
...
dependent.vars <- cbind(CONS_C1_df_dup_JUN_2020_match_top_sel$salud_mean_z,CONS_C1_df_dup_JUN_2020_match_top_sel$dosis_mean_z, CONS_C1_df_dup_JUN_2020_match_top_sel$total_mean_z)
root.manova <- manova(dependent.vars ~ CONS_C1_df_dup_JUN_2020_match_top_sel$compromiso_biopsicosocial, intercept=F)
...
CONS_C1_df_dup_JUN_2020_match_top_sel2<-
  CONS_C1_df_dup_JUN_2020_match_top_sel%>%
...
invisible(
prueba_anormalidad1<-npmv::nonpartest(dosis_mean_z|total_mean_z|salud_mean_z|hurto|robo|venta_drogas|rina|otro|lugar_vivir|vivienda|total_vif_z~compromiso_biopsicosocial, CONS_C1_df_dup_JUN_2020_match_top_sel2,plots=F,permreps=5000,releffects=TRUE)
)
invisible(
prueba_anormalidad2<-npmv::nonpartest(dosis_mean_z|total_mean_z|salud_mean_z|hurto|robo|venta_drogas|rina|otro|lugar_vivir|vivienda|total_vif_z~compromiso_biopsicosocial+origen_ingreso_mod, CONS_C1_df_dup_JUN_2020_match_top_sel2,plots=F,permreps=5000,releffects=TRUE)
)
invisible(
prueba_anormalidad3<-npmv::nonpartest(dosis_mean_z|total_mean_z|salud_mean_z|hurto|robo|venta_drogas|rina|otro|lugar_vivir|vivienda|total_vif_z~compromiso_biopsicosocial+origen_ingreso_mod+dg_trs_fisico, CONS_C1_df_dup_JUN_2020_match_top_sel2,plots=F,permreps=5000,releffects=TRUE)
)
invisible(
prueba_anormalidad4<-npmv::nonpartest(dosis_mean_z|total_mean_z|salud_mean_z|hurto|robo|venta_drogas|rina|otro|lugar_vivir|vivienda|total_vif_z~ compromiso_biopsicosocial+origen_ingreso_mod+dg_trs_fisico+dg_trs_psiq, CONS_C1_df_dup_JUN_2020_match_top_sel2,plots=F,permreps=5000,releffects=TRUE)
)
#
...
#invisible(
#npmv::ssnonpartest(dosis_mean|total_mean|trabajo_edu_mean~compromiso_biopsicosocial,CONS_C1_df_dup_JUN_2020_match_top_sel,test=c(1,0,0,0),alpha=.05,factors.and.variables=TRUE)
#)
```
...
```{r nonpar_vegan1, dpi = 96, message=F, error=F}
...
vegan_1<- 
  vegan::adonis(CONS_C1_df_dup_JUN_2020_match_top_sel2[,c("dosis_mean_z", "total_mean_z", "salud_mean_z", "hurto", "robo", "venta_drogas", "rina", "otro", "lugar_vivir", "vivienda", "total_vif_z"),drop = FALSE] ~ compromiso_biopsicosocial, data=CONS_C1_df_dup_JUN_2020_match_top_sel,permutations=1000, parallel = 4)

vegan_2<- 
  vegan::adonis(CONS_C1_df_dup_JUN_2020_match_top_sel2[,c("dosis_mean_z", "total_mean_z", "salud_mean_z", "hurto", "robo", "venta_drogas", "rina", "otro", "lugar_vivir", "vivienda", "total_vif_z"),drop = FALSE] ~ compromiso_biopsicosocial+origen_ingreso_mod, data=CONS_C1_df_dup_JUN_2020_match_top_sel,permutations=1000, parallel = 4)

vegan_3<- 
  vegan::adonis(CONS_C1_df_dup_JUN_2020_match_top_sel2[,c("dosis_mean_z", "total_mean_z", "salud_mean_z", "hurto", "robo", "venta_drogas", "rina", "otro", "lugar_vivir", "vivienda", "total_vif_z"),drop = FALSE] ~ compromiso_biopsicosocial+origen_ingreso_mod+dg_trs_fisico, data=CONS_C1_df_dup_JUN_2020_match_top_sel, permutations=1000, parallel = 4)

vegan_4<- 
  vegan::adonis(CONS_C1_df_dup_JUN_2020_match_top_sel2[,c("dosis_mean_z", "total_mean_z", "salud_mean_z", "hurto", "robo", "venta_drogas", "rina", "otro", "lugar_vivir", "vivienda", "total_vif_z"), drop = FALSE] ~ compromiso_biopsicosocial+origen_ingreso_mod+dg_trs_fisico+dg_trs_psiq,  data=CONS_C1_df_dup_JUN_2020_match_top_sel, permutations=1000, parallel = 4)
...
res <- glmulti(abandono_temp ~ compromiso_biopsicosocial + origen_ingreso_mod + dg_trs_psiq + dg_trs_fisico, 
               data=CONS_C1_df_dup_JUN_2020_match_top_sel2,
...
We selected the model with lower AIC. Of them, the first selected did not contain biopsychosocial compromise (AICc= `r res@objects[[1]]$aic`). The second contained biopsychosocial compromise as a valid terms in the model (AICc= `r res@objects[[2]]$aic`).
...
```{r fig9_int2,eval=T, echo=T, fig.align='center', paged.print=TRUE,fig.cap= "Figure 9. Interactive Effects in the Prob. of Early Drop-out"}
library(lsmeans)
refR_1 <- lsmeans(res@objects[[2]], specs = c("compromiso_biopsicosocial","dg_trs_psiq"))
ref_dfR_1 <- as.data.frame(cbind(summary(refR_1)[1:2],exp(summary(refR_1)[,c(3,6,7)])))

g4R_1 <- ggplot(ref_dfR_1, aes(x=compromiso_biopsicosocial, y=lsmean,group=dg_trs_psiq, colour=dg_trs_psiq))+
...
```{r fig10_int1,eval=T, echo=T, paged.print=TRUE,fig.align='center', fig.cap= "Figure 10. Interactive Effects in the Prob. of Early Drop-out"}
library(lsmeans)
refR_2 <- lsmeans(res@objects[[2]], specs = c("dg_trs_psiq","origen_ingreso_mod"))
ref_dfR_2 <- as.data.frame(cbind(summary(refR_2)[1:2],exp(summary(refR_2)[,c(3,6,7)])))

g4R_2 <- ggplot(ref_dfR_2, aes(x=dg_trs_psiq, y=lsmean,group=origen_ingreso_mod, colour=origen_ingreso_mod))+
geom_errorbar(aes(ymin=asymp.LCL, ymax=asymp.UCL), width=.1,position=position_dodge(0.1), size=1) +
geom_line(position=position_dodge(0.1), size=.5)+
geom_point(position=position_dodge(0.1), size=2)+
...
```{r fig11_int1,eval=T, echo=T, fig.align='center', paged.print=TRUE,fig.cap= "Figure 11. Interactive Effects in the Prob. of Early Drop-out"}
library(lsmeans)
refR_3 <- lsmeans(res@objects[[2]], specs = c("dg_trs_psiq","dg_trs_fisico"))
ref_dfR_3 <- as.data.frame(cbind(summary(refR_3)[1:2],exp(summary(refR_3)[,c(3,6,7)])))

g4R_3 <- ggplot(ref_dfR_3, aes(x=dg_trs_psiq, y=lsmean,group=dg_trs_fisico, colour=dg_trs_fisico))+
geom_errorbar(aes(ymin=asymp.LCL, ymax=asymp.UCL), width=.1,position=position_dodge(0.1), size=1) +
geom_line(position=position_dodge(0.1), size=.5)+
geom_point(position=position_dodge(0.1), size=2)+
