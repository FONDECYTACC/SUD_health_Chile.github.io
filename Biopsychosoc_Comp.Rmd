...
CONS_C1_df_dup_JUL_2020_match_top%>%
  dplyr::select(compromiso_biopsicosocial,total_oh, dosis_oh, total_thc, dosis_thc, total_pbc, dosis_pbc, total_coc, dosis_coc, total_bzd, dosis_bzd, total_otra, dosis_otra, total_vif,  total_transgresion, salud_psicologica, total_trabajo, total_educacion, salud_fisica, calidad_vida,  hurto, robo,  venta_drogas, rina,  otro, lugar_vivir, vivienda,origen_ingreso_mod,motivodeegreso_mod_imp,hash_key,dg_trs_cons_sus_or,diagnostico_trs_fisico,dg_trs_cons_sus_or,otros_probl_at_sm_or,sus_principal_mod,otras_sus1_mod,otras_sus2_mod,otras_sus3_mod,dg_trs_psiq_cie_10_or,x2_dg_trs_psiq_cie_10_or,x3_dg_trs_psiq_cie_10_or,x4_dg_trs_psiq_cie_10_or,x5_dg_trs_psiq_cie_10_or,x6_dg_trs_psiq_cie_10_or)%>%
   #dplyr::mutate(compromiso_biopsicosocial=as.numeric(compromiso_biopsicosocial))%>%
    dplyr::mutate(compromiso_biopsicosocial=dplyr::case_when(as.character(compromiso_biopsicosocial)== ""~"1-Leve",
                                                           as.character(compromiso_biopsicosocial)== "2"~"2-Moderado",
                                                           as.character(compromiso_biopsicosocial)== "Severo"~"3-Severo",TRUE~NA_character_))%>%  
    ...
    dplyr::mutate(dg_cons_dep= dplyr::case_when(grepl("Dependencia",as.character(dg_trs_cons_sus_or))~1,
                                                  TRUE~0))%>%
    dplyr::mutate(sin_otros_prob_sm= dplyr::case_when(grepl("Sin otros problemas de salud mental",as.character(otros_probl_at_sm_or))~1,
                                                  TRUE~0))%>%
  dplyr::mutate(across(c(dg_trs_psiq_cie_10_or,x2_dg_trs_psiq_cie_10_or,x3_dg_trs_psiq_cie_10_or,x4_dg_trs_psiq_cie_10_or,x5_dg_trs_psiq_cie_10_or,x6_dg_trs_psiq_cie_10_or),~dplyr::case_when(grepl("En estudio",as.character(.))~1,grepl("Sin trastorno",as.character(.))~0,is.na(.)~0,TRUE~2)))%>%
    dplyr::mutate(total_cie_10 = base::rowSums(dplyr::select(., dg_trs_psiq_cie_10_or, x2_dg_trs_psiq_cie_10_or, x3_dg_trs_psiq_cie_10_or, x4_dg_trs_psiq_cie_10_or, x5_dg_trs_psiq_cie_10_or, x6_dg_trs_psiq_cie_10_or)))%>%
    dplyr::mutate(dg_cie_10=ifelse(total_cie_10>0,1,0))%>%
    dplyr::mutate(dg_cie_10=as.factor(dg_cie_10))%>%
  ...
  dplyr::select(compromiso_biopsicosocial,total_oh,dosis_oh, total_thc, dosis_thc, total_pbc, dosis_pbc, total_coc, dosis_coc, total_bzd, dosis_bzd, total_otra, dosis_otra, total_vif,  total_transgresion, salud_psicologica, total_trabajo, total_educacion, salud_fisica, calidad_vida,  hurto, robo,  venta_drogas, rina,  otro, lugar_vivir, vivienda, dg_cie_10, dg_trs_fisico,everything())
