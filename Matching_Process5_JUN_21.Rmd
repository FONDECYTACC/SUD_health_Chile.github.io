...
Once set as constant those terms that had no time-varying effects, we found that treatment modality at baseline had a hazard ratio of `r paste0(round(exp(0.1410),3),"[",
round(exp(0.0522),3),"-", round(exp(0.23000),3),"]")`.
...
```{r plot_td, echo=T, error=T, paged.print=TRUE, fig.height=9, error=T, dpi=500, fig.cap="Figure 1. Observed cumulative coefficients with 95% confidence bands"}
...
```{r long_alluv, echo=T, error=T, paged.print=TRUE, fig.cap="Figure 3. Sankey Plot of Transitions by Treatment Modality", fig.width=12, fig.height=8}
...
```{r alluvial3, echo=T, error=T, paged.print=TRUE, fig.height=9, error=T, dpi=500, fig.cap="Figure 4. Sankey Plot of Transitions by Treatment Modality"}
library(ggrepel)
library(ggalluvial)
