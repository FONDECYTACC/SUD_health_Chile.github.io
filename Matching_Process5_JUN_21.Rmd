...
```{r prueba, echo=T, error=T, paged.print=TRUE, fig.height=9, error=T, dpi=500}

library(reReg)
data("readmission")
#dplyr::glimpse(readmission)

reReg(Recur(Tstart, id, status, Tstop)~tipo_de_plan_res_1, data= ms_d_match_surv)
summary(reReg(Recur("Tstart", "id", "status", "Tstop")~tipo_de_plan_res_1), data= ms_d_match_surv, method="HW")
```
...
comb_subset<-
expand.grid(tipo_de_plan_res_1=c(0,1),tipo_de_plan_res_2=c(0,1,NA),tipo_de_plan_res_3=c(0,1,NA),tipo_de_plan_res_4=c(0,1,NA)) %>% tidyr::unite_("esp",paste0("tipo_de_plan_res_",1:4))

# d_match_surv_msprep %>% 
#   tidyr::unite_("esp",paste0("tipo_de_plan_res_",1:4)) %>% 
#   dplyr::(comb_subset)
