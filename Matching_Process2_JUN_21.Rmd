...
ggarrange(msf0_plot,msf1_plot,
 ...
 if(no_mostrar==1){
jpeg(paste0(gsub("SUD_CL/Matching_Process_APR_21.Rmd","",path),"_mult_state_ags/Cum_haz_markov_set_jun.jpg"), height=14, width= 12, res= 320, units = "in")
...
fig_cox_abcd_lab<-
annotate_figure(fig_cox_abcd, left = textGrob("Cumulative hazards", rot = 90, vjust = 1, gp = gpar(cex = 1.1)),
                    bottom = textGrob("Years", gp = gpar(cex = 1.1)))

fig_cox_abcd_lab


if(no_mostrar==1){
ggsave(paste0(dta_path,"Fig2_alt.jpg"), fig_cox_abcd_three_states_lab, width = 10, height = 11.5, dpi = 600, units= "in")
ggsave(paste0(dta_path,"Fig2_alt.eps"), fig_cox_abcd_three_states_lab, width = 10, height = 11.5, dpi = 600, units= "in")
ggsave(paste0(dta_path,"Fig2_alt.ps"), fig_cox_abcd_three_states_lab, width = 10, height = 11.5, dpi = 600, units= "in")
ggsave(paste0(dta_path,"Fig2_alt.pdf"), fig_cox_abcd_three_states_lab, width = 10, height = 11.5, dpi = 600, units= "in")
}
```
...
if(no_mostrar==1){
jpeg(paste0(gsub("SUD_CL/Matching_Process_APR_21.Rmd","",path),"_mult_state_ags/Cum_haz_ten_st_jun.jpg"), height=20, width= 10, res= 320, units = "in")
...
```{r lmaj_plot_ci952, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 4b. State Occupancies & 95% CIs, landmark Aalen-Johansen estimator from Admission (state 1)", fig.align="center", error=T}
lmaj_ci_df2 %>% 
  ggplot(aes(time, Estimate, color= setting, linetype =setting, fill=setting))+
  geom_step(size=1, alpha=.65) + 
  geom_ribbon(aes(ymin=l95ci, ymax=u95ci, fill=setting), alpha=.5)+
...
```{r probtrans_plot, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 5. Estimate of State Occupancies at Baseline, Ten-states model (Markov)", fig.align="center", error=T}
...
fig_ab_pmatrix_probtrans<-
  ...
  ggplot(ab_pmatrix_probtrans, aes(time, value, color= factor(state_to), linetype =setting))+
    geom_step(size=1, alpha=.65) + 
    scale_y_continuous(labels = scales::percent_format(accuracy = 2))+
    facet_wrap(start~., ncol=3, scales="free_y", labeller = as_labeller(states_trans_lab)) + 
    #facet_wrap(state~., ncol=1, scales="free_y") + #type_of_program
...
if(no_mostrar==1){
jpeg(paste0(gsub("SUD_CL/Matching_Process_APR_21.Rmd","",path),"_mult_state_ags/probtrans_states_jun.jpg"), height=20, width= 10, res= 320, units = "in")
fig_ab_pmatrix_probtrans
dev.off()
}
```
...
P12_clus_3 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
             h=1, j=2, s=365*3, B=1000, LMAJ=T)
 
P23_clus_3 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
             h=2, j=3, s=365*3, B=1000, LMAJ=T)

P34_clus_3 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
             h=3, j=4, s=365, B=1000, LMAJ=T)

P45_clus_3 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
             h=4, j=5, s=365*3, B=1000, LMAJ=T)

P12_clus_2 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
             h=1, j=2, s=365, B=1000, LMAJ=T)
 
P23_clus_2 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
             h=2, j=3, s=365, B=1000, LMAJ=T)

P34_clus_2 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
             h=3, j=4, s=365, B=1000, LMAJ=T)

P45_clus_2 <- patp_test(data=ms_d_match_surv, tmat=trans_matrix, cid="cid", id="id", group="tipo_de_plan_res_1",
