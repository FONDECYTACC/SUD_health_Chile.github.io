...
if (grepl("CISS Fondecyt",path)==T){
    setwd("C:/Users/CISS Fondecyt/OneDrive/Escritorio/SUD_CL/");load("G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_15_jun.RData")
...
```{r cum_haz12a, eval=T, echo=T, paged.print=TRUE, fig.height=15, fig.width=10, fig.cap="Figure 2a. Estimate of Cumulative Hazards (Semi-Markov), Four-states", fig.align="center", error=T}
...
```{r msfit2.5_4s, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 2b. Estimate of Cumulative Hazards (Semi-Markov), Four-states", fig.align="center", error=T, dpi=320}
...
```{r lmaj_plot2, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 4b. State Occupancies, landmark Aalen-Johansen estimator", fig.align="center", error=T}
#A data frame containing estimates and associated standard errors of the transition probabilities P(X(t)=k | X(s) in from) with s and from the arguments of the function.
...
lmaj_ci_df<-
  rbind(cbind(pr="res",lmaj_res_1,transo=1),
  cbind(pr="out",lmaj_out_1,transo=1)) %>% 
  rbind(cbind(pr="res",lmaj_res_2,transo=2),
  cbind(pr="out",lmaj_out_2,transo=2)) %>% 
  rbind(cbind(pr="res",lmaj_res_3,transo=3),
  cbind(pr="out",lmaj_out_3,transo=3)) %>% 
  #rbind(cbind(pr="res",lmaj_res_4,transo=4),
  #cbind(pr="out",lmaj_out_4,transo=4)) %>% 
...
```{r lmaj_plot_ci95, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.width=15, fig.cap="Figure 4a. State Occupancies & 95% CIs, landmark Aalen-Johansen estimator", fig.align="center", error=T}
...
```{r lmaj_plot_ci952, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.width=15, fig.cap="Figure 4b. State Occupancies & 95% CIs, landmark Aalen-Johansen estimator", fig.align="center", error=T}
lmaj_ci_df_plot2<-
  lmaj_ci_df2 %>% 
    dplyr::mutate(l95ci=dplyr::case_when(se<.1 & Estimate==1~1,T~l95ci),
                u95ci=dplyr::case_when(se<.1 & Estimate==1~1,T~u95ci)) %>%
...
```{r probtrans_plot, eval=T, echo=T, paged.print=TRUE, fig.height=10, fig.width=10, fig.cap="Figure 5. Estimate of State Occupancies at Baseline, Ten-states model (Markov)", fig.align="center", error=T}
...
```{r lmaj_clus_plot, eval=T, echo=T, paged.print=TRUE, fig.height=20, fig.width=10, fig.cap="Figure 7a. Estimate of State Occupancies at Baseline, Ten-states model", fig.align="center", error=T}
...
if (grepl("CISS Fondecyt",rstudioapi::getSourceEditorContext()$path)==T){
    save.image("C:/Users/CISS Fondecyt/OneDrive/Escritorio/SUD_CL/mult_state_2_jun.RData")
