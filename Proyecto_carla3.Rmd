...
compareGroups::export2md(restab22, size=13, first.strip=T, hide.no="no", position="center",
                         ...
                         col.names=c("Variables","Mixed gender", "Women only", "Sig."))%>%
...
Another variable that is worth imputing is the Educational Attainment (n= `r CONS_C1_df_dup_SEP_2020_women_miss %>% dplyr::filter(is.na(escolaridad_rec)) %>% nrow()`). In case of ties, we imputed for the most vulnerable category among the candidates.
...
CONS_C1_df_dup_SEP_2020_women_miss_after_imp_conservados %>% 
    dplyr::select(tipo_de_programa_2, catVars_after_imp) %>% 
    tidyr::gather(variable,measure, -tipo_de_programa_2) %>% 
    group_by(variable) %>%
    do(chisq.test(.$tipo_de_programa_2, .$measure) %>% broom::tidy()) %>%
    dplyr::mutate(p.value=ifelse(p.value<.001,"<0.001",sprintf("%1.3f",p.value))) %>% 
    dplyr::mutate(statistic=sprintf("%2.2f",statistic)) %>% 
    dplyr::mutate(report=paste0("X²(",parameter,", N=",nrow(CONS_C1_df_dup_SEP_2020_women_miss_after_imp_conservados),")=",statistic,ifelse(p.value=="<0.001",p.value, paste0("=",p.value)))) %>% copiar_nombres()
}
...
irrs_tipo_de_programa_2<-irrs(x="tipo_de_programa_2", z="day_to_readmission/1000", y="as.numeric(readmission)-1", db="CONS_C1_df_dup_SEP_2020_women_miss_after_imp_conservados_irr")
...
if(no_mostrar==1){
jpeg(paste0(sub("Proyecto_carla3.Rmd","",path_jpg_1),"pred_haz_4st_int_only.jpg"), height=17, width= 10, res= 320, units = "in")
haz_plot_4s_int_only
