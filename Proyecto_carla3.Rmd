---
title: "Risk among Drug Dependent Women Treated in Women-Only versus Mixed-Gender Programs in Chile* (1st step)"
...
df_pred_cmprsk<-
predict(resCrrRelByDis, matrix(rbind(c(program=1,bmtDisMat[120,2:11]),c(program=0,bmtDisMat[120,2:11])),nrow=2))
...
df_pred_cmprsk2<-
predict(resCrrRelByDis2, matrix(rbind(c(program=1,bmtDisMat[120,2:11]),c(program=0,bmtDisMat[120,2:11])),nrow=2))
...
ggplot(cifd2, aes(years, cif, color=program)) + geom_step()
#We see that the probability of dying while serving in the court is declining over time net of age at appointment. For a justice appointed at age 55, the probability would be 31.6% if appointed in 1950 and 22.0% if appointed in the year 2000, assuming of course that current trends continue, so the model applies.
```
...
legend("bottomleft", legend = c("Kaplan-Meier","Weibull (AFT)", "Weibull (PH)", "Gompertz", "Log-logistic", "Gamma",
         ...
         c("red","coral4","navyblue","lightpink","gray25",#"#A0A36D","#886894",
           "darkorchid4","#496A72","gray70","#A0A36D"), 
...
db_for_apply_rmse_a<-
  cbind.data.frame(
      dist=rep(c("Weibull (AFT)", "Weibull (PH)", "Gompertz", "Log-logistic", "Gamma",
                "Generalized gamma", "Lognormal", "Exponential"),3),
      trans=rep(c(1:3),each=8))
...
ggplot()+
    ...
    #geom_ribbon(aes(ymin=lcl, ymax=ucl), alpha=.5, linetype=0) +
    scale_color_manual(name="Distributions", values =                         c("red","gray20","darkolivegreen","#D3A347","#4F3C91","coral4","#085754","lightpink","cornflowerblue")) +
    scale_fill_manual(name="Distributions", values = c("red","gray20","darkolivegreen","#D3A347","#4F3C91","coral4","#085754","lightpink","cornflowerblue")) +
...
dists_wo_covs_4s <- cbind.data.frame(covs=c(rep("fits_",8)),
              formal=rep(c("Weibull (AFT)", "Weibull (PH)", "Gompertz", "Log-logistic", "Gamma",
                "Generalized gamma", "Lognormal", "Exponential"),1),
              model=rep(c("wei2", "weiph2", "gomp2", "llogis2", "gam2","ggam2", "logn2", "exp2"),1))

dists_w_covs_4s <- cbind.data.frame(covs=c(rep("fits_c_",8)),
              formal=rep(c("Weibull (AFT)", "Weibull (PH)", "Gompertz", "Log-logistic", "Gamma",
                "Generalized gamma", "Lognormal", "Exponential"),1),
              model=rep(c("wei2", "weiph2", "gomp2", "llogis2", "gam2","ggam2", "logn2", "exp2"),1))
...
db_for_apply_rmse_c<-
  cbind.data.frame(
      dist=rep(c("Weibull (AFT)", "Weibull (PH)", "Gompertz", "Log-logistic", "Gamma",
                "Generalized gamma", "Lognormal", "Exponential"),5),
      trans=rep(c(1:5),each=8))
...
db_for_apply_rmse_d<-
  cbind.data.frame(tipo_prog=rep(c("0","1"),each=8,5),
      dist=rep(c("Weibull (AFT)", "Weibull (PH)", "Gompertz", "Log-logistic", "Gamma",
                "Generalized gamma", "Lognormal", "Exponential"),2*5),
      trans=rep(c(1:5),each=8*2))
...
Due to convergence issues, the third transition for the Generalized gamma distribution was defined using initial values of `r  paste0("0.0001 and ", round(mean(subset(ms2_CONS_C1_SEP_2020_women_imputed_exp, trans == 3)[,"time"]),3))`. There was very little to choose between the distributions, but we aimed to choose the distribution that achieve the best balance of a reasonable fit to the observed data and a sensible extrapolation. A Gompertz was chosen for the first transition (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Log-logistic" & trans==1)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Log-logistic" & trans==1)["AIC"]),big.mark=",")`). The distribution for the second transition was Gompertz (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Gompertz" & trans==2)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Gompertz" & trans==2)["AIC"]),big.mark=",")`). For the third and fourth transition, we decided to choose Lognormal because it shows a lower AIC  (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Lognormal" & trans==3)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Lognormal" & trans==3)["AIC"]),big.mark=",")`; Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Lognormal" & trans==4)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Lognormal" & trans==4)["AIC"]),big.mark=",")`). For the fifth transition, we decided to choose  Generalized gamma because it shows a lower AIC  (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Generalized gamma" & trans==5)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Generalized gamma" & trans==5)["AIC"]),big.mark=",")`)
