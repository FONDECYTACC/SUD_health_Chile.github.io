...
For individuals tat had a therapeutic discharge, we ended choosing the Gompertz (AIC= `r fit_flexsurvreg0%>% dplyr::filter(dist=="Gompertz"  & status=="Therapeutic discharge") %>% dplyr::select("AIC") %>% as.numeric() %>% format(big.mark=",")`; RMSE= `r rmse_comp_fits_0 %>% dplyr::filter(dist=="Gompertz") %>% dplyr::select("rmse_td") %>% as.numeric() %>% round(4) %>% format(big.mark=",")`) & Weibull (PH) (AIC= `r fit_flexsurvreg0%>% dplyr::filter(dist=="Weibull (PH)" & status=="Therapeutic discharge") %>% dplyr::select("AIC") %>% as.numeric() %>% format(big.mark=",")`; RMSE= `r rmse_comp_fits_0 %>%  dplyr::filter(dist=="Weibull (PH)") %>% dplyr::select("rmse_td") %>% as.numeric() %>% round(4) %>% format(big.mark=",")`) distributions, because of its lower dissimilarities.

For individuals that had a discharge without clinical advice, we ended choosing the Gompertz (AIC= `r fit_flexsurvreg0%>% dplyr::filter(dist=="Gompertz" & grepl("without",status)) %>% dplyr::select("AIC") %>% as.numeric() %>% format(big.mark=",")`; RMSE= `r rmse_comp_fits_0 %>% dplyr::filter(dist=="Gompertz") %>% dplyr::select("rmse_dwca") %>% as.numeric() %>% round(4) %>% format(big.mark=",")`) & Gamma (AIC= `r fit_flexsurvreg0%>% dplyr::filter(dist=="Gamma" & grepl("without",status)) %>% dplyr::select("AIC") %>% as.numeric() %>% format(big.mark=",")`; RMSE= `r rmse_comp_fits_0 %>%  dplyr::filter(dist=="Gamma") %>% dplyr::select("rmse_dwca") %>% as.numeric() %>% round(4) %>% format(big.mark=",")`) distributions, because of its lower dissimilarities.
...
The table above depicts greater hazards of readmission in women with Therapeutic discharge, compared to women in General population programs `r paste0("(Gompertz HR= ",coefs_surv[1,2]," CI 95% ",coefs_surv[1,3],"); Weibull (PH)= ", coefs_surv[1,6]," CI 95% ",coefs_surv[1,7],")")`. However, when controlling for  covariates, these hazards overlapped with null associations `r paste0("(Gompertz HR= ",coefs_surv[1,4]," CI 95% ",coefs_surv[1,5],"); Weibull (PH)= ", coefs_surv[1,8]," CI 95% ",coefs_surv[1,9],")")`. On the other hand, women with Discharge w/o Clinical Advice had greater hazards of readmission, compared to women in General population programs `r paste0("(Gompertz HR= ",coefs_surv[1,10]," CI 95% ",coefs_surv[1,11],"); Weibull (PH)= ", coefs_surv[1,14]," CI 95% ",coefs_surv[1,15],")")`, even controlling on the rest of the covariates `r paste0("(Gompertz HR= ",coefs_surv[1,12]," CI 95% ",coefs_surv[1,13],"); Weibull (PH)= ", coefs_surv[1,16]," CI 95% ",coefs_surv[1,17],")")`.
...
As seen in the Figure above, women in the General population programs show a lower risk of Discharge without clinical advice specifically in the first year, while women in the the General population programs show a sightly lower incidence of Therapeutic discharge, compared to the Women specific program.

First, the print out shows the results of Gray's test for equality of CIFs across groups. In our example, cumulative incidence curves for types of programs are not statistically different for Therapeutic discharge (`r paste0(round(subset(tests_comp_status_crsk,rn=="Therapeutic discharge")[,"stat"],2),"(",subset(tests_comp_status_crsk,rn=="Therapeutic discharge")[,"df"],"), p= ",sprintf("%.3f",subset(tests_comp_status_crsk,rn=="Therapeutic discharge")[,"pv"]))`), but women in Women specific programs seem to be more likely Discharged without clinical advice than women in the General population (`r paste0(round(subset(tests_comp_status_crsk,grepl("without",rn))[,"stat"],2),"(",subset(tests_comp_status_crsk,grepl("without",rn))[,"df"],"), p= ",sprintf("%.3f",subset(tests_comp_status_crsk,grepl("without",rn))[,"pv"]))`).
...
caso_hipotetico<-rbind(bmtDisMat[120,2:11],bmtDisMat[120,2:11])

caso_hipotetico[1,]<- c(1,1,4,3,4,1,1,3,1,1)
caso_hipotetico[2,]<- c(1,1,4,3,4,1,1,3,1,1)
...
Results were not conclusive. Without covariates, Women specific programs had greater hazards of discharge without clinical advice `r paste0("(HR= ",sprintf("%1.2f",data.table::data.table(summary(resCrrRelByDis_only)$conf.int,keep.rownames=T)[,"exp(coef)"]),"; CI 95% ",sprintf("%1.2f",data.table::data.table(summary(resCrrRelByDis_only)$conf.int,keep.rownames=T)[,"2.5%"]),", ",sprintf("%1.2f",data.table::data.table(summary(resCrrRelByDis_only)$conf.int,keep.rownames=T)[,"97.5%"]),")")`. However, Women specific had lower hazards of discharge without clinical advice when controlling for covariates of interest `r paste0("(HR= ",sprintf("%1.2f",data.table::data.table(summary(resCrrRelByDis)$conf.int,keep.rownames=T)[23,"exp(coef)"]),"; CI 95% ",sprintf("%1.2f",data.table::data.table(summary(resCrrRelByDis)$conf.int,keep.rownames=T)[23,"2.5%"]),", ",sprintf("%1.2f",data.table::data.table(summary(resCrrRelByDis)$conf.int,keep.rownames=T)[23,"97.5%"]),")")`.
...
grid.arrange(
    ggplot(cifd2, aes(years, cif, color=program)) + geom_step()+ggtitle("Therapeutic Discharge")+ sjPlot::theme_sjplot2()+ scale_color_manual(name="Type of program",values=c("red","blue"))+theme(legend.position="none")+ylab("")+ scale_y_continuous(labels = function(x) paste0(x*100, "%"), limits=c(0,.5))+ xlim(0,3),
    ggplot(cifd, aes(years, cif, color=program)) + geom_step()+ggtitle("Discharge without clinical advice")+ sjPlot::theme_sjplot2()+theme(legend.position = "bottom")+ scale_color_manual(name="Type of program",values=c("red","blue"))+ylab("")+ scale_y_continuous(labels = function(x) paste0(x*100, "%"),limits=c(0,.5))+ xlim(0,3),
...
CONS_C1_df_dup_SEP_2020_women_miss_after_imp_conservados_msprep2_exp<-
CONS_C1_df_dup_SEP_2020_women_miss_after_imp_conservados_msprep2 %>% 
  dplyr::rename("id"="row", "td.time"="date_ther_disch", 
                "td.status"="ther_disch", "dwca.time"="date_disch_wo_clin_adv", 
                "dwca.status"="disch_wo_clin_adv",
                "readm.time"="date_post_treat",
                "readm.status"="readmission")

#rio::export(CONS_C1_df_dup_SEP_2020_women_miss_after_imp_conservados_msprep2_exp, "4_st_data.csv")
```
...
```{r markov_semimarkov2,eval=F, echo=T, paged.print=TRUE}                                 

MT <- MarkovTest(ms_CONS_C1_SEP_2020_women_imputed, 
                 id = "row", 
                 formula= "edad_al_ing_grupos+ escolaridad_rec+ sus_principal_mod+ 
         #freq_cons_sus_prin+ compromiso_biopsicosocial+ tenencia_de_la_vivienda_mod+ 
         #num_otras_sus_mod+ numero_de_hijos_mod_rec+ tipo_de_programa_2)+ tipo_de_plan_res",
                 transition = 1,
                 grid = grid, 
                 B = 25)
...
Gompertz (Mean RMSE= `r subset(rmse_comp_fits_2a, dist=="Generalized gamma" & trans==1)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a, dist=="Generalized gamma" & trans==1)["AIC"]),big.mark=",")`; Mean RMSE(2)= `r subset(rmse_comp_fits_2a, dist=="Generalized gamma" & trans==2)["rmse"]`; AIC(2)= `r format(as.numeric(subset(fit_flexsurvreg2a, dist=="Generalized gamma" & trans==2)["AIC"]),big.mark=",")`) and Generalized gamma (Mean RMSE= `r subset(rmse_comp_fits_2a, dist=="Generalized gamma" & trans==1)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a, dist=="Generalized gamma" & trans==1)["AIC"]),big.mark=",")`;Mean RMSE(2)= `r subset(rmse_comp_fits_2a, dist=="Generalized gamma" & trans==2)["rmse"]`; AIC(2)= `r format(as.numeric(subset(fit_flexsurvreg2a, dist=="Generalized gamma" & trans==2)["AIC"]),big.mark=",")`) distributions showed better indices for the first 2 transitions, while Log-logistic showed better indices in the third transition  (Mean RMSE= `r subset(rmse_comp_fits_2a, dist=="Generalized gamma" & trans==3)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a, dist=="Generalized gamma" & trans==3)["AIC"]),big.mark=",")`)
...
There was very little to choose between the distributions, but we aimed to choose the distribution that achieve the best balance of a reasonable fit to the observed data and a sensible extrapolation. A Gompertz was chosen for the first transition (Mean RMSE= `r round(subset(rmse_comp_fits_2b, dist=="Gompertz" & trans==1)["mean_rmse"],4)`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b, dist=="Gompertz" & trans==1)["AIC"]),big.mark=",")`). The distribution for the second transition was Generalized gamma (Mean RMSE= `r round(subset(rmse_comp_fits_2b, dist=="Generalized gamma" & trans==2)["mean_rmse"],4)`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b, dist=="Generalized gamma" & trans==2)["AIC"]),big.mark=",")`). We decided to choose the third transition as a Log-logistic distribution because it shows better indices (Mean RMSE= `r subset(rmse_comp_fits_2b, dist=="Log-logistic" & trans==3)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b, dist=="Log-logistic" & trans==3)["AIC"]),big.mark=",")`).
...
We selected the distribution of the first transition as Generalized gamma (Mean RMSE= `r subset(rmse_comp_fits_2c, dist=="Generalized gamma" & trans==1)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a_4s, dist=="Generalized gammal" & trans==1)["AIC"]),big.mark=",")`). For the second transition, we selected the Gompertz distribution (Mean RMSE= `r subset(rmse_comp_fits_2c, dist=="Gompertz" & trans==2)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a_4s, dist=="Gompertz" & trans==2)["AIC"]),big.mark=",")`). In the third transition we must observe that after the 14th year for the third transition, the Generalized gamma shows best estimated hazards, but increases over 1.25 to an abnormal projection of 14,615 at year 14 and after, and none of the distribution reproduced very well the hazards (Mean RMSE= `r subset(rmse_comp_fits_2c, dist=="Generalized gamma" & trans==3)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a_4s, dist=="Generalized gamma" & trans==3)["AIC"]),big.mark=",")`). For the fourth transition, Log-logistic distribution fitted better compared to the other distributions (Mean RMSE= `r subset(rmse_comp_fits_2c, dist=="Log-logistic" & trans==4)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a_4s, dist=="Log-logistic" & trans==4)["AIC"]),big.mark=",")`). For the fifth transition, we selected the Lognormal distribution (Mean RMSE= `r subset(rmse_comp_fits_2c, dist=="Lognormal" & trans==4)["rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2a_4s, dist=="Lognormal" & trans==4)["AIC"]),big.mark=",")`).
...
Due to convergence issues, we defined the third transition for the Generalized gamma distribution using initial values of `r  paste0("0.0001 and ", round(mean(subset(ms2_CONS_C1_SEP_2020_women_imputed_exp, trans == 3)[,"time"]),3))`. There was very little to choose between the distributions, but we aimed to choose the distribution that achieve the best balance of a reasonable fit to the observed data and a sensible extrapolation. A Generalized gamma was chosen for the first transition (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Generalized gamma" & trans==1)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Generalized gamma" & trans==1)["AIC"]),big.mark=",")`). The distribution for the second transition was Gompertz (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Gompertz" & trans==2)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Gompertz" & trans==2)["AIC"]),big.mark=",")`). For the third transition, we decided to choose a Lognormal, considering that Generalized Gamma distributions required to specify starting values (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Lognormal" & trans==3)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Lognormal" & trans==3)["AIC"]),big.mark=",")`). For the fourth transition, we decided to choose a Log-logistic because it shows a lower AIC  (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Log-logistic" & trans==4)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Log-logistic" & trans==4)["AIC"]),big.mark=",")`). For the fifth transition, we decided to choose a Lognormal (Mean RMSE= `r subset(rmse_comp_fits_2b_4s, dist=="Lognormal" & trans==5)["mean_rmse"]`; AIC= `r format(as.numeric(subset(fit_flexsurvreg2b_4s, dist=="Lognormal" & trans==5)["AIC"]),big.mark=",")`)

We decided to not to include by now the Generalized F yet because we are still studying its behavior. However, it showed to be a good candidate for transitions 1 and 2, as well as transition 4 and 5 (but not the better for these two).
