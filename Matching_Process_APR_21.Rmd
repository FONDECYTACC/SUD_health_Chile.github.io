...
```{r fig5 mstate_models_3s, echo=T, fig.align='center', message=FALSE, fig.caption="Figure 17. Tree Structure for the Multistate System, Ten-states model", error=T, fig.height=8, eval=T, dpi=320}
library(Epi)
...
paste0(dta_path,"_mult_state_ags/ten_st_msprep.dta"))
...
d_match_surv_msprep %>% 
  ...
  #dplyr::rename("id"="row", "ther_disch_time"="date_ther_disch","ther_disch_status"="ther_disch",
  #             "readmission_time"="date_post_treat","readmission_status"="readmission"),
paste0(dta_path,"_mult_state_ags/ten_st_msprep.csv"))
```
...
ms_d_match_surv_res<-
  ...
  data.frame() %>%
  dplyr::group_by(id) %>% 
  #dplyr::mutate(across(paste0("arrival.",1:15),~max(.,na.rm=T))) %>% 
  #dplyr::mutate(arrival_alt = ifelse(status==0, 0, arrival), #remove NA
  #        Cum_Sum = cumsum(arrival_alt)) %>%

# dplyr::mutate(arrival_alt1_res = dplyr::case_when(tipo_de_plan_res_1==1 & trans %in% c(4:5)~ arrival,
#                T~0), Cum_Sum1_res = cumsum(arrival_alt1_res)) %>% 
#  dplyr::mutate(arrival_alt1 = dplyr::case_when(tipo_de_plan_res_1==1 & trans %in% c(4:5)~ time,
#                T~0), Cum_Sum1_time_to_readm = cumsum(arrival_alt1)) %>% 
#  dplyr::mutate(arrival_alt2_res = dplyr::case_when(tipo_de_plan_res_2==1 & trans %in% c(9:10)~ arrival,
#                T~0), Cum_Sum2_res = cumsum(arrival_alt2_res)) %>% 
#  dplyr::mutate(arrival_alt2 = dplyr::case_when(tipo_de_plan_res_2==1 & trans %in% c(9:10)~ time,
#                T~0), Cum_Sum2_time_to_readm = cumsum(arrival_alt2)) %>% 
#  dplyr::mutate(arrival_alt3_res = dplyr::case_when(tipo_de_plan_res_3==1 & trans %in% c(14:15)~ arrival,
#                T~0), Cum_Sum3_res = cumsum(arrival_alt3_res)) %>% 
#  dplyr::mutate(arrival_alt3 = dplyr::case_when(tipo_de_plan_res_3==1 & trans %in% c(14:15)~ arrival,
#                T~0), Cum_Sum3_time_to_readm = cumsum(arrival_alt3)) %>% 

  dplyr::mutate(arrival_alt1_res = dplyr::case_when(tipo_de_plan_res_1==1 & status==1 & trans %in% c(1:3)~ time,
                T~0), Cum_Sum1_res = cumsum(arrival_alt1_res)) %>% 
  dplyr::mutate(arrival_alt2_res = dplyr::case_when(tipo_de_plan_res_2==1 &  status==1 & trans %in% c(6:8)~ time,
                T~0), Cum_Sum2_res = cumsum(arrival_alt2_res)) %>% 
  dplyr::mutate(arrival_alt3_res = dplyr::case_when(tipo_de_plan_res_3==1 &  status==1 & trans %in% c(11:12)~ time,
                T~0), Cum_Sum3_res = cumsum(arrival_alt3_res)) %>% 

  dplyr::mutate(arrival_alt1_out = dplyr::case_when(tipo_de_plan_res_1==0 & status==1 & trans %in% c(1:3)~ time,
                T~0), Cum_Sum1_out = cumsum(arrival_alt1_out)) %>% 
  dplyr::mutate(arrival_alt2_out = dplyr::case_when(tipo_de_plan_res_2==0 &  status==1 & trans %in% c(6:8)~ time,
                T~0), Cum_Sum2_out = cumsum(arrival_alt2_out)) %>% 
  dplyr::mutate(arrival_alt3_out = dplyr::case_when(tipo_de_plan_res_3==0 &  status==1 & trans %in% c(11:12)~ time,
                T~0), Cum_Sum3_out = cumsum(arrival_alt3_out)) %>% 

  dplyr::select(-starts_with("arrival_alt")) %>% 
  dplyr::ungroup() %>% 
  #sum of days
  dplyr::mutate(Cum_Sum_res_tot = base::rowSums(dplyr::select(., Cum_Sum1_res, Cum_Sum2_res, Cum_Sum3_res)))%>%
  dplyr::mutate(Cum_Sum_out_tot = base::rowSums(dplyr::select(., Cum_Sum1_out, Cum_Sum2_out, Cum_Sum3_out))) %>% 
  dplyr::mutate(perc_in_res = Cum_Sum_res_tot/(Cum_Sum_res_tot+Cum_Sum_out_tot)) %>% 
  dplyr::mutate(perc_in_res=replace(perc_in_res, is.nan(perc_in_res), 0)) %>% 
  dplyr::mutate(perc_in_res=replace(perc_in_res, is.infinite(perc_in_res), 1))

ms_d_match_surv$perc_in_res<-ms_d_match_surv_res$perc_in_res
```

```{r frailty2,eval=T, echo=T, paged.print=TRUE}                                 

frailty_a<-
frailtypack::frailtyPenal(formula = Surv(Tstart,Tstop,status) ~ cluster(id) + tipo_de_plan_res_1 +perc_in_res, 
                          data = subset(ms_d_match_surv_res,!trans %in%c(1,2,6,7,11,12)),
                          RandDist="Gamma",
...
frailty_c<-
frailtypack::frailtyPenal(formula = Surv(Tstart,Tstop,status) ~ cluster(id) + tipo_de_plan_res_1, 
                          data = subset(ms_d_match_surv_res,!trans %in%c(1,2,6,7,11,12)),
                          RandDist="Gamma",
                          # "Gamma" for a gamma distribution, "LogN" for a log-normal distribution. Default is "Gamma".
...
paste0("Frailty including % of residential (Andersen-Gill model), Rand Effects: Gamma");summary(frailty_a, level = 0.95,len=3)
paste0("Frailty including % of residential (Andersen-Gill model), Rand Effects: Log-normal");summary(frailty_b, level = 0.95,len=3)
paste0("Frailty, Rand Effects: Gamma");summary(frailty_c, level = 0.95,len=3)
paste0("Frailty, Rand Effects: Log-normal");summary(frailty_d, level = 0.95,len=3)
...
```{r ph1,eval=T, echo=T, paged.print=TRUE, fig.height=13, fig.width=10, fig.cap="Figure 18a. LOG CUMULATIVE HAZARD VS LOG TIME PLOT (w/o covars)", dpi=320, fig.align="center"}                                 
...
rm(list=ls()[!ls() %in% c("trans_matrix","ms_d_match_surv")])
pryr::mem_used()
...
# Elimina datos inconsistentes
  ...
  covcols <- 1:ncol(data)
  # aquellas variables que no son las principales
...
```{r semimarkov_add_arrival,eval=T, echo=T, error=T, paged.print=TRUE}                                 
