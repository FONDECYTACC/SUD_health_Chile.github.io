...
CONS_C1_df_dup_MAY_2020_prev_6a%>%
  ...
  dplyr::mutate(origen_ingreso_mod= dplyr::case_when(origen_ingreso=="Otro Centro Tratamiento Drogas/ FONODROGAS/Previene"~"Derivación Asistida", TRUE~as.character(origen_ingreso)))%>%
...
```{r fig8_image-map_chile_hosp, echo=T, fig.align='center', fig.pos='H', message=FALSE, warning=FALSE, fig.cap= "Figure 8. Treemap of Cause of Discharge Depending On Treatment/Events Conditions"}
...
```{r tab7_spaces_treatments_glm_less60,eval=T, echo=T, paged.print=TRUE}
...
```{r fig9a_int_changes_referral,eval=F, echo=T, paged.print=TRUE,fig.cap= "Figure 9a. Interactive Effects in the Prob. of Having <=60 days of difference with the previous treatment"}
#
library("effects")
...
```{r fig9b_int_changes_referral,eval=T, echo=T, paged.print=TRUE,fig.cap= "Figure 9. Interactive Effects on Odds of Having <=60 days of difference with the previous treatment"}
library(lsmeans)
...
table_labels<-
  ...
  write.table(table_labels, file = "G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/SUD_CL/_label_var_to_stata.do", sep = "",
            row.names = FALSE, quote = FALSE,fileEncoding="latin1")
