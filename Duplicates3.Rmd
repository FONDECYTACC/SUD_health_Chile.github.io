...
In case of Age of Onset of Drug Use (edad_ini_cons), users that had Age of Onset of Drug Use (edad_ini_cons) greater than `r as.numeric(edad_ini_cons_summ[,"p75_ranges"])`.

```{r fig2_image-decision_tree_overlapped_cases_edad_ini_cons, echo=T, fig.align='center', fig.pos='H', fig.cap= "Figure 2. Decision Tree for the Users with more than one value in a Age of Onset of Substance Use of Primary Substance", message=FALSE}
...
In the case of Age of Onset of Drug Use Primary Substance (edad_ini_sus_prin), we needed to check for differences within each user, but for every admission in which he/she reported a common primary substance of admission. `r cases_affected_by_hash_sus_prin_age %>% formatC(, format="f", big.mark=",", digits=0)` entries had different ages of onset of drug use, in different treatments where they reported the same primary substance. For cases with ranges lower than `r as.numeric(summ_edad_ini_cons_sus_prin_por_concat_hash_sus_prin[,"p75_ranges"])` years or more, we replaced the age of onset of drug use with the average age for each user that reported the same primary substance.
...
The model was trained using the rest of the dataset with valid attributes on the variable sex (n= `r formatC(nrow(CONS_C1_df_dup_FEB_2020_prev24_train_nn),big.mark = ",")`). We generated a total of 10 cross-validations, with alternatives of 1 to 5 layers, and a threshold of 0, .5 and 1 in every model (10x5x3). The model that obtained the highest area under the curve  (AUC (mean)= `r round(nn_sex_par[1,"auc (mean)"],2)`) had a size of `r as.numeric(nn_sex_par[1,"size"])` `r ifelse(as.numeric(nn_sex_par[1,"size"])<2,"layer","layers")`, and a threshold for weights of `r as.numeric(nn_sex_par[1,"decay"])` (L2 penalty). THe following figure shows the neural network structure Due to the great amount of categories, we generated a garson plot of importance of variables into the weight.

<br>

```{r fig4a_neuralnet_sex,fig.height=8, fig.width=14, echo=T, fig.align='center', fig.pos='H', fig.cap="Figure 4a. Neural Network of Sex", message=FALSE}
...
To obtain valid ages, we predicted the age of onset of drug use.  We generated a total of 10 cross-validations, with alternatives of 1 to 5 layers, and a threshold of 0, .5 and 1 in every model (10x5x3). The model that obtained the best fit (RMSE (mean)= `r round(nn_edad_ini_cons_par[,3],2)`) had a size of `r result_edad_ini_cons_sus$size` hidden layers, and a threshold for weights of `r result_edad_ini_cons_sus$decay` (L2 penalty). 

<br>

```{r fig5a_NN_age_onset_drug_use_PLOT, fig.height=8, fig.width=13, warning=FALSE, fig.align = "center", message=F,fig.cap="Figure 5a. Neural Network of Age of Onset of Drug Use"}
...
```{r fig5b_neuralnet_age_onset_drug_use,fig.height=4, fig.width=10, echo=T, fig.align='center', fig.pos='H', fig.cap="Figure 5b. Garson Plot of Variable Importance, Age of Onset of Drug Use", message=FALSE}
...
We selected the age of onset of drug use of cases within users that had the same amount of different values in the age of onset of drug use. We also extended this selection to the age of onset of drug use in the primary (users=`r nrow(age_onset_drug_use_replace_neuralnet)`).

<br>

#### 3.3.Ties and major differences in age of onset of drug use of primary substance


We used a neural network model to obtain valid ages of onset of drug use of the primary substance for `r CONS_C1_df_dup_MAY_2020_prev_4 %>% dplyr::filter(concat_hash_sus_prin %in% c(as.character(unlist(casos_fecha_inicio_cons_sus_prin_empates_3["concat_hash_sus_prin"])))) %>% group_by(hash_key)%>% distinct(hash_key) %>% nrow()` users that had more than one age of onset of drug use in treatments with the same primary substance. 
...
To obtain valid ages, we predicted the age of onset of drug use.  We generated a total of 10 cross-validations, with alternatives of 1 to 5 layers, and a threshold of 0, .5 and 1 in every model (10x5x3). The model that obtained the best fit (RMSE (mean)= `r round(nn_edad_ini_cons_sus_prin_par[,3],2)`) had a size of `r result_edad_ini_cons_sus_prin$size` hidden layers, and a threshold for weights of `r result_edad_ini_cons_sus_prin$decay` (L2 penalty). 

<br>

```{r fig6a_NN_age_onset_drug_use_prim_subs_PLOT, fig.height=8, fig.width=13, warning=FALSE, fig.align = "center", message=F,fig.cap="Figure 6a. Neural Network of Age of Onset of Drug Use"}
...
```{r fig6b_neuralnet_age_onset_drug_use_prim_subs,fig.height=4, fig.width=10, echo=T, fig.align='center', fig.pos='H', fig.cap="Figure 6b. Garson Plot of Variable Importance, Age of Onset of Drug Use", message=FALSE}
#https://rdrr.io/cran/radiant.model/man/plot.nn.html  
...
We selected the age of onset of drug use in the primary substance of cases within users that had the same amount of different values in the age of onset of drug use of primary substance. We also extended this selection to the age of onset of drug use in the primary substance of admission (users=`r nrow(age_onset_drug_use_prim_subs_replace_neuralnet)`).
...
```{r fig7_image-map_chile_comunas, echo=T, fig.align='center', fig.pos='H', message=FALSE, warning=FALSE, fig.cap= "Figure 7b. Cases By Region"}
...
dplyr::mutate(edad_al_ing_grupos=ifelse(edad_al_ing>45,">45",
      ifelse(edad_al_ing>=36,"36-45",
      ifelse(edad_al_ing>=30,"30-35",
      ifelse(edad_al_ing>=25,"25-29",
      ifelse(edad_al_ing>=18,"18-24",
      NA)))))) %>%
dplyr::mutate(edad_al_ing_grupos=as.factor(edad_al_ing)) %>%  
    #edad_al_ing_grupos
...
We observed the cases that remained with more or equal than 1,095 days (n= `r CONS_C1_df_dup_MAY_2020_prev_6c%>% dplyr::filter(dias_trat_imp>1095)%>% nrow()`). Must take note that SENDAs professionals believed that treatments should not last more than 1095 days.
...
As seen in the Table above, most of the users appeared in more than one yearly database with the same date of admission. Additionally, many of those cases that had more than 2000 days of treatment came from databases of 2010. Additionally, we were interested to find the most frequent dates of discharge, to check if there were eventual common dates of discharge among these abnormal cases.
...
Considering that we were focused on treatments from 2010 to 2019, we decided to delete those cases with more than 1095 days of treatment that started before 2010. Equally, we deleted those cases that rounded the change of the databases.
...
CONS_C1_df_dup_MAY_2020_prev_6c%>%  
  ...
  ungroup()%>%
  dplyr::mutate(obs=case_when(sum_excess_days_bef_2010>0~glue::glue("{obs};4.04.1.HASH w/ cases w/ >1095 d treat started <2010"),TRUE~obs))%>%  
  dplyr::mutate(obs=case_when(sum_near_year_more_1095>0~glue::glue("{obs};4.04.2.HASH w/ cases w/ >1095 d treat, discharge start of new yr"),TRUE~obs))%>%  
...
The dataset ended in `r nrow(CONS_C1_df_dup_MAY_2020_prev_6d)%>% formatC(big.mark=",")` entries, of `r length(unique(CONS_C1_df_dup_MAY_2020_prev_6d$hash_key))%>% formatC(big.mark=",")` users.

<br>

#### 4.5. Related to the number of dependent and others

<br>

There are two questions concerning the number of children: the number of dependent children (`numero_de_hijos`) and the amount of children to enter to a residential treatment (`num_hijos_ing_trat_res`). Must note that `r CONS_C1_df_dup_MAY_2020_prev_6d%>% dplyr::group_by(hash_key)%>% dplyr::arrange(hash_key, desc(fech_ing))%>%dplyr::mutate(lag_numero_de_hijos=lag(numero_de_hijos),n=n(),validador_n_hijos=ifelse(lag_numero_de_hijos-numero_de_hijos<0,1,0),sum_validador=sum(validador_n_hijos,na.rm=T))%>% ungroup()%>% dplyr::filter(sum_validador>0)%>% distinct(hash_key)%>% nrow()%>%  formatC(big.mark=",")` users report less children compared to at least one of the next treatments that user had. We did not changed this number because we had no reason to think that some children could have died or not longer being recognized as they were in the previous treatment.
...
Lastly, we did not modify the question that asked to the relative(s) with whom the user lives. Consistent with the above rules, we could have discarded those cases that did not report any children but declared that lived with one. However, it is possible that in the last 30 days the user could not have been living with their children. That is why we decided to keep this variable unaltered.
...
As seen in the table above, most of the users that were in a drug treatment courts modality entered to a treatment motivated by initiatives from the justice sector. However, we could not define a rule to estimate is one value of the variables is valid or not.

<br>

One of the variables that is not available in the glossary of terms are the ones related to reported disability and cause of disability. 
...
As seen in the Table above, most of the reported causes of disability were physical and psychical. However, there were not evident errors of classification of types depending on disabilities.
...
```{r fig10a_int_changes_referral,eval=F, echo=T, paged.print=TRUE,fig.cap= "Figure 10a. Interactive Effects in the Prob. of Having <60 days of difference with the previous treatment"}
#
library("effects")
...
```{r fig10b_int_changes_referral,eval=T, echo=T, paged.print=TRUE,fig.cap= "Figure 10. Interactive Effects on Odds of having <60 days of difference with the previous entry"}
library(lsmeans)
...
CONS_C1_df_dup_MAY_2020_prev_6e%>%
  dplyr::left_join(CONS_C1_df_dup_MAY_2020_prev_6e_glm_mas_de_un_trat[c("row",                                                                        "menor_60_dias_diff","diff_bet_treat","id_centro_sig_trat","tipo_plan_sig_trat",              "tipo_programa_sig_trat","senda_sig_trat","motivoegreso_derivacion","obs_cambios",
...
We looked over the center in which each user was referred, but considering the cause of admission.
...
From the table above, we could observe that no other causes of discharge had a type of referral, so there was no reason to alter this variable.

<br>
  
```{r fig11_,eval=T, echo=T, paged.print=TRUE,fig.cap= "Figure 11. Interactive Effects on Odds of having <=60 days of difference with the previous entry", eval=F}
...
```{r fig11_type_center_referral_changes, eval=T, echo=T, fig.cap="Figure 11. Changes with the Next Treatment by No. of Days of Treatment, depending on Type of Referrals", out.height='200%'}
 CONS_C1_df_dup_JUN_2020%>% 
    dplyr::filter(!is.na(diff_bet_treat))%>%
    #dplyr::mutate(motivoegreso_derivacion=factor(motivoegreso_derivacion, levels = c("Motivo: Otro", "Motivo: Derivación","NA")))%>%
    dplyr::mutate(obs_cambios_num=factor(obs_cambios_num))%>%
...
As seen in the Table above, it was not clear whether the absence of a following treatment is explicitly associated with a type of center of referral. This is why we had to ask SENDAs professionals about the characteristics of the referrals.
...
dplyr::mutate_at(c('dg_trs_psiq_cie_10_or','x2_dg_trs_psiq_cie_10_or','x3_dg_trs_psiq_cie_10_or','x2_dg_trs_psiq_sub_cie_10_or','x3_dg_trs_psiq_sub_cie_10_or','compromiso_biopsicosocial','pais_nacimiento','nacionalidad','dg_trs_psiq_sub_dsm_iv_or','x2_dg_trs_psiq_sub_dsm_iv_or','x3_dg_trs_psiq_sub_dsm_iv_or','dg_trs_psiq_sub_cie_10_or','etnia_cor_2','motivodeegreso_mod_imp','fecha_ultimo_tratamiento','fecha_ultimo_tratamiento','tiene_menores_de_edad_a_cargo'),~as.factor(.)) %>%
  ...
  dplyr::mutate_at(vars(at_least_one_cont_entry,menor_45_dias_diff, menor_45_dias_diff,menor_60_dias_diff,obs_cambios_ninguno,motivoegreso_derivacion),~as.factor(.))%>%
  assign("CONS_C1_df_dup_JUN_2020",., envir = .GlobalEnv)
...
at_least_one_cont_entry= "Usuarios con más de una entrada después de otra/Users with more than one entry after another one"
)
