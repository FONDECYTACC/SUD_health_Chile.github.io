...
#### 4.4. Related to days of treatment

<br>

We observed the cases that remained with more or equal than 1,095 days (n= `r CONS_C1_df_dup_JUN_2020%>% dplyr::filter(dias_trat_imp>1095)`). Must take note that SENDAs professionals believed that treatments should not last more than 1095 days.
...
df_tab5%>%
  knitr::kable(format= "html", format.args= list(decimal.mark= ".", big.mark= ","),
            caption="Table 5. Trajectories of users with more than 1,095 days (w/ dates of the original dataset)",
            align= rep('c', 101))  %>%
        kableExtra::kable_styling() %>%
        kableExtra::row_spec(which(df_tab5$dias_trat_imp>1095), bold= F, color= "white", background= "steelblue")%>%
        kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size= 8)%>%
...
As seen in the Table above, most of the users appeared in more than one yearly database with the same date of admission. 

<br>

### 5.Exploration of spaces between treatments
...
CONS_C1_df_dup_MAY_2020_prev_6c_exp_spaces<-
      CONS_C1_df_dup_MAY_2020_prev_6c %>%
          dplyr::group_by(hash_key) %>%
...
knitr::kable(cbind(data.frame(psych::describe(CONS_C1_df_dup_MAY_2020_prev_6c_exp_spaces$diff_bet_treat)),t(quantile(CONS_C1_df_dup_MAY_2020_prev_6c_exp_spaces$diff_bet_treat,c(.10,.25,.5,.75,.9),na.rm=T))),
            format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
            caption="Table 6. Summary Table for Difference Between Treatments (with the following entry)",
            align =rep('c', 101))  %>%
        kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
...
quantiles_diff_bet_treat<-data.table::data.table(cbind(quantile(CONS_C1_df_dup_MAY_2020_prev_6c_exp_spaces$diff_bet_treat,c(seq(1,100)/100),na.rm=T)),keep.rownames = T)
#117387-31665
...
```{r fig8_image-map_chile_hosp, echo=T, fig.align='center', fig.pos='H', message=FALSE, warning=FALSE, fig.cap= "Figure 9. Treemap of Cause of Discharge Depending On Treatment/Events Conditions"}
...
```{r tab7_spaces_treatments,eval=T, echo=T, paged.print=TRUE}
CONS_C1_df_dup_MAY_2020_prev_6c %>%
    dplyr::group_by(hash_key) %>%
    dplyr::mutate(fech_ing_num_may20=as.numeric(as.Date(fech_ing))) %>%
    dplyr::mutate(fech_egres_num_may20=as.numeric(as.Date(fech_egres_imp)))%>%
  ...
  knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
            caption="Table 7. Frequency and Percentage of Types of Changes Between Treatments (<=60 days between treatments)",
...
```{r tab8_spaces_treatments_glm_less60,eval=T, echo=T, paged.print=TRUE}
...
CONS_C1_df_dup_MAY_2020_prev_6c_glm_mas_de_un_trat <-
  CONS_C1_df_dup_MAY_2020_prev_6c %>%
  dplyr::group_by(hash_key) %>%
  dplyr::mutate(fech_ing_num_may20=as.numeric(as.Date(fech_ing))) %>%
  dplyr::mutate(fech_egres_num_may20=as.numeric(as.Date(fech_egres_imp))) %>%
...
glm_60d <- glm(menor_60_dias_diff ~ motivoegreso_derivacion*obs_cambios_fac, 
                data=CONS_C1_df_dup_MAY_2020_prev_6c_glm_mas_de_un_trat, family="binomial")
library(finalfit)
...
knitr::kable(t,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
            caption="Table 8. Summary Table for Difference Between Treatments",
            align =rep('c', 101))  %>%
        kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
        kableExtra::scroll_box(width = "100%", height = "350px")
```
...
CONS_C1_df_dup_MAY_2020_prev_6c%>%
  dplyr::left_join(CONS_C1_df_dup_MAY_2020_prev_6c_glm_mas_de_un_trat[c("row",                                                                        "menor_60_dias_diff","diff_bet_treat","id_centro_sig_trat","tipo_plan_sig_trat",              "tipo_programa_sig_trat","senda_sig_trat","motivoegreso_derivacion","obs_cambios",
