...
### 4.Exploration of spaces between treatments
...
```{r tab5_spaces_treatments2,eval=T, echo=T, paged.print=TRUE}
CONS_C1_df_dup_MAY_2020_prev_5_exp_spaces<-
...
knitr::kable(cbind(data.frame(psych::describe(CONS_C1_df_dup_MAY_2020_prev_5_exp_spaces$diff_bet_treat)),t(quantile(CONS_C1_df_dup_MAY_2020_prev_5_exp_spaces$diff_bet_treat,c(.10,.25,.5,.75,.9),na.rm=T))),
            format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
            caption="Table 5. Summary Table for Difference Between Treatments",
            align =rep('c', 101))  %>%
        kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
...
quantiles_diff_bet_treat<-data.table::data.table(cbind(quantile(CONS_C1_df_dup_MAY_2020_prev_5_exp_spaces$diff_bet_treat,c(seq(1,100)/100),na.rm=T)),keep.rownames = T)
#117387-31665
...
Considering the distribution of differences in days between each entry, we defined the cutpoint in 30 days, which represented the `r quantiles_diff_bet_treat %>% dplyr::filter(V1>=30) %>% slice(1) %>% dplyr::select(rn)` of the total differences .(n= `r dplyr::filter(CONS_C1_df_dup_MAY_2020_prev_5_exp_spaces,!is.na(diff_bet_treat)) %>% nrow() %>% formatC(, format="f", big.mark=",", digits=0)`).


### 5.Standardization of the Cause of Discharge


```{r tab6_spaces_treatments,eval=T, echo=T, paged.print=TRUE}

### Casos que tienen un motivo de egreso de derivación, pero en
sin_tratamiento_posterior=0
if(sin_tratamiento_posterior==1){
      ...
      CONS_C1_df_dup_MAY_2020_prev_5 %>%
       ...
       # dplyr::select(row,hash_key,fech_ing,fech_egres_imp, dias_trat_imp,diff_bet_treat,motivodeegreso_imp,motivodeegreso_mod_imp,id_centro)
    # 1. Motivo de egreso
        dplyr::filter(is.na(diff_bet_treat)) %>% janitor::tabyl(motivodeegreso_mod)  
    # Una8817 tiene como motivo Derivación
        #dplyr::select(row,hash_key,fech_ing,fech_egres_imp, dias_trat_imp,diff_bet_treat,motivodeegreso_imp,motivodeegreso_mod_imp,id_centro)
}

con_tratamiento_posterior=0
if(con_tratamiento_posterior==1){
      ...
      CONS_C1_df_dup_MAY_2020_prev_5 %>%
       ...
       # dplyr::select(row,hash_key,fech_ing,fech_egres_imp, dias_trat_imp,diff_bet_treat,motivodeegreso_imp,motivodeegreso_mod_imp,id_centro)
    # 1. Motivo de egreso
        dplyr::filter(!is.na(diff_bet_treat),diff_bet_treat<=30) %>% janitor::tabyl(motivodeegreso_mod)  
    # Una8817 tiene como motivo Derivación
        #dplyr::select(row,hash_key,fech_ing,fech_egres_imp, dias_trat_imp,diff_bet_treat,motivodeegreso_imp,motivodeegreso_mod_imp,id_centro)
}
...
CONS_C1_df_dup_MAY_2020_prev_5 %>%
  ...
  #ver los criterios ppt
    dplyr::filter(!is.na(diff_bet_treat),diff_bet_treat<=30) %>%
  ...
  janitor::tabyl(obs_cambios)%>% knitr::kable(format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
            caption="Table 6. Frequency and Percentage of Types of Changes Between Treatments",
            align =rep('c', 101))  %>%
        kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
...
```{r tab7_spaces_treatments,eval=F, echo=T, paged.print=TRUE}

    #CASE_WHEN= Si la dif_bet_treat era menor que 30 DIAS, HACEMOS ESTO, SINO, HACEMOS ESTO OTRO.
  dplyr::mutate(motivodeegreso_imp=dplyr::case_when(diff_bet_treat<=30~"Derivación", TRUE~as.character(motivodeegreso_imp)))%>%
  dplyr::mutate(motivodeegreso_mod_imp=dplyr::case_when(diff_bet_treat<=30~"Deri
                                                        vación", TRUE~as.character(motivodeegreso_mod_imp)))%>%  
  ## Calcular de nuevo las variables que dependendn de dias trat para todos los casos imputados
  dplyr::mutate(motivodeegreso_mod_imp= case_when(dias_trat_alta_temprana_imp=="Menos de 90 días" & motivodeegreso_imp=="Abandono", "Abandono Temprano",as.character(motivodeegreso_mod_imp))) %>% 
...
```{r change labels,echo=T, paged.print=TRUE}
  metadata(CONS_C1_df_dup_MAY_2020_prev_5)$name <- "Agreement 1 SENDA"
  metadata(CONS_C1_df_dup_MAY_2020_prev_5)$description <- "Information About Agreement 1 of SENDA and MINSAL"
  
codebook::var_label(CONS_C1_df_dup_MAY_2020_prev_5) <- list(
...
codebook::val_labels(CONS_C1_df_dup_MAY_2020_prev_5$dg_trs_psiq_cie_10_or) <- c('En estudio(NA)' = 1,
...
codebook::val_labels(CONS_C1_df_dup_MAY_2020_prev_5$x2_dg_trs_psiq_cie_10_or) <- c('En estudio(NA)' = 1,
'Esquizofrenia, trastorno esquizotípico y trastornos de ideas delirantes(F20-29)' = 2,
'Retraso Mental(F70-79)' = 3,
'Sin trastorno(NA)' = 4,
'Trastornos de la conducta alimentaria(F50)' = 5,
...
'Trastornos neuróticos, secundarios a situaciones estresantes y somatomorfos(F40-49)' = 12,
'Trs. del comportamiento y de las emociones de comienzo habitual en la infancia y adolescencia(F90-98)' = 13)
codebook::val_labels(CONS_C1_df_dup_MAY_2020_prev_5$x3_dg_trs_psiq_cie_10_or) <- c('En estudio(NA)' = 1,
'Esquizofrenia, trastorno esquizotípico y trastornos de ideas delirantes(F20-29)' = 2,
'Retraso Mental(F70-79)' = 3,
'Sin trastorno(NA)' = 4,
'Trastornos de la conducta alimentaria(F50)' = 5,
...
'Trastornos neuróticos, secundarios a situaciones estresantes y somatomorfos(F40-49)' = 12,
'Trs. del comportamiento y de las emociones de comienzo habitual en la infancia y adolescencia(F90-98)' = 13)
codebook::val_labels(CONS_C1_df_dup_MAY_2020_prev_5$dg_trs_psiq_sub_cie_10_or) <- c('Abuso de sustancias que no producen dependencia(F55X)' = 1,
...
codebook::val_labels(CONS_C1_df_dup_MAY_2020_prev_5$x2_dg_trs_psiq_sub_cie_10_or) <- c('Abuso de sustancias que no producen dependencia(F55X)' = 1,
'Delirium no inducido por alcohol u otras sustancias psicotropas(F059)' = 2,
'Demencia en enfermedades clasificadas en otro lugar(F02)' = 3,
...
codebook::val_labels(CONS_C1_df_dup_MAY_2020_prev_5$x3_dg_trs_psiq_sub_cie_10_or) <- c('Delirium no inducido por alcohol u otras sustancias psicotropas(F059)' = 1,
...
#####to_export_labels
tibble::rownames_to_column(data.frame(Hmisc::label(CONS_C1_df_dup_MAY_2020_prev_5))) %>% data.frame() %>%
...
#                                                                     "Cause of Discharge w/ Early or Late Withdrawal (Imputed)"))) %>% 
  write.csv2("__labels_to_stata_C1_may_2020.csv")
CONS_C1_df_dup_MAY_2020_prev_5 %>%
  dplyr::arrange(hash_key, desc(fech_ing)) %>% 
  rio::export(file = "G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/CONS_C1_df_dup_MAY_2020.dta")
save.image("G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/6.RData")
```

### 6. Preliminary Summary in June 2020
