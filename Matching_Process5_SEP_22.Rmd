...
We set the data in different formats. We stratified the data depending on patient's baseline treatment completion status.
...
cbind(1,data.table(biostat3::survRate(Surv(TD_time, TD_status) ~ 1, data=d_match_surv_msprep_oct_2022), keep.rownames = T))
  ...
  dplyr::mutate_at(c("rate", "lower", "upper"),~round(.*1e5,0)) %>% 
  dplyr::mutate(tipo_de_plan_res_1= dplyr::case_when(tipo_de_plan_res_1==0~ "Ambulatory",tipo_de_plan_res_1==1~ "Residential", T~ "Total")) %>% 
  knitr::kable(caption="TC", col.names= c("Setting", "tstop", "event", "rate (per 100,000)", "lower", "upper"))%>%
...
cbind(1,data.table(biostat3::survRate(Surv(DWCA_time, DWCA_status) ~ 1, data=d_match_surv_msprep_oct_2022), keep.rownames = T))
  ...
  dplyr::mutate_at(c("rate", "lower", "upper"),~round(.*1e5,0)) %>% 
  dplyr::mutate(tipo_de_plan_res_1= dplyr::case_when(tipo_de_plan_res_1==0~ "Ambulatory",tipo_de_plan_res_1==1~ "Residential", T~ "Total")) %>% 
  knitr::kable(caption="TNC", col.names= c("Setting", "tstop", "event", "rate (per 100,000)", "lower", "upper"))%>%
  kableExtra::kable_classic()
```
...
cbind(1,data.table(biostat3::survRate(Surv(Readmission_time, Readmission_status) ~ 1, data=d_match_surv_msprep_oct_2022), keep.rownames = T))
  ...
  dplyr::mutate_at(c("rate", "lower", "upper"),~round(.*1e5,0)) %>% 
  dplyr::mutate(tipo_de_plan_res_1= dplyr::case_when(tipo_de_plan_res_1==0~ "Ambulatory",tipo_de_plan_res_1==1~ "Residential", T~ "Total")) %>% 
  knitr::kable(caption="Readmission", col.names= c("Setting", "tstop", "event", "rate (per 100,000)", "lower", "upper"))%>%
  kableExtra::kable_classic()
```
...
plot(traveller_oct_2022) %>%
    DiagrammeRsvg::export_svg %>% charToRaw %>% rsvg::rsvg_eps(paste0(path,"/_mult_state_ags/_flowchart_transition_22oct.eps"))
```
...
#load("C:/Users/CISS Fondecyt/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_5_apr22.RData")
#sel_param_fits_a
#dt_ms_d_match_surv_oct_2022_a

#https://cran.r-project.org/web/packages/tipr/tipr.pdf
...
HR_evalue<-
function(x, trans, term="factor(tipo_de_plan_res_1)1"){
  # compute E-value for HR = 0.56 with CI: [0.46, 0.69]
  # for a common outcome
  # toRR(HR(0.56, 0.46, 0.69, rare = FALSE))
  
  fit<-get(x)[[trans]]
  rare_cond<-(fit$event/fit$N)<.15
  y<- match(term,attr(coef(fit), "names"))

      RR_est<-toRR(HR(exp(coef(fit)[y]),rare=rare_cond))
      RR_lo<-toRR(HR(exp(confint(fit)["factor(tipo_de_plan_res_1)1",1]),rare=rare_cond))
      RR_up<-toRR(HR(exp(confint(fit)["factor(tipo_de_plan_res_1)1",2]),rare=rare_cond))
      
      evalue_or<- EValue::evalues.RR(RR_est, 
                   RR_lo, 
                   RR_up, 
                   rare=rare_cond)
...
HR_evalue_aft<-
function(x, trans, term="factor(tipo_de_plan_res_1)1", time= "90 days"){
  # compute E-value for HR = 0.56 with CI: [0.46, 0.69]
  # for a common outcome
  # toRR(HR(0.56, 0.46, 0.69, rare = FALSE))
  # "90 days", "1 year", "3 years" "5 years"
  # get("fits_c_ggam3")[[4]]$dlist$name
  
  fit<-get(x)[[trans]]
  rare_cond<-(fit$events/fit$N)<.15
  tt<-time
  print(fit$dlist$name)
  if(fit$dlist$name =="lnorm"){
      RR_est<-toRR(HR(as.numeric(logn_aft_to_ahr(x, trans)[tt]), rare=rare_cond))
      RR_lo<- toRR(HR(as.numeric(logn_aft_to_ahr_up(x, trans)[tt]), rare=rare_cond))
      RR_up<- toRR(HR(as.numeric(logn_aft_to_ahr_lo(x, trans)[tt]), rare=rare_cond))
      ...
      RR_est<-toRR(HR(as.numeric(ggam_aft_to_ahr(x, trans)[tt]), rare=rare_cond))
      RR_lo<- toRR(HR(as.numeric(ggam_aft_to_ahr_up(x, trans)[tt]), rare=rare_cond))
      RR_up<- toRR(HR(as.numeric(ggam_aft_to_ahr_lo(x, trans)[tt]), rare=rare_cond))
 ...
 evalue_gomp1<- EValue::evalues.RR(toRR(HR(as.numeric(exp(coef(fits_c_gomp3[[1]])["factor(tipo_de_plan_res_1)1"])),rare= (fits_c_gomp3[[1]]$events/fits_c_gomp3[[1]]$N))), 
                   toRR(HR(as.numeric(exp(confint(fits_c_gomp3[[1]])["factor(tipo_de_plan_res_1)1",1])),rare= (fits_c_gomp3[[1]]$events/fits_c_gomp3[[1]]$N))), 
                   toRR(HR(as.numeric(exp(confint(fits_c_gomp3[[1]])["factor(tipo_de_plan_res_1)1",2])),rare= (fits_c_gomp3[[1]]$events/fits_c_gomp3[[1]]$N))), 
                   rare=(fits_c_gomp3[[1]]$events/fits_c_gomp3[[1]]$N)<.15)

 evalue_gomp2<-  EValue::evalues.RR(toRR(HR(as.numeric(exp(coef(fits_c_gomp3[[2]])["factor(tipo_de_plan_res_1)1"])),rare= (fits_c_gomp3[[2]]$events/fits_c_gomp3[[2]]$N))), 
                   toRR(HR(as.numeric(exp(confint(fits_c_gomp3[[2]])["factor(tipo_de_plan_res_1)1",1])),rare= (fits_c_gomp3[[2]]$events/fits_c_gomp3[[2]]$N))), 
                   toRR(HR(as.numeric(exp(confint(fits_c_gomp3[[2]])["factor(tipo_de_plan_res_1)1",2])),rare= (fits_c_gomp3[[2]]$events/fits_c_gomp3[[2]]$N))), 
                   rare=(fits_c_gomp3[[2]]$events/fits_c_gomp3[[2]]$N)<.15)
           
#logn_aft_to_ahr("fits_c_logn3",3)

e_values_df <-
 dplyr::bind_rows( 
cbind.data.frame(Trans=1, tt="", data.table::data.table(evalue_gomp1, keep.rownames = T)),
cbind.data.frame(Trans=2, tt="", data.table::data.table(evalue_gomp2, keep.rownames = T)),
cbind.data.frame(Trans=3, tt="90 d", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=3, time="90 days"), keep.rownames = T)),
cbind.data.frame(Trans=3, tt="1 yr", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=3, time="1 year"), keep.rownames = T)),
cbind.data.frame(Trans=3, tt="3 yr", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=3, time="3 years"), keep.rownames = T)),
cbind.data.frame(Trans=3, tt="5 yr", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=3, time="5 years"), keep.rownames = T)),
cbind.data.frame(Trans=4, tt="90 d", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=4, time="90 days"), keep.rownames = T)),
cbind.data.frame(Trans=4, tt="1 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=4, time="1 year"), keep.rownames = T)),
cbind.data.frame(Trans=4, tt="3 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=4, time="3 years"), keep.rownames = T)),
cbind.data.frame(Trans=4, tt="5 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=4, time="5 years"), keep.rownames = T)),
cbind.data.frame(Trans=5, tt="90 d", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=5, time="90 days"), keep.rownames = T)),
cbind.data.frame(Trans=5, tt="1 yr", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=5, time="1 year"), keep.rownames = T)),
cbind.data.frame(Trans=5, tt="3 yr", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=5, time="3 years"), keep.rownames = T)),
cbind.data.frame(Trans=5, tt="5 yr", data.table::data.table(HR_evalue_aft(x="fits_c_logn3",trans=5, time="5 years"), keep.rownames = T)),
cbind.data.frame(Trans=6, tt="90 d", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=6, time="90 days"), keep.rownames = T)),
cbind.data.frame(Trans=6, tt="1 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=6, time="1 year"), keep.rownames = T)),
cbind.data.frame(Trans=6, tt="3 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=6, time="3 years"), keep.rownames = T)),
cbind.data.frame(Trans=6, tt="5 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=6, time="5 years"), keep.rownames = T)),
cbind.data.frame(Trans=7, tt="90 d", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=7, time="90 days"), keep.rownames = T)),
cbind.data.frame(Trans=7, tt="1 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=7, time="1 year"), keep.rownames = T)),
cbind.data.frame(Trans=7, tt="3 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=7, time="3 years"), keep.rownames = T)),
cbind.data.frame(Trans=7, tt="5 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=7, time="5 years"), keep.rownames = T)),
cbind.data.frame(Trans=8, tt="90 d", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=8, time="90 days"), keep.rownames = T)),
cbind.data.frame(Trans=8, tt="1 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=8, time="1 year"), keep.rownames = T)),
cbind.data.frame(Trans=8, tt="3 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=8, time="3 years"), keep.rownames = T)),
cbind.data.frame(Trans=8, tt="5 yr", data.table::data.table(HR_evalue_aft(x="fits_c_ggam3",trans=8, time="5 years"), keep.rownames = T))
)

# HR_evalue_aft(x="fits_c_logn3",trans=3, time="90 days")
# HR_evalue_aft(x="fits_c_logn3",trans=3, time="1 year")
# HR_evalue_aft(x="fits_c_logn3",trans=3, time="3 years")
# HR_evalue_aft(x="fits_c_logn3",trans=3, time="5 years")
# 
# #ggam_aft_to_ahr( "fits_c_ggam3", 4)
# HR_evalue_aft(x="fits_c_ggam3",trans=4, time="90 days")
# HR_evalue_aft(x="fits_c_ggam3",trans=4, time="1 year")
# HR_evalue_aft(x="fits_c_ggam3",trans=4, time="3 years")
# HR_evalue_aft(x="fits_c_ggam3",trans=4, time="5 years")
# 
# #logn_aft_to_ahr("fits_c_logn3",3)
# HR_evalue_aft(x="fits_c_logn3",trans=5, time="90 days")
# HR_evalue_aft(x="fits_c_logn3",trans=5, time="1 year")
# HR_evalue_aft(x="fits_c_logn3",trans=5, time="3 years")
# HR_evalue_aft(x="fits_c_logn3",trans=5, time="5 years")
# 
# #ggam_aft_to_ahr( "fits_c_ggam3", 6)
# HR_evalue_aft(x="fits_c_ggam3",trans=6, time="90 days")
# HR_evalue_aft(x="fits_c_ggam3",trans=6, time="1 year")
# HR_evalue_aft(x="fits_c_ggam3",trans=6, time="3 years")
# HR_evalue_aft(x="fits_c_ggam3",trans=6, time="5 years")
# 
# #ggam_aft_to_ahr( "fits_c_ggam3", 8)
# HR_evalue_aft(x="fits_c_ggam3",trans=8, time="90 days")
# HR_evalue_aft(x="fits_c_ggam3",trans=8, time="1 year")
# HR_evalue_aft(x="fits_c_ggam3",trans=8, time="3 years")
# HR_evalue_aft(x="fits_c_ggam3",trans=8, time="5 years")

e_values_df %>% 
    tidyr::pivot_wider(id_cols= c("Trans", "tt"), names_from= "rn", values_from= c("point", "lower", "upper")) %>% 
    dplyr::select(Trans, tt, `point_E-values`, `lower_E-values`, `upper_E-values`, "point_RR", lower_RR, upper_RR) %>% 
  knitr::kable(caption="E-values by each geometric averaged Hazard Ratio by each transition (onlñy 1 row= PH models)", col.names= c("Transition", "Time", "E value", "95%IC Lo", "95%IC Up", "RR", "RR 95%CI Lo", "RR 95%CI Up"), digits=2)%>%
  kableExtra::kable_classic()

# Trans   tt       rn    point    lower    upper
```
