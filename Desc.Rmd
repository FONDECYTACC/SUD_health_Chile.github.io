...
```{r c1_std_no_ocupar,echo=T, paged.print=TRUE, eval=F}
CONS_C1_df_SEP_2020<-
clean_names(CONS_C1_df) %>% 
    dplyr::filter(!row %in% unlist(CONS_C1_df_dup_JUL_2020$row)) %>% 
#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:  
...
dplyr::mutate(across(c(dg_trs_psiq_cie_10_or,x2_dg_trs_psiq_dsm_iv_or,x3_dg_trs_psiq_dsm_iv_or,x4_dg_trs_psiq_dsm_iv_or),~dplyr::case_when(grepl("En estudio",as.character(.),ignore.case = T)~0,grepl("Sin trastorno",as.character(.),ignore.case = T)~0,is.na(.)~0,TRUE~1),.names = "{col}_mod2b"))%>%
  ...
  dplyr::mutate(dg_dsm_iv_rec=dplyr::case_when(total_dsm_iv_dg>0 ~"With psychiatric comorbidity",
                       is.na(total_dsm_iv_dg) & total_dsm_iv_en_est>0~"Diagnosis unknown (under study)",
                       total_dsm_iv_dg==0 & total_dsm_iv_en_est>0~"Diagnosis unknown (under study)",
                       TRUE~"Without psychiatric comorbidity"))%>%
  dplyr::mutate(dg_dsm_iv_rec=as.factor(dg_dsm_iv_rec)) %>% 
  dplyr::select(-ends_with("_mod1b"),-ends_with("_mod2b"),-total_dsm_iv_dg,-total_dsm_iv_en_est)%>%
  #:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:
  #MACROZONA
...
#Corrección número de diagnósticos dsm-iv
  #_#_#_#_#_#_#_#_#_#_#_
...
#_#_#_#_#_#_#_#_#_#_#_
  #Corrección número de diagnósticos físicos
#_#_#_#_#_#_#_#_#_#_#_
  #sumrows de los diagnósticos, excalesism1actevv
...
cnt_mod_cie_10_or= 'Recuento de Diagnóstico CIE-10(sólo diagnósticos)/Count of Psychiatric Diagnoses (ICD-10)(only diagnoses)',
