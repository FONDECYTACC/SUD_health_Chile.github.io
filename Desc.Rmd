...
cool_survival <- function(data = data_frame, time = time, censor = censor, group = group){
	require(survival)
	require(ggfortify)
	fit <- survfit(Surv(time, censor) ~ group, data = data)
	plot_data <- fortify(fit, surv.connect = TRUE)
	plot <- ggplot(plot_data, aes(x = time, y = surv * 100)) + 
		# plot the survival curve(s)
		geom_step(aes(colour = strata), size = 2) + 
		# add the censor indicators
		geom_point(data = subset(plot_data, n.censor > 0), shape = 3, size = 3, stroke = 2, aes(colour = strata)) + 
		# add confidence intervals using the stepribbon geom in the RcmdrPlugin.KMggplot2 package
		RcmdrPlugin.KMggplot2::geom_stepribbon(data = plot_data, 
			aes(x = time, ymin = lower * 100, ymax = upper * 100, fill = strata), 
			alpha = 0.15, colour = "transparent", show.legend = FALSE, kmplot = TRUE) + 
		# set the colours of the confidence intervals
		scale_fill_manual(values = c("steelblue", "darkorange")) +
		# set the colours of the survival curves
		scale_colour_manual(values = c("steelblue", "darkorange"), name = "Hip") + 
		# apply a theme based on the Roboto font size 16
		theme_bw(base_size = 16, base_family = "Roboto") +
		# add a title, x label, y label and footnote
		labs(title = "Custom Made Survival Curve",
			subtitle = "with ggfortify and ggplot2",
     	  x = "Time [years]",
     	  y = "Survival Probability [%]",
     	  caption = "countcool.com") +
	  theme_bw()+
 		# set the caption to a smaller italic font in darkolivergreen colour
    	theme(plot.caption = element_text(colour = "darkolivegreen", face = "italic", size = 10)) +
  		# set the y axis limit in steps of 20 %
  		scale_y_continuous(breaks = seq(0, 100, 20)) +
  		# set the x axis in steps of 2 years
  		scale_x_continuous(breaks = seq(0, max(plot_data$time) + 2, 2)) +
  		# add numbers at risk when failed (label creates a border)
		annotate(geom = "label", x = subset(plot_data$time, plot_data$n.censor == 0), 
			y = -15, label = subset(plot_data$n.risk, plot_data$n.censor == 0)) +
		annotate(geom = "text", x = 0.2, y = -8, label = "At Risk", fontface = "italic")
	return(plot)
}
...
#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:#:  
 ...
 dplyr::mutate(nombre_region=as.factor(dplyr::recode(region_del_centro,
  ...
  #physical diagnose
    ...
    dplyr::select(row, sexo_2, escolaridad_rec, freq_cons_sus_prin, tipo_de_plan_2, tipo_de_programa_2, edad_al_ing_grupos, sus_principal_mod, motivodeegreso_mod_imp, origen_ingreso_mod, dg_cie_10_rec, estado_conyugal_2, edad_ini_cons, cat_ocupacional, estatus_ocupacional, macrozona, nombre_region, tipo_centro,dias_treat_imp_sin_na, dg_fis_anemia, dg_fis_card, dg_fis_in_study, dg_fis_enf_som, dg_fis_ets, dg_fis_hep_alc, dg_fis_hep_b, dg_fis_hep_cro, dg_fis_inf, dg_fis_otr_cond_fis, dg_fis_otr_cond_fis_ries_vit, dg_fis_pat_buc, dg_fis_pat_ges_intrau, dg_fis_trau_sec, otros_pr_sm_abu_sex, otros_pr_sm_exp_com_sex, otros_pr_sm_otros, otros_pr_sm_prision, otros_pr_sm_vif,dg_trs_cons_sus_or)
...
#CONS_C1_df_dup_JUL_2020_explore%>% janitor::tabyl(sus_principal_mod)
  ...
  dplyr::select(row, sexo_2, escolaridad_rec, freq_cons_sus_prin, tipo_de_plan_2, tipo_de_programa_2, edad_al_ing_grupos, sus_principal_mod, motivodeegreso_mod_imp, origen_ingreso_mod, dg_cie_10_rec, estado_conyugal_2, edad_ini_cons, cat_ocupacional, estatus_ocupacional, macrozona, nombre_region, tipo_centro,dias_treat_imp_sin_na, dg_fis_anemia, dg_fis_card, dg_fis_in_study, dg_fis_enf_som, dg_fis_ets, dg_fis_hep_alc, dg_fis_hep_b, dg_fis_hep_cro, dg_fis_inf, dg_fis_otr_cond_fis_ries_vit, dg_fis_otr_cond_fis, dg_fis_pat_buc, dg_fis_pat_ges_intrau, dg_fis_trau_sec, otros_pr_sm_abu_sex, otros_pr_sm_exp_com_sex, otros_pr_sm_otros, otros_pr_sm_prision, otros_pr_sm_vif, dg_trs_cons_sus_or)
...
table <- compareGroups(selected ~ sexo_2+ escolaridad_rec+ freq_cons_sus_prin+ tipo_de_plan_2+ tipo_de_programa_2+ 
        edad_al_ing_grupos+  sus_principal_mod+  motivodeegreso_mod_imp+  origen_ingreso_mod+  dg_cie_10_rec+  estado_conyugal_2+  edad_ini_cons+  cat_ocupacional+  estatus_ocupacional+  macrozona+  nombre_region+  tipo_centro+ dias_treat_imp_sin_na+ dg_fis_anemia+ dg_fis_card+ dg_fis_in_study+ dg_fis_enf_som+ dg_fis_ets+ dg_fis_hep_alc+ dg_fis_hep_b+ dg_fis_hep_cro+ dg_fis_inf+ dg_fis_otr_cond_fis_ries_vit+ dg_fis_otr_cond_fis+ dg_fis_pat_buc+ dg_fis_pat_ges_intrau+ dg_fis_trau_sec+ otros_pr_sm_abu_sex+ otros_pr_sm_exp_com_sex+ otros_pr_sm_otros+ otros_pr_sm_prision+ otros_pr_sm_vif+ dg_trs_cons_sus_or,
                       method= c(sexo_2=3,
                                   escolaridad_rec=3,
...
restab <- createTable(table, show.p.overall = F)
 export2md(restab, size=9, first.strip=T, hide.no="no", position="center",
...
Next, we discarded posterior treatments.
...
CONS_C1_df_dup_SEP_2020_prev2 %>% 
    dplyr::filter(!is.na(a_botar)) %>% #nrow() 85,048
...
We calculated a total rate from person time, which could be represented as:

$$
\begin{aligned}
\frac{\sum cases}{\sum p-t}
\end{aligned}
$$

<br>

The total incidence rate was `r formatC(17764/(sum(CONS_C1_df_dup_SEP_2020_prev3$person_years))*1000,digits=1,format = "f")` readmissions per person-years.
...
CONS_C1_df_dup_SEP_2020_prev3 %>% 
  dplyr::mutate(mot_egres_latewdl=dplyr::case_when(grepl("Tard",motivodeegreso_mod_imp)~1,
                                                        grepl("Terap",motivodeegreso_mod_imp)~0,
                                                        TRUE~NA_real_)) %>% 
  dplyr::mutate(mot_egres_latewdl=factor(mot_egres_latewdl,labels=c("0.Therapeutic Disch.","1.Late WDRL"))) %>% 
  dplyr::mutate(mot_egres_latewdl=factor(mot_egres_latewdl,)) %>% 
  dplyr::mutate(mot_egres_earlywdl=dplyr::case_when(grepl("Temprano",motivodeegreso_mod_imp)~1,
                                                        grepl("Terap",motivodeegreso_mod_imp)~0,
                                                        TRUE~NA_real_)) %>% 
  dplyr::mutate(mot_egres_earlywdl=factor(mot_egres_earlywdl,labels=c("0.Therapeutic Disch.","1.Early WDRL"))) %>% 
  dplyr::mutate(droga_rec_coc=dplyr::case_when(grepl("Coca",sus_principal_mod)~1,
                                                        grepl("Alcohol",sus_principal_mod)~0,
                                                        TRUE~NA_real_)) %>% 
  dplyr::mutate(droga_rec_coc=factor(droga_rec_coc,labels=c("0.Alcohol","1.Cocaine"))) %>% 
  dplyr::mutate(droga_rec_pbase=dplyr::case_when(grepl("Pasta",sus_principal_mod)~1,
                                                        grepl("Alcohol",sus_principal_mod)~0,
                                                        TRUE~NA_real_)) %>% 
  dplyr::mutate(droga_rec_pbase=factor(droga_rec_pbase,labels=c("0.Alcohol","1.Paste Base"))) %>% 
  dplyr::mutate(droga_rec_mar=dplyr::case_when(grepl("Mari",sus_principal_mod)~1,
                                                        grepl("Alcohol",sus_principal_mod)~0,
                                                        TRUE~NA_real_)) %>% 
  dplyr::mutate(droga_rec_mar=factor(droga_rec_mar,labels=c("0.Alcohol","1.Marijuana")))
```
...
Compared with those with alcohol as the main substance at admission, there were small differences in the cumulative incidence for those using marijuana. However, users that were admitted for marijuana had lower incidence of readmission (`r round(as.numeric(irrs_mar$estimate[1]),2)` [IC95% `r round(irrs_mar$conf.int[1],2)`-`r round(irrs_mar$conf.int[2],2)`]) than users admitted for alcohol.
...
```{r fig1_survplot_km_early_widthdrawal, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 1. Cum. Hazards to Experience Readmission to SUD Treatment, by cause of Discharge", error=T,fig.width=8}
library(survminer)
...
```{r fig2_survplot_km_late_widthdrawal, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 2. Cum. Hazards to Experience Readmission to SUD Treatment, by cause of Discharge", error=T,fig.width=8}
library(survminer)
...
```{r fig3_survplot_coc, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 3. Cum. Hazards to Experience Readmission to SUD Treatment, by Primary Substance At Admission", error=T,fig.width=8}
library(survminer)
...
ggsurvplot_coc<-
  ggsurvplot(fit_coc, 
           ...
           ylim=c(0,10),
           legend = c(0.88, 0.15), 
           legend.title="Main Drug at Admission",
           #cumevents=T,
           surv.connect = T,
           palette = c("skyblue4","orangered4"))
 # scale_y_continuous(breaks = sort(c(seq(0, 100, 10), 56)))
...
```{r fig4_survplot_km_coc, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 4. Cum. Hazards to Experience Readmission to SUD Treatment, by Primary Substance At Admission", error=T,fig.width=8}
library(survminer)
...
```{r fig5_survplot_km_mar, dpi = 96, warning=F,message=F,fig.align='center', fig.cap="Figure 5. Cum. Hazards to Experience Readmission to SUD Treatment, by Primary Substance At Admission", error=T,fig.width=8}
library(survminer)
...
CONS_C1_df_dup_SEP_2020_prev2_explore<-
    CONS_C1_df_dup_SEP_2020_prev4%>% 
    dplyr::mutate(event=ifelse(duplicates_filtered>1,1,0)) %>% 
...
label(CONS_C1_df_dup_SEP_2020_prev2_explore$sexo_2) <- "Sex"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$escolaridad_rec) <- "Educational Attainment"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$freq_cons_sus_prin) <- "Frequency of use of primary drug at admission"

label(CONS_C1_df_dup_SEP_2020_prev2_explore$tipo_de_plan_2) <- "Treatment Plan"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$tipo_de_programa_2) <- "Treatment Program"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$edad_al_ing_grupos) <- "Age at Admission"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$sus_principal_mod) <- "Primary substance at admission"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$motivodeegreso_mod_imp) <- "Motive of discharge"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$origen_ingreso_mod) <- "Motive of Admission to Treatment (First Entry)"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_cie_10_rec) <- "Psychiatric comorbidity"

label(CONS_C1_df_dup_SEP_2020_prev2_explore$estado_conyugal_2) <- "Marital Status"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$edad_ini_cons) <- "Age of Onset of Drug Use"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$cat_ocupacional) <- "Occupational Category"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$estatus_ocupacional) <- "Occupational Status"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$macrozona) <- "Macrozones of the Center"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$nombre_region) <- "Chilean Region of the Center"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$tipo_centro) <- "Type of Center"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dias_treat_imp_sin_na) <- "Days of Treatment (missing dates of discharge were replaced with difference from 2019-11-13)"

label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_anemia) <- "Physical Dg. Anemia"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_card) <- "Physical Dg. Heart Disease"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_in_study) <- "Physical Dg. Under Study"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_enf_som) <- "Physical Dg. Somatic illnesses"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_ets) <- "Physical Dg. STDs"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_hep_alc) <- "Physical Dg. Alcoholic hepatitis"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_hep_b) <- "Physical Dg. Hepatitis B"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_hep_cro) <- "Physical Dg. Chronic hepatitis"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_inf) <- "Physical Dg. Infectuous diseases"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_otr_cond_fis_ries_vit) <- "Physical Dg. Other phsysical conditions"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_otr_cond_fis) <- "Physical Dg. Other phsysical conditions"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_pat_buc) <- "Physical Dg. Oral-care pathologies"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_pat_ges_intrau) <- "Physical Dg. Development or Intrauterine"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_fis_trau_sec) <- "Traumatisms and disabling sequelae"

label(CONS_C1_df_dup_SEP_2020_prev2_explore$otros_pr_sm_abu_sex) <- "Other mental health problems- Sexual abuse"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$otros_pr_sm_exp_com_sex) <- "Other mental health problems- Commercial sexual exploitation"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$otros_pr_sm_otros) <- "Other mental health problems- Other"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$otros_pr_sm_prision) <- "Other mental health problems- Inprisonment"
label(CONS_C1_df_dup_SEP_2020_prev2_explore$otros_pr_sm_vif) <- "Other mental health problems- Domestic violence"

label(CONS_C1_df_dup_SEP_2020_prev2_explore$dg_trs_cons_sus_or) <- "Diagnosis of substance use"

table <- compareGroups(event ~ sexo_2+ escolaridad_rec+ freq_cons_sus_prin+ tipo_de_plan_2+ tipo_de_programa_2+ 
        edad_al_ing_grupos+  sus_principal_mod+  motivodeegreso_mod_imp+  origen_ingreso_mod+  dg_cie_10_rec+  estado_conyugal_2+  edad_ini_cons+  cat_ocupacional+  estatus_ocupacional+  macrozona+  nombre_region+  tipo_centro+ dias_treat_imp_sin_na+ dg_fis_anemia+ dg_fis_card+ dg_fis_in_study+ dg_fis_enf_som+ dg_fis_ets+ dg_fis_hep_alc+ dg_fis_hep_b+ dg_fis_hep_cro+ dg_fis_inf+ dg_fis_otr_cond_fis_ries_vit+ dg_fis_otr_cond_fis+ dg_fis_pat_buc+ dg_fis_pat_ges_intrau+ dg_fis_trau_sec+ otros_pr_sm_abu_sex+ otros_pr_sm_exp_com_sex+ otros_pr_sm_otros+ otros_pr_sm_prision+ otros_pr_sm_vif+ dg_trs_cons_sus_or,
                       method= c(sexo_2=3,
                                   escolaridad_rec=3,
                                   freq_cons_sus_prin=3,
                                  ...
                                  otros_pr_sm_vif=3,
                                  dg_trs_cons_sus_or=3),
                       data = CONS_C1_df_dup_SEP_2020_prev2_explore,
                       include.miss = T,
                       var.equal=T
                       )
 #Possible values are: 1 - forces analysis as "normal-distributed"; 2 - forces analysis as "continuous non-normal"; 3 - forces analysis as "categorical"; and 4 - NA, which performs a Shapiro-Wilks test to decide between normal or non-normal. 
...
pvals <- getResults(table)
#p.adjust(pvals, method = "BH")
restab <- createTable(table, show.p.overall = T)
 export2md(restab, size=9, first.strip=T, hide.no="no", position="center",
           format="html",caption= "Table 3. Summary descriptives table by groups, between selected rows (=1) and records that did not matched, duplicate treatments and invalid ones (=0)")%>%
  kableExtra::add_footnote(c("Note. Variables of C1 dataset had to be standardized before comparison;", "Continuous variables are presented as Medians and Percentiles 25 and 75 were shown;", "Categorical variables are presented as number (%)"), notation = "none")%>%
  kableExtra::scroll_box(width = "100%", height = "375px")
```
