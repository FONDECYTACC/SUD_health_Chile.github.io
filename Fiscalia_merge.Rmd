...
if (grepl("CISS Fondecyt",path)==T){
    ...
    setwd(paste0(path,"/"));load(paste0(path,"/8.Rdata"))
  }
rm(list=setdiff(ls(), c("CONS_C1_df_dup_SEP_2020_match","CONS_C1_df_dup_SEP_2020","CONS_C1","C1_fech_nac_nas","CONS_C1_df_dup_FEB_2020_prev25b")))
```
...
library(readxl)
base_fiscalia_orig <- read_excel(gsub("/Fiscalia_merge.Rmd","/BASE.xlsx",dirname(rstudioapi::getSourceEditorContext()$path)), 
...
library(readxl)
Base_fiscalia_v2 <- read_excel(gsub("/SUD_CL/Fiscalia_merge.Rmd","/Base_v3_dic_2021.xlsx",dirname(rstudioapi::getSourceEditorContext()$path)), 
sheet = "Base", skip = 4, guess_max = min(1000000, Inf))%>% janitor::clean_names() %>% 
  dplyr::mutate(gls_region=dplyr::case_when(gls_region=="REGION METROPOLITANA CENTRO NORTE"~ "RM Centro Norte",
                                            gls_region=="REGION METROPOLITANA OCCIDENTE"~ "RM Occidente",
                                            gls_region=="REGION METROPOLITANA ORIENTE"~ "RM Oriente",
