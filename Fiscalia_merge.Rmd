...
tab2b<- as.data.frame.TableOne(CreateTableOne(vars = myVars_nva, data = Base_fiscalia_v2, factorVars = catVars_bd_nva, smd=T, includeNA=T,test=T, addOverall = T, strata="encontrado_como_imputado"), nonnormal= c("edad_comision","edad_ter_rel"),smd=T) %>% 
  dplyr::mutate(char2=characteristic, level2=level) %>% 
  tidyr::fill(char2) %>% 
  dplyr::select(char2,everything())%>% 
  dplyr::mutate(level=ifelse(is.na(level),"[Missing]",level)) %>% 
  dplyr::mutate(level2=ifelse(is.na(level2),"[Missing]",level2))
...
kable(size=11, format="html",caption= "Table 1. Summary descriptives, by accused status",smd=T, test=T, varLabels=T,noSpaces=T, printToggle=T, dropEqual=F, col.names=c("Variable ","Level",rep(c("Total (%)","No", "Yes", "p-value","test", "SMD"),2)))%>%
...
knitr::kable(format="html",caption="Table 7. An example of distinct combination of charges by different criminal processes") %>% 
...
```{r 3_5_ser, echo=T, error=T, paged.print=TRUE, eval=F}
...
```{r 3_5_ser_wrong, echo=T, error=T, paged.print=TRUE, eval=F}
