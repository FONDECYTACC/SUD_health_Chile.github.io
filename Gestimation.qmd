...
system.time(fit2 <- aftgee(Surv(time, status) ~ tipo_de_plan_res_1 + TD, data = subset(ms_d_match_surv_res_tm,id %in% subs_gee), id = id, weights=wcomp$ipw.weights, corstr = "ar2", B =b_iter))
...
system.time(fit1b <- aftgee(Surv(time, status) ~ tipo_de_plan_res_1*factor(strata) + TD, data = subset(ms_d_match_surv_res_tm,id %in% subs_gee), id = id, weights=wcomp$ipw.weights, corstr = "ar1", B =b_iter))
system.time(fit2b <- aftgee(Surv(time, status) ~ tipo_de_plan_res_1*factor(strata) + TD, data = subset(ms_d_match_surv_res_tm,id %in% subs_gee), id = id, weights=wcomp$ipw.weights, corstr = "ar2", B =b_iter))
system.time(fit3b <- aftgee(Surv(time, status) ~ tipo_de_plan_res_1*factor(strata) + TD, data = subset(ms_d_match_surv_res_tm,id %in% subs_gee), id = id, weights=wcomp$ipw.weights, corstr = "ind", B =b_iter))
system.time(fit4b <- aftgee(Surv(time, status) ~ tipo_de_plan_res_1*factor(strata) + TD, data = subset(ms_d_match_surv_res_tm,id %in% subs_gee), id = id, weights=wcomp$ipw.weights, corstr = "unstructured", B =b_iter))
system.time(fit5b <- aftgee(Surv(time, status) ~ tipo_de_plan_res_1*factor(strata) + TD, data = subset(ms_d_match_surv_res_tm,id %in% subs_gee), id = id, weights=wcomp$ipw.weights, corstr = "ex", B =b_iter))

#     user   system  elapsed 
# 38684.36    34.92 38715.95 
# 38715.95 /(60*60) = 10 hrs por cada uno, con 100 B. Debiese ser 5x= 2 días por cada uno
#en mi computador toma 14027.92 (4 hrs),  pero en este,  33504.06 
# 4hrs. por 5 veces el B anterior= 20. 200 hrs.= 8 días toma hacerlo
...
En el ejemplo, se ve que son readmitidos mayor `r 1/exp(-1.128)`

exp(-0.4708723) efecto tratamiento

exp(0.4032758) efecto treatment discharge

DEBIESE COMPARAR AR1 AR2 Y AR3 PARA VER CUAL TIENE MEJOQ QIC

DE AHI, HACER UN MODELO CON LA INTERACCIÓN ENTRE RESIDENTIAL Y CADA UNO A LA BASE
