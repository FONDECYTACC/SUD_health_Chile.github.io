...
#ACTUALIZADO install.packages(paste0(gsub("\\\\","/",Sys.getenv("GUROBI_HOME")),"/R/gurobi_9.1-2.zip"), repos = NULL, type="source")
...
smd_df_prev_match_after_match %>%
 ...
 # dplyr::mutate(unlist_var_nms_22= wrap.labels(unlist_var_nms_22, 25)) %>% 
  ...
  guides(y = guide_axis_nested::guide_axis_nested(delim = "~-~", order=1))

loveplot_bal3

if(no_mostrar==1){
  
jpeg(paste0(gsub("SUD_CL.*","",path),"loveplot_bal32_2022.jpg"), height=8, width= 8, res= 500, units = "in")
loveplot_bal3
dev.off()
}
```
