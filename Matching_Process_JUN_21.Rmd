...
rm(list=setdiff(ls(), c("CONS_C1_df_dup_SEP_2020_match","CONS_C1_df_dup_SEP_2020","CONS_C1","path")))
```
...
match.on_tot <- c("row", "hash_key","sus_principal_mod","sus_ini_mod_mvv","estado_conyugal_2","escolaridad_rec","edad_ini_cons","freq_cons_sus_prin","origen_ingreso_mod","dg_cie_10_rec","nombre_region","sexo_2","edad_al_ing","fech_ing_num","abandono_temprano_rec","tipo_de_plan_res","duplicates_filtered","dg_trs_cons_sus_or","evaluacindelprocesoteraputico", "tenencia_de_la_vivienda_mod", "condicion_ocupacional_corr")
# `tipo_centro_pub`, `abandono_temprano_rec`, and `tipo_de_plan_res` don't exist.
...
CONS_C1_df_dup_SEP_2020_match %>%
    ...
    dplyr::mutate_at(.vars = vars(c(tipo_de_plan_res,sus_ini_mod_mvv, estado_conyugal_2,escolaridad_rec,edad_ini_cons,freq_cons_sus_prin,origen_ingreso_mod, dg_cie_10_rec, nombre_region, tipo_centro_pub, sexo_2,edad_al_ing,dg_trs_cons_sus_or,evaluacindelprocesoteraputico)),
                     .funs = ~ifelse(is.na(.), 1, 0)) %>% 
      dplyr::ungroup() %>%
dplyr::mutate(sumVar=rowSums(dplyr::select(., c("tipo_de_plan_res","sus_ini_mod_mvv", "estado_conyugal_2","escolaridad_rec","edad_ini_cons","freq_cons_sus_prin","origen_ingreso_mod", "dg_cie_10_rec", "nombre_region", "tipo_centro_pub", "sexo_2","edad_al_ing","dg_trs_cons_sus_or")))) %>% 
...
From the figure above, we could see that the starting substance (`sus_ini_mvv`), the onset of drug use (`edad_ini_cons`), and the evaluation of the therapeutic process (`evaluacindelprocesoteraputico`) counted for approximately 6% of the missing data. These values should be imputed. We first focused on the age of onset of drug use. It is important to consider that the evaluation of the therapeutic process could be distorted due to censoring (many users did not finish their treatment, and did not have this evaluation in the study period).
...
We started looking over the missing values in the age at admission (n= `r missing.values %>% dplyr::select(edad_al_ing) %>% as.numeric()`). Since `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss %>% dplyr::group_by(hash_key) %>%  dplyr::mutate(miss_edad_al_ing=ifelse(is.na(edad_al_ing),1,0),sum_miss_edad_al_ing=sum(miss_edad_al_ing,na.rm=T)) %>% dplyr::ungroup() %>% dplyr::filter(sum_miss_edad_al_ing>0, duplicates_filtered>1) %>% nrow()==0," there were not cases with more than one treatment that did not have an age of admission, we did not have to impute taking into account serial dependencies in the dates of treatment.","**there were some users that had more than one treatment, it is necessary to analyse serial dependencies into the dates that were candidates to imputation**.")`
...
Based on the figure above, the age of onset of drug use was similar between the imputed values and the observed. However, we followed the rules stated in the `Duplicates` process ([link](https://fondecytacc.github.io/SUD_health_Chile.github.io/Duplicates)). There were three logical conditions to fulfill in order to adequately replace these values in the database: the age of onset must not be greater than the age of onset of drug use in the primary substance at admission (1), the age of oonset may not be greater than the age of admission to treatment (2), and the age of onset of drug use must be greater than 4 years. Then, we selected the minimum value of age of onset of drug use among the imputed values, because one user could not have more than one age of onset of drug use. 
...
paste0("Number of rows with values that did not fulfill the conditions: ",CONS_C1_df_dup_SEP_2020_match_miss1 %>% 
    dplyr::filter(is.na(edad_ini_cons)) %>% 
...
paste0("Number of rows with values that did not fulfill the conditions after replacement with the minimum by users: ",CONS_C1_df_dup_SEP_2020_match_miss1 %>% 
    dplyr::filter(is.na(edad_ini_cons)) %>% 
    dplyr::select(hash_key, edad_ini_cons, edad_al_ing,edad_ini_sus_prin) %>% nrow())
...
There were `r CONS_C1_df_dup_SEP_2020_match_miss1 %>% dplyr::filter(is.na(edad_ini_cons)) %>% nrow()` cases of imputed ages of onset of drug use that did not fulfill the conditions necessary to replace the missing values with the imputed ones.
...
As a result of the process of imputation, `r ifelse(CONS_C1_df_dup_SEP_2020_match_miss9 %>% dplyr::filter(is.na(tipo_de_plan_res)) %>% nrow()==0,paste0("there were no missing values imputed."),paste0(" some values were not possible to impute (n=",CONS_C1_df_dup_SEP_2020_match_miss9 %>% dplyr::filter(is.na(tipo_de_plan_res)) %>% nrow(),")."))`
...
#broom::tidy(MASS::rlm(fech_ing_num ~ tipo_de_plan_res, data = subset(d_match_surv, dup==1)), conf.int=T) %>% copiar_nombres()
```
...
var_names_2022<- 
  list("origen_ingreso_mod_Spontaneous"="Motive Admission~-~Spontaneous",
    ...
    "freq_cons_sus_prin_1 day a week or more"="Freq Drug Cons~-~1d/wk or more",
    "freq_cons_sus_prin_2 to 3 days a week"="Freq Drug Cons~-~2 to 3d/wk",
    "freq_cons_sus_prin_4 to 6 days a week"="Freq Drug Cons~-~4 to 6d/wk",
    "freq_cons_sus_prin_Daily"="Freq Drug Cons~-~Daily",
    "freq_cons_sus_prin_Did not use"="Freq Drug Cons~-~Did not use",
    "freq_cons_sus_prin_Less than 1 day a week"="Freq Drug Cons~-~Less 1d/wk",
    "nombre_region_Antofagasta (02)"="Region~-~Antofagasta(02)",
    ...
    "dg_trs_cons_sus_or"= "Drug Dependence~-~TRUE",
    "edad_ini_cons"="Age of Onset of Drug Use~-~ ",
    "condicion_ocupacional_corr_Employed"="Occ.Status~-~Employed",
    "condicion_ocupacional_corr_Inactive"="Occ.Status~-~Inactive",
    "condicion_ocupacional_corr_Looking for a job for the first time"="Occ.Status~-~Looking 1st job",
...
comp_status_readmission_modality <-
...
Excluding censored cases, the incidence rate of readmission was significantly higher (`r round(as.numeric(comp_status_readmission$estimate[1]),2)`, 95% IC `r round(comp_status_readmission$conf.int[1],2)`-`r round(comp_status_readmission$conf.int[2],2)`) in users that had a Discharge without clinical advice, compared to users that had a Therapeutic discharge (p`r ifelse(sprintf("%1.3f",comp_status_readmission$p.value)=="0.000","<0.001",paste0("= ", sprintf("%1.3f",comp_status_readmission$p.value)))`).
...
```{r fig5 mstate_models_3s, echo=T, fig.align='center', message=FALSE, fig.caption="Figure 17. Tree Structure for the Multistate System, Ten-states model", error=T, fig.height=6, fig.width=12, eval=T, dpi=320}
library(Epi)
...
if(no_mostrar==1){
jpeg(paste0(gsub("SUD_CL/Matching_Process_JUN_21.Rmd","",path),"_mult_state_ags/ph_test1.jpg"), height=7, width= 13, res= 320, units = "in")
...
if(no_mostrar==1){
jpeg(paste0(gsub("SUD_CL/Matching_Process_JUN_21.Rmd","",path),"_mult_state_ags/ph_test2.jpg"), height=7, width= 13, res= 320, units = "in")
...
```{r cox_zph,eval=T, echo=T, paged.print=TRUE}                                 
...
invisible(scales::scientific(c(0.000420, 0.000314,  0.000525), digits = 3))
...
if (grepl("CISS Fondecyt",path)==T){
    save.image("C:/Users/CISS Fondecyt/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_jun.RData")
  } else if (grepl("andre",path)==T){
