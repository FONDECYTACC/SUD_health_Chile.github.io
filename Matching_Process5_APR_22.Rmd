...
#load("C:/Users/CISS Fondecyt/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_apr22.RData")
...
```{r first_try_2,eval=T, echo=T, paged.print=TRUE, error=T}                                 
#load("C:/Users/CISS Fondecyt/Mi unidad/Alvacast/SISTRAT 2019 (github)/mult_state_5_jun.RData")
coxphw_pwp<-
  coxphw(Surv(Tstart,Tstop,status) ~ tipo_de_plan_res_1+  
...
res_pwp_w <- cbind.data.frame(Avg_HR=round(exp(coxphw_pwp$coeff),2),
                             Lo_ci95=round(exp(confint(coxphw_pwp))[,1],2),
                             Up_ci95=round(exp(confint(coxphw_pwp))[,2],2),
                             p=round(summary(coxphw_pwp)$prob,4))%>% 
  data.table::data.table(keep.rownames=T)
res_pwp_w2 <- cbind.data.frame(Avg_HR=round(exp(coxphw_pwp2$coeff),2),
                             Lo_ci95=round(exp(confint(coxphw_pwp2))[,1],2),
                             Up_ci95=round(exp(confint(coxphw_pwp2))[,2],2),
                             p=round(summary(coxphw_pwp2)$prob,4))%>% 
  data.table::data.table(keep.rownames=T)
...
Once set as constant those terms that had no time-varying effects, we found that treatment setting at baseline had a hazard ratio of `r paste0(round(exp(0.1410),3),"[",round(exp(0.0522),3),"-", round(exp(0.23000),3),"]")`.
...
#     tidyr::separate(V1,c("Variables","Coef", "SE", "Robust SE", "z", "Sig.", "95%CI_lo", "95%CI_up"), sep="\\s+") %>%
  ...
  dplyr::mutate(Variables=dplyr::case_when(grepl("tipo_de_plan", Variables)~"Treatment setting at baseline",
