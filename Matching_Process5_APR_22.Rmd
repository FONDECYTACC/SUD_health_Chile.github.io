...
coxphw_pwp_int_gt<-
  coxphw(Surv(time,status) ~ tipo_de_plan_res_1*strata(trans)+  
           TD_1+ TD_2+ TD_3+ TD_4+ cluster(id) + strata(trans), data = dt_ms_d_match_surv,template="AHR",  robust=T)
coxphw_pwp_int2_gt<-
  coxphw(Surv(time,status) ~ tipo_de_plan_res_1+ tipo_de_plan_res_1:strata(trans)+ TD_1+ TD_2+ TD_3+ TD_4+ cluster(id), data = dt_ms_d_match_surv,template="AHR",  robust=T)

coxph_pwp_int_gt<-
  coxph(Surv(time,status) ~ tipo_de_plan_res_1*strata(trans)+  
           TD_1+ TD_2+ TD_3+ TD_4+ cluster(id) + strata(trans), data = dt_ms_d_match_surv)
coxph_pwp_int2_gt<-
  coxph(Surv(time,status) ~ tipo_de_plan_res_1+ tipo_de_plan_res_1:strata(trans)+ TD_1+ TD_2+ TD_3+ TD_4+ cluster(id) + strata(trans), data = dt_ms_d_match_surv)
...
timecox_pwp_const2_opc1<- 
  ...
  timecox(Surv(Tstart,Tstop,status) ~ const(tipo_de_plan_res_1)*const(strata(trans))+ 
            TD_1+ const(TD_2)+ const(TD_3)+ const(TD_4)+ cluster(id), Nit = 1000,
          data = dt_ms_d_match_surv, n.sim=3000, robust = 1, weighted.test = 1)
```
</div>
...
timecox_pwp_const1_gt<- 
  timecox(Surv(time,status) ~ tipo_de_plan_res_1*strata(trans)+ 
            TD_1+ const(TD_2)+ TD_3+ const(TD_4)+ cluster(id) + strata(trans), Nit = 100, data = dt_ms_d_match_surv, n.sim=3000, robust = 1, weighted.test = 1)
```
</div>
...
timecox_pwp_const2_opc2_gt<- 
  timecox(Surv(time,status) ~ const(tipo_de_plan_res_1)*strata(trans)+ 
            const(TD_1)+ const(TD_2)+ const(TD_3)+ const(TD_4)+ cluster(id), Nit = 1000, data = dt_ms_d_match_surv, n.sim=3000, robust = 1, weighted.test = 1)
```
</div>
