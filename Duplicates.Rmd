...
We used a neural network model to obtain valid dates of birth of the `r CONS_C1_df_dup_ENE_2020_prev52 %>% dplyr::filter(is.na(fech_nac))%>% distinct(HASH_KEY) %>% nrow()` users that did not have a valid date of birth, or those cases that had a date at admission of less than 15 years old. We did not selected the cutpoint at 16 years old, because one user (616a9f5147684b5bb649eff9936db849) had an age at admission of 15 years old, but also had another entry with the same date of birth in the dataset of 2019.  This led us to think that in specific cases, users of 15 years old could be admissible.
