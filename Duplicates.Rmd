...
cbind(Coef.=rbind("Intercept","Date of Discharge(NULL)", "Cause of Discharge(NULL)", "Days of Treatment(NULL)", "Date of Last Treatment(NULL)"),
      round(data.table(rbind(round(Epi::ci.lin(m1, Exp=T),3))),3)) %>% 
...
sample_plot <- ans3 %>% 
  ...
  geom_segment(aes(x = fech_ing, xend = fech_egres,
    ...
    labs(x = "Dates of admission and discharge", y = "", caption="Note. Only patients that share characteristics overlap between them",
         subtitle="Sample of 25 cases. Colored lines represent different rows in the dataset")+
...
In Figure 4, we may notice that `r nrow(ans3)` entries from `r ans3 %>% distinct(HASH_KEY_2) %>% nrow()` may share the same HASH Key, both are SENDA programs, but the date of admission is lesser than the date of discharge of another case, and the date of discharge is greater than the date of admission of that other case, given that he other case does not correspond to the same row. These conditions let us see how many cases overlap with another entry in the dataset.
