...
#Join datasets 
  ...
  dplyr::mutate(Edad_al_ing=replace(Edad_al_ing, is.na(Edad), NA)) %>%
   dplyr::select(-id_mod.TOP, -duplicated_HASH_date) %>%  
    #dplyr::group_by(Edad) %>% summarise(n=n()) %>% View()
...
Leaving behind the last questions that may condition the analysis, we ran a probabilistic match based on the criteria defined above in the **`dtalink`** software. In Table 17 there is a selection of probabilistics matches that have elements that are worth of discussion.  Most of them have many information in common, describing derivations to another center or parallel treatments, but come characteristics change, requiring to be precautious to not automatically delete one of them.
...
<!--- tipo_centro, convertir en factor 1. Privado, 2. Público;  --->
