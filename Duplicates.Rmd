...
```{r echo=T, paged.print=TRUE}
#Tomo a los que tienen una combinación de ID y HASH Key distinta al resto
#Con esto puede que esté subestimando la cantidad de casos (filas) con concatenación distinta.
# de esos todos combinados distintos, saco los que tienen un ID duplicado
#selecciona a los IDs duplicados, los ordena y...
#filtra en base a tener distinto ID
CONS_C1_df %>% mutate(concat=paste0(id,"_",HASH_KEY)) %>% dplyr::distinct(concat, .keep_all = TRUE) %>% #aquí ya dejo de ver n° de casos
  dplyr::filter(duplicated(id)) %>% dplyr::arrange(id) %>%  #filtro los casos de entre los que ya descarté duplicados de la combinación de id y HASH
  distinct(id) %>% assign("ids_more_one_hash",., envir = .GlobalEnv) # saco los tipos distintos de IDs que hay por hash, de los casos en que hay distintas                                                                              combinaciones de IDs y hash, y que en ese subgrupo existen IDs duplicados.

#luego, aplica esos casos a toda la población
CONS_C1_df %>%
dplyr::filter(id %in% as.character(as.vector(unlist(as.data.table(unlist(ids_more_one_hash)))))) %>% # selecciono los ids de los casos citados
dplyr::arrange(id) %>% #ordeno por ids 
  dplyr::filter(duplicated(HASH_KEY)) %>% #filtro, 
  #762 casos.
...
We found that the age is a time invariant variable. In order to identify similar matches, we need to identify the date of admission and the HASH Key, but considering the year. In Table 9 we can see that 45,526 cases had at least one case with the same combination of HASH-Key and date of admission.
...
# Duplicated rows
  ...
  as.data.frame(.) %>%  
  knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                   caption="Table 9. Duplicated rows of the combination of HASH-Key & Date of Admission", col.names= c(" ", " Frequencies", "Percentage"), 
                 align =rep('c', 2))  %>%
