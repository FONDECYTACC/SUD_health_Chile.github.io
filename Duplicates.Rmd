...
In Figure 2 we can see every case and the distribution of duplicated 
...
CONS_C1_df %>%
  ...
  dplyr::summarize(n=n()) %>%
  knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                   caption="Table 13. Missing Date of Discharge, Difference between Treatment Dates", col.names= c("Treat Days", "N"),  align =rep('c', 2))  %>%
    kable_styling(bootstrap_options = c("striped", "hover"),font_size = 12) %>%
...
```{stata dtalink, eval=FALSE}
  ...
  *dtalink concat, cutoff(4) id(id_match) bestmatch noweight
