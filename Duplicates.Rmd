...
CONS_C1_df_dup_ENE_2020_prev4 %>%
  ...
  dplyr::mutate(OBS=case_when((edades_por_hash>1 & ids_por_hash>1 & HASH_reps>2 & !is.na(id.C1_mf_age))~paste0(OBS,";","1.6.1. Replaced diff ages w/ MF age"),
                              TRUE ~ OBS))%>%
  ...
  dplyr::mutate(OBS=case_when((edades_por_hash>1 & ids_por_hash>1 & HASH_reps==2 & !is.na(id.C1_max_year))~paste0(OBS,";","1.6.2. Replaced diff ages w/ recent dataset"), 
                              TRUE ~ OBS))%>%
  ...
  dplyr::mutate(OBS=case_when((edades_por_hash>1 & ids_por_hash==1 & !is.na(id.C1_min_diff))~paste0(OBS,";","1.6.3. Replaced diff ages w/ most precise age"), 
                           TRUE ~ OBS))%>%
...
- Deletion of cases in Parole (n= `r CONS_C1_df_dup_ENE_2020_prev5 %>% dplyr::filter(Tipo.de.Plan == "PAI LV") %>% nrow()`)
