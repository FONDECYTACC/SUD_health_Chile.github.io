...
data.table::data.table(CONS_C1_df_dup_ENE_2020_prev) %>%
  ...
  dplyr::mutate(OBS=ifelse(dup_todo>1, "1.1. Duplicated Cases in Almost Every Variable", "")) %>%
...
dup_partes<-data.table::as.data.table(CONS_C1_df_dup_ENE_2020_prev)[, dup_partes := .N, by = names_c1_stage2] %>%
distinct(row)
...
data.table::data.table(CONS_C1_df_dup_ENE_2020_prev) %>%
  ...
  dplyr::mutate(OBS=ifelse(dias_trat<0,glue::glue("{OBS};1.2. Negative Days of Treatment, Changed Date of Discharge"),OBS))%>% # APLICADO EN ABRIL 2020, PARA HACER UN CONTROL A LOS CASOS EXTRAÑOS
  dplyr::mutate(fech_egres=ifelse(dias_trat<0,NA,as.character(fech_egres)))%>% # APLICADO EN ABRIL 2020, AL VER QUE 9 CASOS FUERON OBVIADOS
  ...
  dplyr::mutate(OBS=ifelse(HASH_KEY %in% as.character(as.vector(unlist(as.data.table(unlist(dup_partes))))),
                           paste0(OBS,";","1.3. Duplicated Cases In Treatment and Substance Use Characteristics"), OBS)) %>%
...
```{r table 6, echo=T, paged.print=TRUE, warning=F, message=F}
CONS_C1_df %>%
...
#Join datasets 
  ...
  dplyr::mutate(OBS=ifelse((Edad<18 & !is.na(Edad.y))|(Edad>90 & !is.na(Edad.y)),paste0(OBS,";","1.4. Replaced invalid ages with same users within the dataset"), OBS))%>%
...
#CONS_C1_df_dup_ENE_2020_prev3 %>% group_by(Edad) %>% summarise() %>% View() #son puros NA's
  ...
  dplyr::left_join(CONS_C1_df_dup_ENE_2020_prev3, CONS_TOP_df_dup_age, by="HASH_KEY", suffix= c("",".TOP")) %>%
    ...
    dplyr::mutate(OBS=ifelse(is.na(Edad) & !is.na(id.TOP),paste0(OBS,";","1.5. Replaced invalid ages with TOP information"), OBS))%>%
...
From Table 12, we can observe cases that could have different ages because this value is calculated dynamically given the date of retrieval, adding some variability among users (let’s say, differences of a few days that traduces into differences of age). However, some of the differences in ages within HASHs could be caused by mistyping in the registry, or other reasons not related to the process of retrieval.
...
```{r fig2_dec_tree_disc_dup_age, fig.height=4, fig.width=8, warning=FALSE, fig.align = "center", message=F,fig.cap="Figure 2. Decision Tree for the Discard of Duplicated Ages by User"}
#plot(CONS_C1_df_dup_ENE_2020_prev2$ano_nac, CONS_C1_df_dup_ENE_2020_prev2$Edad, ylab="Age", xlab="Year of Birth")
knitr::include_graphics("G:/Mi unidad/Alvacast/SISTRAT 2019 (github)/SUD_CL/Figures/Figure2_Duplicated1.svg")
```

<br>

In the first place, we replaced only the cases which had more than one age per HASH. Then we checked whether each HASH Key had more than one ID. If there were more than one, we selected the most frequent value. If there were only two  cases with different IDs, we selected data that comes from the most recent dataset (assuming that each of these entries had more control over each case), followed by the case with greater age. Finally, if there was only one ID by HASH, we selected cases in which there is only one ID. However, there were no pairs of cases with the sameHASH, the same ID, and from the same yearly dataset. These cases would have produced a tie in terms of criteria exposed.

<br>

```{r final_assignment, echo=T,cache=T, paged.print=TRUE}
#Join datasets 
require(lubridate)
...
CONS_C1_df_dup_ENE_2020_prev4 %>%
  ...
  dplyr::mutate(OBS=ifelse((edades_por_hash>1 & ids_por_hash>1 & HASH_reps>2 & !is.na(id.C1_mf_age)),paste0(OBS,";","1.6.1. Replaced diff ages w/ MF age"), OBS))%>%
  ...
  dplyr::mutate(OBS=ifelse((edades_por_hash>1 & ids_por_hash>1 & HASH_reps==2 & !is.na(id.C1_max_year)),paste0(OBS,";","1.6.2. Replaced diff ages w/ recent dataset"), OBS))%>%
  ...
  dplyr::mutate(OBS=ifelse((edades_por_hash>1 & ids_por_hash==1 & !is.na(id.C1_min_diff)),paste0(OBS,";","1.6.3. Replaced diff ages w/ most precise age"), OBS))%>%
...
- Deletion of cases in Parole (n= `r CONS_C1_df_dup_ENE_2020_prev5 %>% dplyr::filter(is.na(Tipo.de.Plan) | Tipo.de.Plan != "PAI LV") %>% nrow()`)
...
- Collapse and Standardize Substances into the following categories: hallucinogens (such as LSD, mushrooms); cocaine; amphetamine-type stimulants (eg., methamphetamine, ecstasy); inhalants; marihuana; opioids (eg., heroin, methadone, painkillers); cocaine paste base and crack; other substances; sedatives, hypnotics and tranquilizers; and others (such as anabolic steroids)
...
- Declare as an invalid value the category “Unknow” of the route of administration of the main substance
- Declare as an invalid value the category “Unknow” of the frequency of consumption of the main Substance
...
#LV se elimina el tipo de plan
  dplyr::mutate(OBS= ifelse(Tipo.de.Plan %in% c("M-PAI2","M-PR2","PG PAI 2","Otro","CALLE"),paste0(OBS,";","1.7.01. Collapsed Treatment Plans"),OBS))%>%
...
#En SER 2020, dejé los Calle y Otro en PG-PAB, pero ahora lo dejaré en PG-PR
  dplyr::mutate(OBS= ifelse(identidad.de.genero=="Masculino" & sexo=="Hombre" & tipo_de_plan %in% c("M-PAI", "M-PR", "M-PAB"), paste0(OBS,";","1.7.02. Standardized Plans By Sex & Gender Id"),OBS))%>%
...
#Si la edad de inicio de consumo es mayor a la edad actual, lo mismo la edad de iniciación a la sustancia principal, las declaro perdidas.
  dplyr::mutate(OBS=ifelse(Edad.Inicio.Consumo<=Edad, paste0(OBS,";","1.7.03. Invalid Age Of Onset of Drug Use, Higher than age"),OBS))%>%
  dplyr::mutate(edad_ini_cons= ifelse(Edad.Inicio.Consumo<=Edad, Edad.Inicio.Consumo, NA)) %>%
  dplyr::mutate(OBS=ifelse(Edad.Inicio.Consumo<=Edad, paste0(OBS,";","1.7.04. Invalid Age Of Onset of Primary Substance, Higher than age"),OBS))%>%
...
#Si se inicia en la sustancia principal o inicia el consum a una edad menor a 5 años, lo declaro como perdido.
  dplyr::mutate(OBS=ifelse(edad_ini_cons<5, paste0(OBS,";","1.7.05. Invalid Age Of Onset of Drug Use, < 5 yrs age"),OBS))%>%
  dplyr::mutate(edad_ini_cons= ifelse(edad_ini_cons<5,NA,edad_ini_cons)) %>%
  dplyr::mutate(OBS=ifelse(edad_ini_cons<5, paste0(OBS,";","1.7.06. Invalid Age Of Onset of Primary Substance, < 5 yrs age"),OBS))%>%
...
#Embarazos en hombres se declaran como perdidos
  dplyr::mutate(OBS=ifelse(embarazo=="Si" & sexo=="Hombre",paste0(OBS,";","1.7.07. Invalid Pregnancy= Pregnant Man"),OBS))%>%
...
#CAMBIOS DIAS
  dplyr::mutate(OBS=ifelse(dias_trat<0,paste0(OBS,";","1.7.08. Negative Treatment Days, Changed Treat Days"),OBS))%>%
...
#data.table(table(CONS_C1_df_dup_ENE_2020_prev3$Otras.Sustancias.nº1))
  ...
  dplyr::mutate(otras_sus1= dplyr::recode(otras_sus1,
                                              ...
                                              "Esteroides Anabólicos"="Otros",
                                              "Hongos"="Alucinógenos")) %>%
dplyr::mutate(OBS=ifelse(otras_sus1=="SIN CONSUMO",paste0(OBS,";","1.7.09. Other Substances1, Invalid due to No Consumption"),OBS))%>%
...
#data.table(table(CONS_C1_df_dup_ENE_2020_prev3$Otras.Sustancias.nº2))
  ...
  dplyr::mutate(otras_sus2= dplyr::recode(otras_sus2,
                                              ...
                                              "Esteroides Anabólicos"="Otros",
                                              "Hongos"="Alucinógenos")) %>%
dplyr::mutate(OBS=ifelse(otras_sus2=="SIN CONSUMO",paste0(OBS,";","1.7.10. Other Substances2, Invalid due to No Consumption"),OBS))%>%    
...
#data.table(table(CONS_C1_df_dup_ENE_2020_prev3$Otras.Sustancias.nº3))
  ...
  dplyr::mutate(otras_sus3= dplyr::recode(otras_sus3,
                                              ...
                                              "Esteroides Anabólicos"="Otros",
                                              "Hongos"="Alucinógenos")) %>%
dplyr::mutate(OBS=ifelse(otras_sus3=="SIN CONSUMO",paste0(OBS,";","1.7.11. Other Substances3, Invalid due to No Consumption"),OBS))%>%    
...
dplyr::mutate(OBS=ifelse(estado_conyugal=="Nocontesta",paste0(OBS,";","1.7.12. Marital State, Invalid due to No Response"),OBS))%>%    
...
dplyr::mutate(OBS=ifelse(escolaridad=="NO SABE O NO SE APLICA",paste0(OBS,";","1.7.13. Educational Attainment, Invalid due to No Response"),OBS))%>%    
...
dplyr::mutate(OBS=ifelse(via_adm_sus_prin== "No sabe",paste0(OBS,";","1.7.14. Route of Adm Primary Substance, Invalid due to Unknown"),OBS))%>%    
...
dplyr::mutate(OBS=ifelse(freq_cons_sus_prin== "Desconocida",paste0(OBS,";","1.7.15. Frequency of Consumption Primary Substance, Invalid due to Unknown"),OBS))%>%    
...
```{r fig3_dup_rows, fig.height=4, fig.width=8, warning=FALSE, fig.align = "center", message=F}
require(zoo)
...
From Figure 3, we can see that people that admitted to treatments from 2007 to 2008 presented most of the duplicated cases, while treatments that started from the second semester of 2015 had almost no duplicated cases. (n= `r dup_cases_quarter_n`).
...
```{r Fig_4, fig.height=4, fig.width=8, warning=FALSE, fig.align = "center", message=F}
...
ggplot(aes(n)) +
  ...
  labs(title=paste0("Figure 4. Histogram of HASH Keys in matches (n=",matches_from_stata_c1 %>% group_by(hash_key) %>% summarise(n=n()) %>% nrow(), ")"), y = "Freq.")+
...
```{r  Fig5, fig.height=8, fig.width=10, warning=FALSE, fig.align = "center", message=F, cache=T}
...
as.data.table(overlap_dates_C1) %>%
  ...
  theme(plot.caption = element_text(hjust = 0, face= "italic")) +
    ...
    ggtitle("Figure 5.1 Trajectories of HASHs from dates of admission to discharge")
```
</div>
...
```{r  Fig 5_v2, fig.height=14, fig.width=8, warning=FALSE, fig.align = "center", message=F, cache=T}
c26 <- c(
...
sample_plot <- overlap_dates_C1 %>% 
  ...
  scale_color_manual(values=c26) +
    ...
    ggtitle("Figure 5.2 Trajectories of every HASH from dates of admission to discharge")
    #+
...
clean_plot <- CONS_C1_df_dup_ENE_2020 %>%
  ...
  #scale_x_date(breaks = scales::date_breaks("1 year"), date_labels = "%b %d") +
    ...
    ggtitle("Figure 5.3 Trajectories of every HASH from dates of admission to discharge")
...
In Figure 5.2, we selected 25 randomly assigned cases with overlapped treatments. We may appreciate that horizontal lines (users) had overlapped colors (treatments) over time. In contrast, Figure 5.3 shows four examples of clear trajectories that do not have overlaps over time and are separated by a white gap.
...
Table 16 shows the entries with missing dates of discharge, the difference between the date of retrieval (Second week of November of 2019), and the days of treatment, by each yearly dataset obtained. NA’s seen represent cases in which the number of treatment days was not available, **These w **. Datasets from 2010 to 2018 would have been retrieved 8 days earlier than the dataset of 2019. SENDAs professionals believed that treatments should not last more than 1005 days. That is why we think that **cases in datasets of 2018 and 2019 may still be in treatment until the date of retrieval (leaving an approximate of `r print(n_casos[[1]])` HASHs and date of admissions left to analyze). We asked SENDAs professionals about the cases that are still being treated and came from older datasets **. Meanwhile, we looked over those cases with the same dates of admission and HASH that had a recent date of discharge that could replace the missing value.
...
```{r image-ref-for-in-text, echo=T, fig.align='center', message=FALSE, fig.caption="Figure 6. Summary of Process of Data Cleaning and Standardization"}
