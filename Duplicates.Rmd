...
# Duplicated/ Repeated Cases in SISTRAT C1
...
# OR plot
m1=glm(duplicated ~ is.na(sexo) + is.na(fech_ing) + is.na(fech_egres) + is.na(motivodeegreso) + is.na(dias_trat) + is.na(Fecha.Ultimo.Tratamiento), data=CONS_C1_df, family = binomial ( link = logit )) 
m2=glm(duplicated ~ factor(sexo) + fech_ing + fech_egres + factor(motivodeegreso) + dias_trat + Fecha.Ultimo.Tratamiento, data=CONS_C1_df, family = binomial ( link = logit )) 

#star.1 <- stargazer(m1, m2,
...
#print(rbind(xtable::xtable(m1), type = "html"),xtable::xtable(m2, type = "html"))

#xtable::xtable(list(m1,m2), title="Regression of Duplicated Data", align=TRUE, no.space=TRUE,dep.var.labels=c("Null Values","Factor"), 
#                center = TRUE, doctype = FALSE, caption = "", star.symbol = "\\*") %>%

#table3 <- tabglm(m1, latex = TRUE,columns = c("beta.se", "betaci"), compress.factors=F)
#> Waiting for profiling to be done...
#kable(table3, 
#      caption = "Table 3. Logistic regression fit (created by `tabglm` and `kable`).")
#kable(xtable(linear.1))
log_reg<- rbind(summary(m1)$coefficients,summary(m2)$coefficients)

knitr::kable(cbind(Model=rbind("NA's","","","","","","Factor/Numeric","","","","","","","","","","","","",""),
      Coef.=rbind("Intercept","Date of Admission (NULL)", "Date of Discharge(NULL)", "Cause of Discharge(NULL)", "Days of Treatment(NULL)", "Date of Last Treatment(NULL)","Intercept","Sex(Women)", "Date of Admission", "Date of Discharge", "Cause of Discharge- Administrative Discharge", "Cause of Discharge- Therapeutic Resolution","Cause of Discharge- Derivation.","Cause of Discharge- Death", "Days of Treatment", "Date of Last Treatment- 3-4 yrs","Date of Last Treatment- 5 + yrs","Date of Last Treatment- NA","Date of Last Treatment- 12 months","Date of Last Treatment- 6 months"),
      as.data.table(log_reg)), digits=3, scientific=FALSE,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 3. Case of example that showed more than one HASH",align =rep('c', 101)) %>%
...
CONS_TOP_df_dup_age <- dplyr::select(CONS_TOP, HASH_KEY, ID, Sexo, Edad)%>% 
  ...
  dplyr::select(-ID) %>%
  dplyr::filter(!duplicated(HASH_KEY)) #85490 rows, 4 columns
...
CONS_C1_df_dup %>% 
  ...
  knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
               caption="Table 12. Replaced Information from TOP dataset",
                 align =rep('c', 101))  %>%
...
#:#_#_#_#_#__#_#_#_#__#_#_#_#_#__#_#__#_#_#_#_#_#__
dt <- data.table(CONS_C1_df, key=c("HASH_KEY","fech_ing", "fech_egres"))
dt <- na.omit(dt, cols= c("HASH_KEY","fech_ing", "fech_egres")) #baja a 153392 casos
ovl <- foverlaps(dt, dt, type = "within")
#THIS CODE DOES NOT RUN, BUT ITS NECESSARY TO UNDERSTAND THE OVERLAPPING
