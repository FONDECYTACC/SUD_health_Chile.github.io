...
CONS_C1_df_dup_ENE_2020_prev4 %>%
  dplyr::filter(Tipo.de.Plan!="PAI LV") %>% #Deletion of cases in Parole
...
#Si se inicia en la sustancia principal o inicia el consumo a una edad menor a 5 años, lo declaro como perdido.
...
#CAMBIOS DIAS
  ...
  CONS_C1_df_dup_ENE_2020_prev4 %>% dplyr::mutate(dias_trat_alta_temprana=ifelse(dias_trat>=90,0,1)) %>%
...
dplyr::select(-Edad.y, -Sexo.TOP, -Edad.TOP, -dup_todo) %>%
as.data.frame(.) %>%  
...
matches_from_stata_c1 %>%
  dplyr::arrange(`_score`,`_matchID`,hash_key) %>%
  dplyr::select(`_score`, `_matchID`, hash_key, ano_bd, idcentro, date_in, fech_ing, fech_egres, edad, dias_trat) %>%
  as.data.frame() %>%
    knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
                   caption="Table 13. Preliminary View of Matches from Stata", align =rep('c', 100)) %>%
     kableExtra::kable_styling(bootstrap_options = c("striped", "hover"),font_size = 8) %>%
    kableExtra::scroll_box(width = "100%", height = "350px")
```
<br>
...
hist(matches_from_stata_c1 %>% group_by(hash_key) %>% summarise(n=n()) %>% as.data.frame() %>%dplyr::select(n), main=paste0("Figure 3. Histogram of HASH Keys in matches (n=",matches_from_stata_c1 %>% group_by(hash_key) %>% summarise(n=n()) %>% as.data.frame() %>% summarise(length(n)),")"), breaks=20,xlab="Repetitions", ylab="Freq.")
```
...
require(sqldf)
require(gridExtra)
  ans3 <- janitor::clean_names(sqldf("SELECT *
