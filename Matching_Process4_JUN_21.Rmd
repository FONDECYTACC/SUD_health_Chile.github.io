...
cbind.data.frame(t=rep(c(90,183,365,1096,1826),each=dim(trans_matrix)[2]),
...
pmatrix2_t_b_df_final %>% 
  ...
  dplyr::filter(t%in% c(90,183,365,1096,1826)) %>%
      dplyr::mutate(t=factor(t, levels=c(90,183,365,1096,1826),
                                labels=c("~3 months", "~6 months", "~1 year", "~3 years", "~5 years")))
...
pmatrix2_t_a_df_final %>% 
  ...
  kableExtra::pack_rows("~3 months", 1, dim(trans_matrix)[2]-1) %>% 
  kableExtra::pack_rows("~6 months", dim(trans_matrix)[2], dim(trans_matrix)[2]*2-2) %>%
  kableExtra::pack_rows("~1 year", dim(trans_matrix)[2]*2-1, dim(trans_matrix)[2]*3-3) %>%
  kableExtra::pack_rows("~3 years", dim(trans_matrix)[2]*3-2, dim(trans_matrix)[2]*4-4) %>%
  kableExtra::pack_rows("~5 years", dim(trans_matrix)[2]*4-3, dim(trans_matrix)[2]*5-5) %>%
...
pmatrix2_t_a_rp_df_1096_final %>% 
  ...
  dplyr::filter(!grepl("5",trans)) %>% 
        dplyr::mutate(trans=factor(trans, levels=c(1:4),
                                labels=c("1)Admission","2)Readmissio","3)2nd Readmission","4)3rd Readmission"))) %>% 
          knitr::kable(.,format = "html", format.args = list(decimal.mark = ".", big.mark = ","),
...
newdat2_a<-newdat_a %>% dplyr::mutate_at(vars(starts_with("TD")),~factor(1))
newdat2_b<-newdat_b %>% dplyr::mutate_at(vars(starts_with("TD")),~factor(1))
newdat2_a_1096<-newdat_a_1096 %>% dplyr::mutate_at(vars(starts_with("TD")),~factor(1))
#2021-07-06, had newdat_a_1096, changed it for b
newdat2_b_1096<-newdat_b_1096 %>% dplyr::mutate_at(vars(starts_with("TD")),~factor(1))
...
```{r sim_final_plot1, eval=T, echo=T, paged.print=TRUE, fig.height=8, fig.cap="Figure 1a. Time to Readmission for Users who Experience it (Mean and 95%CIs)", fig.align="center", error=T}
simfinal_fmsm_st_df %>%
  ...
  scale_color_manual(name ="Type of program",values=c("#0E53A7","#FFB540"), labels= c("Outpatient","Residential"))+
  scale_fill_manual(name ="Type of program",values=c("#0E53A7","#FFB540"), labels= c("Outpatient","Residential"))+
...
cbind.data.frame(tolos_t_b_str4_df,program="Outpatient", start=4)
  ...
  dplyr::mutate(comb=dplyr::case_when(start==1 & state==2~"a",
                                      start==2 & state==3~"a",
                                      start==3 & state==4~"a",
                                      start==4 & state==5~"a",
                                      T~NA_character_)) %>% 
  ...
  dplyr::mutate(t=factor(t,
                levels=c("90","183","365","1096","1826"), 
                labels=c("~3 months", "~6months", "~1 year", "~3 years", "~5 years"))) %>% 
